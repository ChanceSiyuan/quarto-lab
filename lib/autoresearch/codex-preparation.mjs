...
export async function preflightPreparationCodex({ codexPath, skillPath, schemaPath, processRunner = runProcess }) {
  ...
  try {
    await processRunner({ command: codexPath, args: ["--version"], timeoutMs: PREPARATION_PREFLIGHT_TIMEOUT_MS, env: {} });
    await processRunner({ command: codexPath, args: ["login", "status"], timeoutMs: PREPARATION_PREFLIGHT_TIMEOUT_MS, env: {} });
  } catch (error) {
...
export async function runPreparationCodex({ codexPath, stageDir, problem, problemMarkdown, answers, schemaPath, processRunner = runProcess }) {
  nonEmptyPath(codexPath, "codexPath");

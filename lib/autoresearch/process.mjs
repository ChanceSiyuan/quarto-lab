...
function copyAllowedEnvironment(environment) {
  const result = {};
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      child = spawnFn(command, args, {
        cwd,
        env: copyAllowedEnvironment(env),
        shell: false,
    ...
    const append = (stream, chunk, limit) => {
      ...
      if ((stream === "stdout" ? stdoutBytes : stderrBytes) > limit) {
        stop(child, killFn, "SIGTERM");
        finishReject(new ProcessOutputLimitError(stream, limit));
        return false;
      }
    ...
    const onExit = (code, signal) => {
      if (code === 0) finishResolve({ stdout, stderr, code, signal });
      else finishReject(new ProcessExecutionError(code, signal, stdout, stderr));
    };

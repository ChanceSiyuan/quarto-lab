...
function outcomePasses(id, outcome) {
  ...
  if (id === "score-arithmetic" || id === "baseline-reproduction") return outcome.ok === true && Number.isFinite(outcome.score) && outcome.score === outcome.baselineScore;
  return outcome.ok === true;
}

async function runCommand({ id, stageDir, manifest, privateDataRoot, processRunner }) {
  ...
  const options = {
    ...
    env: hostEnvironment(), timeoutMs: manifest.resources.attemptTimeoutSeconds * 1000, shell: false,
  };
  if (evaluator) Object.assign(options, { privateDataRoot, privateDataReadOnly: true });
...
export async function runInfrastructurePreflight({ stageDir, manifest, privateDataRoot, processRunner = runProcess, now = () => new Date() }) {
  ...
  for (const id of PREFLIGHT_CHECK_IDS.slice(2)) {
    try {
      const outcome = await runCommand({ id, stageDir, manifest: validated, privateDataRoot, processRunner });
      const passed = outcomePasses(id, outcome);
      const evaluator = EVALUATOR_CHECKS.has(id);

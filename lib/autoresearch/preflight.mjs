...
export async function runInfrastructurePreflight({ stageDir, manifest, privateDataRoot, processRunner = runProcess, now = () => new Date(), environment = process.env }) {
  ...
  try { for (const id of PREFLIGHT_CHECK_IDS.slice(2)) {
    try {
      ...
      const outcome = await runCommand({ id, stageDir, manifest: validated, privateDataRoot: evaluatorRoot, processRunner, env });
      if (evaluator) await ensureSnapshotUnchanged(privateSnapshot);
      let passed = outcomePasses(id, outcome);
      if (id === "baseline-reproduction") {
        const repeated = await runCommand({ id, stageDir, manifest: validated, privateDataRoot: privateSnapshot.root, processRunner, env });

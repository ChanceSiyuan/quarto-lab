...
const FORWARD = new Map([
  ...
  ["preparing_datasets", new Set(["preflight", "failed", "interrupted"])],
...
export function createJobStore({ rootDir = process.cwd(), now = () => new Date(), randomBytes = systemRandomBytes } = {}) {
  ...
  async function create({ problemId, kind, parentJobId = null } = {}) {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      const job = { jobId, problemId, kind, parentJobId, state: "queued", createdAt, updatedAt: createdAt };
      try {
  ...
  async function recoverInterrupted() {
    const recovered = [];
    for (const job of await list()) if (EXECUTING_STATES.has(job.state)) {
      const next = { ...job, state: "interrupted", updatedAt: timestamp(now) };

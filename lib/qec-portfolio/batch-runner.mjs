  ...
  return Object.freeze({
    async run() {
      ...
      for (const row of problems) {
        let snapshot = await verifiedSnapshot(activeValuationStore, row.id);
        ...
        try {
          const started = await activeAssessmentManager.start(row.id, { valuationSnapshotId: snapshot.manifest.snapshotId });

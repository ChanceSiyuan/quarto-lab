...
function valuationIntervalOk(interval) {
  return isRecord(interval)
...
function quantitativeValueOk(value) {
  if (!isRecord(value) || !["known", "unknown"].includes(value.state)) return false;
  if (value.state === "unknown") return hasOnlyFields(value, ["state", "reason"]) && nonEmptyString(value.reason);
  return hasOnlyFields(value, ["state", "interval", "unit", "visibility", "evidenceState", "evidenceTier", "sourceIds", "sources"])
    && valuationIntervalOk(value.interval)
    && nonEmptyString(value.unit)
    && visibilityOk(value.visibility)
    && ["reported", "inferred"].includes(value.evidenceState)
    && ["primary", "authoritative-secondary", "vendor-or-news", "assumption"].includes(value.evidenceTier)
    && Array.isArray(value.sourceIds)
    && value.sourceIds.every(nonEmptyString)
...
function collectScoreAnchorErrors(assessment, errors) {
  ...
  for (const anchor of quantitative.scoreAnchors) {
    ...
    const evidenceText = anchor.evidenceIds.join(" ").toLowerCase();
    if ((/scientific|citation|momentum/.test(evidenceText) || anchor.momentumAdjustment !== undefined)
      && anchor.dimensionId !== "importance") {
      errors.push("scientific attention and momentum may anchor Importance only, never novelty.");
    }
    if (/coverage/.test(evidenceText) || /concentration/.test(evidenceText)) {
...
export function validateScoreAnchors(assessment) {
  ...
  collectScoreAnchorErrors(assessment, errors);
  return errors.length === 0 ? { ok: true, value: assessment.quantitativeEvidence.scoreAnchors } : { ok: false, errors };
}

function validateQuantitativeEvidence(assessment, errors) {
  const quantitative = assessment.quantitativeEvidence;
  ...
  if (!hasOnlyFields(quantitative, fields)
    ...
    || !hasOnlyFields(quantitative.scientificAttention, ["value", "momentum", "coverage", "concentration", "warnings"])
...
function validateAssessmentObject(assessment, resolution, errors) {
  ...
  if (assessment.schemaVersion === 2) validateQuantitativeEvidence(assessment, errors);
...
export function summarizeCompletedAssessment({ run, envelope, computed }) {
  ...
  if (assessment.schemaVersion === 2) {
    ...
    summary.quantitative = {
      scientificAttention: quantitative.scientificAttention.value,
      technicalSuccess: quantitative.technicalFeasibility,

...
import { PROBLEM_ID_PATTERN, QUANTUM_AREAS } from "../problems/schema.mjs";
...
export function createAssessmentService({ rootDir = process.cwd(), token, manager, valuationManager = null, portfolioReader = null } = {}) {
  ...
  return http.createServer(async (request, response) => {
    try {
      ...
      const valuationProblem = pathMatch(pathname, /^\/__local\/assessments\/problems\/([^/]+)\/valuation$/);
      if (request.method === "GET" && valuationProblem) {
        if (!validProblemId(valuationProblem[0])) return send(response, 400, { error: "INVALID_PROBLEM_ID" });
        if (!valuationManager) return send(response, 404, { error: "NOT_FOUND" });
        return send(response, 200, await valuationManager.getProblemState(valuationProblem[0]));
      }

      const valuationStart = pathMatch(pathname, /^\/__local\/assessments\/problems\/([^/]+)\/valuation\/jobs$/);
      if (request.method === "POST" && valuationStart) {
        if (!validProblemId(valuationStart[0])) return send(response, 400, { error: "INVALID_PROBLEM_ID" });
        if (!valuationManager) return send(response, 404, { error: "NOT_FOUND" });
        const body = await readJsonBody(request);
        if (body.scopeOverride !== undefined && (typeof body.scopeOverride !== "string" || !QUANTUM_AREAS.includes(body.scopeOverride))) {
          return send(response, 400, { error: "INVALID_SCOPE_OVERRIDE" });
        }
        const result = await valuationManager.start(valuationStart[0], body.scopeOverride === undefined ? {} : { scopeOverride: body.scopeOverride });
        return send(response, result.accepted ? 202 : 400, result);
      }

      const valuationConfirmation = pathMatch(pathname, /^\/__local\/assessments\/valuation\/jobs\/([^/]+)\/confirmation$/);
      if (request.method === "POST" && valuationConfirmation) {
        if (!validRunId(valuationConfirmation[0])) return send(response, 400, { error: "INVALID_RUN_ID" });
        if (!valuationManager) return send(response, 404, { error: "NOT_FOUND" });
        const result = await valuationManager.confirm(valuationConfirmation[0], await readJsonBody(request));
        return send(response, result.accepted ? 202 : 400, result);
      }

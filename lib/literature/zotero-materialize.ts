...
export async function materializeZoteroRecord(options: {
  ...
  try {
    ...
    await rm(path.join(stagedPaper, "source"), { recursive: true, force: true });
    await rm(path.join(stagedPaper, "figures"), { recursive: true, force: true });
    await rm(path.join(stagedPaper, "paper.pdf"), { force: true });
    const extraction = await extractSourceArchive({
      archive: sourceArchive,
      citekey: options.itemKey,
      sourceDir: path.join(stagedPaper, "source"),
      figuresDir: path.join(stagedPaper, "figures"),
    });
    await writeFile(path.join(stagedPaper, "paper.pdf"), pdf, { mode: 0o644 });
    ...
    record.latex = { entrypoint: `source/${extraction.mainTex}` };

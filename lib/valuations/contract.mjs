...
function validUnit(unit) {
  return nonEmptyString(unit) && (monetaryUnit(unit) !== null || [
    "fraction", "percent", "count", "hours", "years", "qubits", "logical-qubits", "physical-qubits", "operations",
  ].includes(unit));
}
...
export function validateAtomicEvidence(value) {
  ...
  if (value.state === "unknown") {
    if (!hasOnlyFields(value, ["state", "reason"]) || !nonEmptyString(value.reason)) errors.push("unknown values require only a reason.");
    return errors.length === 0 ? { ok: true, value } : { ok: false, errors };
  }

  const allowed = ["id", "state", "interval", "unit", "visibility", "evidenceState", "evidenceTier", "sourceIds", "sources", "currency", "priceBaseYear", "conversionSourceId", "derivation", "kind"];
...
function validateReferences(items, type, typeIds, allIds, inputIds, sourceIds, entityIds, errors) {
...
export function validateQuantitativeEvidence(value) {
  ...
  validateReferences(value.stages, "stages", ids.stages, allIds, inputIds, sourceIds, ids, errors);
  validateReferences(value.outputs, "outputs", ids.outputs, allIds, inputIds, sourceIds, ids, errors);
  validateReferences(value.assumptions, "assumptions", ids.assumptions, allIds, inputIds, sourceIds, ids, errors);
  if (!Array.isArray(value.scoreAnchors)) errors.push("scoreAnchors must be an array.");
  else for (const anchor of value.scoreAnchors) {
    if (!isRecord(anchor) || !nonEmptyString(anchor.id) || allIds.has(anchor.id)) { errors.push("scoreAnchors must have unique IDs."); continue; }
    ids.scoreAnchors.add(anchor.id);

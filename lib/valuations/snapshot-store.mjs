...
function identityFreeManifest(manifest) {
  ...
  const { snapshotId: _snapshotId, contentHash: _contentHash, ...content } = manifest;
  return content;
}
...
export function createValuationSnapshotStore({ rootDir = process.cwd(), now = () => new Date() } = {}) {
  ...
  return Object.freeze({
    async readInputs(problemId) {
      ...
      const publicInputs = await readJsonIfPresent(join(base, "inputs.json"), {});
      const privateInputs = await readJsonIfPresent(join(base, "inputs.private.json"), {});
    ...
    async writeInputs(problemId, inputs) {
      ...
      const base = await valuationDir(problemId);
      await writeFile(join(base, "inputs.json"), `${canonicalJson(inputs)}\n`);

...
export function createOpenAlexClient({ fetchFn = globalThis.fetch, apiKey, baseUrl = DEFAULT_BASE_URL, maxWorks = DEFAULT_MAX_WORKS, now = () => new Date() } = {}) {
  ...
  return Object.freeze({
    async expand({ anchors = [], topicIds = [], normalizedProblem = "" } = {}) {
      ...
      const referencedIds = [...new Set(seeds.flatMap((work) => (work.referenced_works ?? []).map(stableId).filter(Boolean)))].sort().slice(0, limit);
      ...
      const retained = [
        ...[...normalized.values()].filter((paper) => anchorIds.has(paper.id)).sort((left, right) => left.id.localeCompare(right.id)),
        ...[...normalized.values()].filter((paper) => !anchorIds.has(paper.id)).sort((left, right) => left.id.localeCompare(right.id)),
      ].slice(0, limit);

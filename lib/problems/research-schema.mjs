...
function rangesAreContiguous(ranges) {
  return ranges.every((range, index) => isObject(range)
    ...
    && (index === 0 || range.first === ranges[index - 1].last + 1));
}
...
function validateSourceRanges(ranges, sourceCommit, add) {
  ...
  for (const range of ranges) {
    for (let sequence = range.first; Number.isInteger(sequence) && sequence <= range.last; sequence += 1) {
      if (expectedRange(sequence)?.commit !== sourceCommit) {
        add("attemptRanges", "attempt ranges must match sourceCommit in the infrastructure range map.");
        return;
      }
    }
  }

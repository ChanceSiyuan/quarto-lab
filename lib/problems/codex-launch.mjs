...
Before discussing or writing the candidate, scan every existing problems/Prob-NNN directory and every parseable manifest ID. Treat both sources as reserved IDs, including damaged or otherwise invalid records. If ${nextProblemId} is already reserved, stop and do not write or overwrite anything; report that a fresh index and candidate ID are required.
...
Prepare and validate all required problem files in a temporary staging location outside problems/${nextProblemId}; do not make a draft problem.json visible to the indexer. Validate the staged manifest and problem.md with this repository's schema before publishing anything.
...
Copy problem.md and the generation audit records into the new directory first. Publish problem.json last using an atomic rename from the staged, already validated manifest. Never expose problem.json while problem.md or generation audit records are incomplete.

...
function expectedAttemptIds(manifest) {
  ...
  return Array.from(
    { length: manifest.attemptCount },
    (_, index) => `ATT-${String(first + index).padStart(3, "0")}`,
  );
}
...
function compareAttemptDirectorySet(entries, expectedIds, attemptsRelativePath) {
  ...
  for (const attemptId of expectedIds) {
    if (!actual.has(attemptId)) {
      errors.push(diagnostic(attemptsRelativePath, "attempts", `Missing declared attempt directory: ${attemptId}`));
    }
  }
  return errors;
}
...
export async function buildResearchIndex({
  ...
  for (const entry of problemDirectories) {
    ...
    const attemptsRelativePath = relative(workspacePath, attemptsPath);
    const expectedIds = expectedAttemptIds(manifestValidation.value);
    const attempts = [];
    const problemDiagnostics = [];
    problemDiagnostics.push(...compareAttemptDirectorySet(attemptEntries, expectedIds, attemptsRelativePath));
    ...
    for (const attemptId of expectedIds) {
      const attemptPath = join(attemptsPath, attemptId, "attempt.json");

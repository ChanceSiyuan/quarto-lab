...
1. **Targeted Completion & Gap-Filling:** Locate every instance of `(t.b.c.)` in my draft. Replace these placeholders with rigorous, step-by-step mathematical proofs. 
...
2. **Identifying & Adding Missing Theorems:** As you review the logic linking the `(t.b.c.)` proofs and the surrounding text, watch for implicit dependencies. 
...
* **Traceable Extension:** While you are authorized to use external knowledge to bridge logical gaps, every new concept, mathematical step, or theorem you introduce *must* be standard, verifiable quantum computing/chemistry literature. 
* **Strict Citation:** Any external theorem or significant mathematical trick you bring in to complete the `(t.b.c.)` sections must be properly cited inline and included in the final `.bib` output.

# Workflow Execution
Before generating the final `.qmd` and `.bib` text, please provide a brief outline detailing:
1. The specific mathematical steps and logical flow you intend to use to fill each `(t.b.c.)` gap.

...
import { previewDraft, resolveDraftFile } from "../../../src/lib/drafts/preview.js";
import type { ProcessRunner } from "../../../src/lib/knowledge/site.js";
...
test("the drafts project includes only QMD notes, shares the reviewed bibliography, preserves semantic blocks, disables execution, and builds no site", async () => {
...
test("every draft has promotion-ready title, description, and category frontmatter", async () => {
  const draftsRoot = path.join(REPO_ROOT, "drafts");
  const qmdFiles = (await fileTree(draftsRoot)).filter(
    (file) => file.endsWith(".qmd") && !file.split("/").some((segment) => segment.startsWith(".")),
  );
  assert.ok(qmdFiles.length > 0, "the drafts tree should contain QMD notes");

  for (const relativePath of qmdFiles) {
    const source = await readFile(path.join(draftsRoot, relativePath), "utf8");
    const match = source.match(/^---\n([\s\S]*?)\n---\n/);
    assert.ok(match, `${relativePath}: must start with YAML frontmatter`);
    const metadata = parse(match[1]!, { uniqueKeys: true }) as Record<string, unknown>;
    assert.deepEqual(
      Object.keys(metadata),
      ["title", "description", "categories"],
      `${relativePath}: frontmatter must contain exactly the promotion fields`,
    );
    assert.ok(typeof metadata.title === "string" && metadata.title.trim(), `${relativePath}: title`);
    assert.ok(
    ...
    assert.ok(
      Array.isArray(metadata.categories)
        ...
        && ["theory", "experiment", "codes"].includes(String(metadata.categories[0])),
      `${relativePath}: categories must contain one Research Loop category`,
    );
  }
});
...
test("the retired quantum-harness import is absent from drafts", async () => {
  await assert.rejects(
    stat(path.join(REPO_ROOT, "drafts", "imported-quantum-harness")),
...
/** Runs the packed draft-preview CLI and reports what the shell would see. */
async function runCli(args: readonly string[]): Promise<{ code: number | null; stderr: string }> {
  const child = spawn(
    process.execPath,
    ["--import", "tsx", ".research-loop/tooling/scripts/draft-preview.ts", ...args],
...
test("the preview binds a requested loopback port and refuses one outside the unprivileged range", async (t) => {
  const repo = await makeRepo(t);
  const { calls, runner } = recordingRunner();
  ...
  for (const rejected of [80, 70_000, 1.5, Number.NaN]) {
    await assert.rejects(
      previewDraft({
        repoRoot: repo,
        requestedFile: "drafts/imported/deep/note.qmd",
        previewPort: rejected,
        runner,
      }),
      /port/,
      `port ${rejected} must be refused`,
    );
  }

  assert.equal(calls.length, 1, "a refused port never reaches Quarto");
});

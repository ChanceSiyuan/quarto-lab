...
} from "../../src/lib/problems/research-route-data.mjs";
...
test("problem routes place the local preparation panel between headers and research detail", async () => {
  const page = await readFile(new URL("../../src/app/problems/[id]/page.tsx", import.meta.url), "utf8");
  const panel = await readFile(new URL("../../src/app/problems/[id]/autoresearch-panel.tsx", import.meta.url), "utf8");
  const staticPanel = await readFile(new URL("../../src/app/problems/[id]/static-autoresearch-panel.tsx", import.meta.url), "utf8");
...
test("preserved dashboard and global layout sources are unchanged from the task base", async () => {
  const preservedBase = "7a011205395bbbbc1d15afce73c26099283b7824";

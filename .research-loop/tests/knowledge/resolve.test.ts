...
import {
  ...
  validateGraph,
  validateKnowledge,
} from "../../../src/lib/knowledge/validate.js";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(HERE, "..", "..", "..");
const FIXTURES = path.join(REPO_ROOT, ".research-loop", "tests", "fixtures", "knowledge");
const CLI = path.join(REPO_ROOT, ".research-loop", "tooling", "scripts", "knowledge.ts");
...
test("the public interface exports exactly the entry points of this phase", async () => {
  const publicInterface = await import("../../../src/lib/knowledge/index.js");
...
test("the CLI exits 1 on an invocation it cannot honour", async () => {
  for (const args of [
    ...
    ["build", "--knowledge-dir", ".research-loop/tests/fixtures/knowledge/valid"],
    ["build", "--output-dir", "/tmp/anywhere"],
    ["preview", "--knowledge-dir", ".research-loop/tests/fixtures/knowledge/valid"],
  ]) {

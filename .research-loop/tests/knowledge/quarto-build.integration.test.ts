...
import {
  ...
  type ProcessRunner,
} from "../../../src/lib/knowledge/site.js";
import { KnowledgeValidationError, validateKnowledge } from "../../../src/lib/knowledge/validate.js";
...
const INTEGRATION_TREE: Record<string, string> = {
  ...
  "ising/proof.qmd": [
    ...
    "",
    ":::: {#lem-duality .callout-important icon=\"false\"}",
    "Self-duality fixes the critical point.",
    "::::",
    "",
    "The conclusion follows from @lem-duality.",
    "",
    ":::: {.callout-note collapse=\"true\"}",
    "## Derivation details",
    "The high- and low-temperature expansions exchange under duality.",
    "::::",
    "",
    "$$",
...
test("an editor preview binds only to its requested loopback port", async (t) => {
  const repo = await makeRepo(t);
  const { calls, runner } = recordingRunner(async () => {});

  await previewKnowledgeSite({ repoRoot: repo, runner, previewPort: 43123 });
...
test("an editor preview mirrors only validated source changes into the safe projection", async (t) => {
  const repo = await makeRepo(t);
  const sourcePath = path.join(repo, "knowledge", "index.qmd");
  const original = await readFile(sourcePath, "utf8");
  const updated = `${original}\nLive editor refresh.\n`;
  let editorConfig = "";
  const { calls, runner } = recordingRunner(async (call) => {
    await writeFile(sourcePath, updated);
    const projected = path.join(call.options.cwd, "index.qmd");
    for (let attempt = 0; attempt < 40; attempt++) {
      if (await readFile(projected, "utf8") === updated) {
        editorConfig = await readFile(path.join(call.options.cwd, "_quarto.yml"), "utf8");
        return;
      }
      await new Promise((resolve) => setTimeout(resolve, 50));
    }
    assert.fail("the safe preview projection did not receive the validated source edit");
  });

  await previewKnowledgeSite({
    repoRoot: repo,
    runner,
    previewWatchPath: "knowledge/index.qmd",
  });
  // A live editor compiles one page, not the entire knowledge website.
  // The projection still contains the validated graph for references/assets.
  assert.equal(calls[0].args[1], ".");
  assert.match(editorConfig, /render:\s*\n\s*- index\.qmd/);
});

test("an invalid live edit leaves the last valid preview projection untouched", async (t) => {
  const repo = await makeRepo(t);
  const sourcePath = path.join(repo, "knowledge", "index.qmd");
  const original = await readFile(sourcePath, "utf8");
  const { runner } = recordingRunner(async (call) => {
    ...
    assert.equal(await readFile(projected, "utf8"), original);
  });
...
test("interrupting a preview removes its workspace", { timeout: 120_000 }, async (t) => {
  ...
  const initialWorkspaces = await workspaces(REPO_ROOT);
  const stub = path.join(binDir, "quarto");
  await writeFile(stub, '#!/bin/sh\necho "STUB-QUARTO $*"\nsleep 30\n', { mode: 0o755 });

  const child = spawn(process.execPath, ["--import", "tsx", ".research-loop/tooling/scripts/knowledge.ts", "preview"], {
    cwd: REPO_ROOT,
  ...
  assert.equal(
    (await workspaces(REPO_ROOT)).filter((name) => !initialWorkspaces.includes(name)).length,
    1,
  ...
  assert.deepEqual(await workspaces(REPO_ROOT), initialWorkspaces, "an interrupted preview leaves no new workspace behind");
});
...
test("Quarto renders the whole fixture site, and executes nothing", { timeout: 300_000 }, async (t) => {
  ...
  // 5. Semantic theorem blocks and ordinary collapsible callouts survive the
  // isolated projection and retain their interactive Quarto markup.
  assert.match(proof, /id="lem-duality"/);
  assert.match(proof, /Lemma(?:\s|&nbsp;)+1/, "the custom lemma prefix is numbered");
  assert.match(proof, /href="#lem-duality"/, "the lemma cross-reference is linked");
  assert.match(proof, /Derivation details/);
  assert.match(proof, /callout-collapse/);

  // 6. The citation, resolved against the bibliography copied into the project.
  ...
  // 9. The category views list what the graph filed under them.

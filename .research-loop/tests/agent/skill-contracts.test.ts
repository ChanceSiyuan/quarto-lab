 ...
 * - **Agreement.** The skills, the packed skills guide, and the repository they
...
async function collectMarkdownFiles(relativeRoot: string): Promise<string[]> {
...
for (const name of SKILL_NAMES) {
  ...
  test(`${name} is a self-contained SKILL.md`, async () => {
    ...
    assert.deepEqual(
      entries.sort(),
      ["SKILL.md"],
      `${name}: a local skill is one SKILL.md; helpers belong in packed tooling and src/lib`,
    );
  });
}
...
test("the packed skills guide documents ownership, inputs, writes, and prohibitions", async () => {
  const source = flow(await readFile(path.join(REPO_ROOT, ".research-loop", "docs", "project", "skills.md"), "utf8"));

  const header = /\|\s*Skill\s*\|\s*Role\s*\|\s*Ownership[^|]*\|\s*Trusted inputs\s*\|\s*Writes\s*\|\s*Prohibited[^|]*\|/i;
  assert.match(source, header, "the skills guide needs the skill/role/ownership/inputs/writes/prohibited table");
  for (const name of SKILL_NAMES) {
    assert.match(source, new RegExp(`\\|\\s*\`${name}\``), `the skills guide has no row for ${name}`);
  }

  assert.match(
    source,
    /quantum\.harness[^.\n]*download-ref skill/i,
    "the skills guide must attribute the adapted download-ref skill to its source",
  );
  assert.match(
    source,
    /Superpowers[^.\n]*runtime dependenc/i,
    "the skills guide must state that external Superpowers skills are runtime dependencies",
  );
  assert.match(
    source,
    /not copied into this repository/i,
    "the skills guide must state that external skills are not copied into this repository",
  );
});

test("agent-facing docs do not preserve machine-specific checkout paths", async () => {
  for (const relativePath of ["AGENTS.md", ...(await collectMarkdownFiles(".research-loop/docs/project"))]) {
...
test("every command a skill hands an agent is documented in the packed skills guide", async () => {
  const documented = await readFile(
    path.join(REPO_ROOT, ".research-loop", "docs", "project", "skills.md"),
    "utf8",
  );
  for (const name of SKILL_NAMES) {
    ...
    for (const target of targets) {
      assert.match(
        ...
        `the skills guide does not document "make ${target}", which ${name} tells agents to run`,
      );
    }
  }
});

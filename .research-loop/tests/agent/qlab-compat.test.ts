...
test("the Zotero import snapshot also refreshes the reviewed bibliography non-destructively", async () => {
  const script = await readFile(path.join(ROOT, ".research-loop", "tooling", "scripts", "qlab.ts"), "utf8");
  assert.match(script, /zotero\.bib/);
  assert.match(script, /ref\.bib/);
  assert.match(script, /mergeZoteroBibliography/);
});

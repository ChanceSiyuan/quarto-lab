...
import { buildAddProblemPrompt, buildCodexLaunch } from "../../src/lib/problems/codex-launch.mjs";

test("builds a short discussion-to-draft hand-off prompt", () => {
  const prompt = buildAddProblemPrompt({ nextProblemId: "Prob-007" });
  assert.match(prompt, /npm run skills:ensure-sci-brain/);
  assert.match(prompt, /`brainstorm-ideas` skill/);
  assert.doesNotMatch(prompt, /sci-brain:brainstorm-ideas/);
  assert.match(prompt, /continue in this task; do not stop after installation/i);
  ...
  assert.ok(prompt.length < 700, `prompt is ${prompt.length} characters`);
});

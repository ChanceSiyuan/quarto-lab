...
import {
  ...
  validateQecPortfolioCatalog,
} from "../../src/lib/qec-portfolio/catalog.mjs";
...
].map(([id, title, summary, candidateQuestion, gateType, anchorTitle, sourceUrl, persistentId, createdAt]) => ({
  id,
  title,
  summary,
  candidateQuestion,
...
test("catalog preserves the approved twenty-problem identities, copy, anchors, and timestamps", () => {
  assert.deepEqual(QEC_PORTFOLIO_PROBLEMS, EXPECTED_CATALOG);
  for (const record of QEC_PORTFOLIO_PROBLEMS) {
    assert.deepEqual(Object.keys(record), ["id", "title", "summary", "candidateQuestion", "gateType", "technicalAnchor", "createdAt", "updatedAt"]);
    assert.deepEqual(Object.keys(record.technicalAnchor), ["id", "title", "sourceUrl", "persistentId", "relevanceRationale"]);
  }
});
...
test("catalog IDs, visible copy, source URLs, and OpenAlex identifiers are safe", () => {
  ...
  assert.deepEqual(QEC_PORTFOLIO_IDS, EXPECTED_CATALOG.map((record) => record.id));
  assert.deepEqual(validateQecPortfolioCatalog(), { ok: true, errors: [] });
  for (const record of QEC_PORTFOLIO_PROBLEMS) {
...
test("catalog rejects invalid visible copy and source anchors", () => {
  const invalid = structuredClone(QEC_PORTFOLIO_PROBLEMS);
  invalid[0].summary = "English copy with 汉 character.";
  invalid[1].technicalAnchor.sourceUrl = "http://example.test/not-https";
  invalid[2].technicalAnchor.persistentId = "arxiv:2206.06557";
  const result = validateQecPortfolioCatalog(invalid);
  assert.equal(result.ok, false);
  assert.deepEqual(result.errors, [
    "Visible copy must be English-only for Prob-002.",
    "Technical source URL is invalid for Prob-003.",
    "OpenAlex persistent ID is invalid for Prob-004.",
  ]);
});
...
test("catalog lookup and common economic evidence keep approved external provenance immutable", () => {
  ...
  assert.equal(Object.isFrozen(QEC_PORTFOLIO_PROBLEMS), true);
  assert.equal(Object.isFrozen(QEC_PORTFOLIO_PROBLEMS[0]), true);
  assert.equal(Object.isFrozen(QEC_PORTFOLIO_PROBLEMS[0].technicalAnchor), true);
  assert.deepEqual(COMMON_ECONOMIC_EVIDENCE, [
    {
  ...
  assert.equal(Object.isFrozen(COMMON_ECONOMIC_EVIDENCE), true);
  assert.equal(Object.isFrozen(COMMON_ECONOMIC_EVIDENCE[0].sources[0]), true);
});

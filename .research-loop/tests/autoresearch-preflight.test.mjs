...
import { PREFLIGHT_CHECK_IDS, runInfrastructurePreflight } from "../../src/lib/autoresearch/preflight.mjs";
...
async function fixture(t, { outcomes = {}, tamper = null, assertRuntimeIsolation = false, evaluatorWrites = false } = {}) {
  ...
  const manifest = {
    ...
    runtime: { image: "example.invalid/research-loop/preflight@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
...
function runner(calls) {
  return async (options) => {
    calls.push(options);
    const { runProcess } = await import("../../src/lib/autoresearch/process.mjs");
...
test("runs the stable host-owned suite with isolated candidate and evaluator inputs", async (t) => {
  ...
  const report = await runInfrastructurePreflight({ ...value, processRunner: runner(calls), now: () => FIXED_NOW, signal: controller.signal });
  ...
  assert.ok(calls.every((call) => call.cwd === value.stageDir && call.timeoutMs === 60_000 && call.graceMs === 5_000));
  assert.ok(calls.every((call) => call.image === value.manifest.runtime.image));
  assert.ok(calls.every((call) => Object.isFrozen(call.resources)));
  for (const call of calls) assert.deepEqual(call.resources, value.manifest.resources);
...
test("uses the immutable validated runtime resources for every generated check", async (t) => {
  const value = await fixture(t);
  const calls = [];
  let mutated = false;
  const report = await runInfrastructurePreflight({
    ...value,
    processRunner: async (options) => {
      calls.push(options);
      if (!mutated) {
        mutated = true;
        value.manifest.runtime.image = "example.invalid/research-loop/preflight@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        value.manifest.resources.memoryMb = 1;
      }
  ...
  assert.equal(report.status, "passed");
  assert.ok(calls.every((call) => call.image.endsWith("a".repeat(64))));
  assert.ok(calls.every((call) => call.resources.memoryMb === 256 && Object.isFrozen(call.resources)));
});

test("default runner uses the OCI boundary instead of invoking generated host commands", async (t) => {
  const value = await fixture(t);
  const runtimeDir = join(value.privateDataRoot, "oci-runtime");
  const hostSentinel = join(runtimeDir, "host-sentinel.mjs");
  const docker = join(runtimeDir, "docker");
  const datasets = Object.fromEntries(Object.entries(value.manifest.datasets).map(([name, dataset]) => [name, dataset.digest]));
  await mkdir(runtimeDir);
  await writeFile(hostSentinel, `#!${process.execPath}\nthrow new Error("generated command escaped the OCI boundary");\n`);
  await chmod(hostSentinel, 0o755);
  for (const key of Object.keys(value.manifest.commands)) value.manifest.commands[key] = [hostSentinel];
  await writeFile(docker, [
    `#!${process.execPath}`,
    "const args = process.argv.slice(2);",
    "if (args[0] === 'version' || args[0] === 'rm') process.exit(0);",
    "if (args[0] !== 'run') process.exit(2);",
    "const check = args.at(-1);",
    `const datasets = ${JSON.stringify(datasets)};`,
    "const baseline = { id: 'baseline-v1', digest: 'e'.repeat(64), score: 7, components: [3, 4] };",
    "const result = check === 'hard-code-negative' ? { ok: false } : check === 'baseline-reproduction' ? { ok: true, baseline, datasets } : check === 'reproducibility' ? { ok: true, score: 7, components: [3, 4], baseline, datasets } : check === 'score-arithmetic' ? { ok: true, score: 7, components: [3, 4] } : { ok: true, score: 7, components: [3, 4] };",
    "process.stdout.write(JSON.stringify(result));",
  ].join("\n"));
  await chmod(docker, 0o755);

  const report = await runInfrastructurePreflight({ ...value, now: () => FIXED_NOW, environment: { PATH: runtimeDir } });
  assert.equal(report.status, "passed");
});
...
test("aborted preflight stops immediately instead of recording ordinary check failures", async (t) => {
  const value = await fixture(t);
  const controller = new AbortController();
  controller.abort();
  let calls = 0;

  await assert.rejects(() => runInfrastructurePreflight({
    ...value,
    signal: controller.signal,
    processRunner: async () => { calls += 1; throw controller.signal.reason; },
    now: () => FIXED_NOW,
  }), /abort/i);

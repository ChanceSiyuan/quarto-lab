...
import {
  ...
  verifyZoteroRecord,
} from "../../../src/lib/literature/zotero-materialize.js";
...
test("materialize writes verified PDF and LaTeX into the gitignored raw tree", async (t) => {
  ...
  assert.equal(await readFile(path.join(result, ".raw", "paper.pdf"), "latin1"), pdf.toString("latin1"));
  assert.ok((await readFile(path.join(result, ".raw", "source", "main.tex"))).length > 0);
...
test("verify detects a changed materialized file", async (t) => {
  ...
  await writeFile(path.join(paper, ".raw", "paper.pdf"), "tampered");

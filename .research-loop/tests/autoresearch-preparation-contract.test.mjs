...
} from "../../src/lib/autoresearch/preparation-contract.mjs";
...
function validManifest() {
  return {
    ...
    runtime: { image: `example.invalid/research-loop/preflight@sha256:${"a".repeat(64)}` },
...
test("infrastructure manifest returns a frozen normalized clone only when its complete host contract is valid", () => {
  ...
  assert.equal(result.runtime.image, `example.invalid/research-loop/preflight@sha256:${"a".repeat(64)}`);
  assert.equal(Object.isFrozen(result.runtime), true);
  assert.equal(result.files.length, 1);
});

test("infrastructure manifest requires an immutable OCI runtime image without extra fields", () => {
  const mutableTag = validManifest();
  mutableTag.runtime.image = "example.invalid/research-loop/preflight:latest";
  assertInvalid(() => validateInfrastructureManifest(mutableTag), ["runtime.image"]);

  const uppercaseDigest = validManifest();
  uppercaseDigest.runtime.image = `example.invalid/research-loop/preflight@sha256:${"A".repeat(64)}`;
  assertInvalid(() => validateInfrastructureManifest(uppercaseDigest), ["runtime.image"]);

  const missingRuntime = validManifest();
  delete missingRuntime.runtime;
  assertInvalid(() => validateInfrastructureManifest(missingRuntime), ["runtime"]);

  const extraRuntimeField = validManifest();
  extraRuntimeField.runtime.extra = true;
  assertInvalid(() => validateInfrastructureManifest(extraRuntimeField), ["runtime.extra"]);
});

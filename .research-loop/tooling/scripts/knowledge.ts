 ...
 *   knowledge.ts build
 *   knowledge.ts preview [--port <loopback-port>] [--watch-source knowledge/<page>.qmd]
 *
...
function previewOptionsOf(args: readonly string[]): {
  previewPort?: number;
  ...
  if (values.port !== undefined) {
    previewPort = Number(values.port);
    if (!Number.isInteger(previewPort) || previewPort < 1024 || previewPort > 65535) {
      throw new UsageError("preview --port must be an integer between 1024 and 65535");
    }
  }
  return {
    ...(previewPort === undefined ? {} : { previewPort }),
    ...(values["watch-source"] === undefined ? {} : { previewWatchPath: values["watch-source"] }),
  };
}
...
async function main(argv: readonly string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case "preview": {
      ...
      await previewKnowledgeSite({ repoRoot: REPO_ROOT, ...previewOptionsOf(rest) });
      return;
    }
    case undefined:

...
import { materializeZoteroRecord, verifyZoteroRecord } from "../../../src/lib/literature/zotero-materialize.js";
import {
  ZoteroClient,
...
async function writeBibliography(
  ...
  const target = path.join(literatureRoot, filename);
...
async function importFromZotero(command: "connect" | "import", args: string[]): Promise<void> {
  ...
  const bibliography = mergeZoteroBibliography(
    await readFile(path.join(literatureRoot, "ref.bib"), "utf8"),
    bibtex,
  );
  await writeBibliography(
    literatureRoot,
    "zotero.bib",
    sanitizeZoteroBibliographySnapshot(bibtex),
  );
  await writeBibliography(literatureRoot, "ref.bib", bibliography.contents);
  console.log(`Imported ${report.imported} Zotero metadata records into ${literatureRoot}`);
  console.log(
    `Updated ref.bib: ${bibliography.added} added, ${bibliography.updated} refreshed, ` +
    `${bibliography.retained} retained, ${bibliography.renamed.length} citekeys normalized, ` +
    `${bibliography.skipped.length} skipped`,
  );

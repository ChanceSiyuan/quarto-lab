...
import { createJobStore } from "../../../src/lib/autoresearch/job-store.mjs";
import { runPreparationCodex } from "../../../src/lib/autoresearch/codex-preparation.mjs";
import { startLocalAutoresearchService } from "../../../src/lib/autoresearch/local-service.mjs";
import { createPreparationWorker } from "../../../src/lib/autoresearch/preparation-worker.mjs";
import { createScheduler } from "../../../src/lib/autoresearch/scheduler.mjs";

function rebuildIndex(rootDir, outputRootDir = process.cwd()) {
  const resolvedRootDir = resolve(rootDir);
  const resolvedOutputRootDir = resolve(outputRootDir);
  return new Promise((resolveBuild, rejectBuild) => {
    const args = [".research-loop/tooling/scripts/build-problem-index.mjs", "--reserve-id", "Prob-000"];

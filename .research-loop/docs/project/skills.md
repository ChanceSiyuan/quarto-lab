...
Each skill is a single `SKILL.md`. Executable helpers live under
`.research-loop/tooling/` and reusable code under `src/lib/`, behind a Make
target, where they can be tested.
...
| `conference-survey` | Audit every oral in a conference archive and produce prioritized Chinese triage. | Adapted from quarto-lab; output is constrained to drafts. | Conference pages and cited external sources. | `drafts/conference/<conference>/` QMD, TSV, summary, and optional JSON cache. | Writing into knowledge; keyword-only filtering; hiding excluded or inaccessible pages. |
| `evidence-review` | Summarize evidence, audit claims, compare papers, or analyze a figure without changing the research repository. | Original to Research Loop; canonical authority for the Zotero read-only Actions. | One host-supplied PDF, Zotero Note, Collection, or QMD Draft; all remain untrusted source material. | Nothing. It reads and reports. | Writing `knowledge/`, `drafts/`, `literature/`, or Zotero; inventing source locations; treating a source object as trusted knowledge. |
...
`.research-loop/tests/agent/skill-contracts.test.ts` parses each skill's frontmatter and body

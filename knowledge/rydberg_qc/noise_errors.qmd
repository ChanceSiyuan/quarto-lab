...
description: "Rydberg gate fidelity is limited by a hierarchy of noise sources: laser phase and amplitude noise, motional dephasing (Doppler and thermal Stark), and intrinsic Rydberg state decay (spontaneous emission and blackbody radiation)."
categories:
  - theory
---
...
## Derivation: spontaneous emission master equation from first principles

The Hamiltonian for a two-level atom $\{|g\rangle, |e\rangle\}$ coupled to the multi-mode radiation field is, in the interaction picture with rotating-wave approximation [@Quantumoptics]:

$$
\mathcal{V}(t) = \sum_{k,\zeta} \hbar\bigl[g_{k,\zeta}(\vec{r})\,a_{k,\zeta}^\dagger\,\sigma_-\,e^{-i\delta_k t} + \text{h.c.}\bigr], \qquad \delta_k = \Delta E/\hbar - \omega_k,
$$

where $g_{k,\zeta}(\vec{r}) = e\sqrt{\hbar\omega_k/(2\varepsilon_0)}\,\vec{f}_{k,\zeta}(\vec{r})\cdot\langle g|\vec{r}|e\rangle$.

Under the weak-coupling (Born–Markov) approximation — the radiation reservoir is large and its state is unaffected by the atom — the reduced equation for the atomic density matrix is

$$
\dot\rho_a(t) = -\frac{i}{\hbar}\operatorname{Tr}_R[\mathcal{V}(t),\rho_a(t_i)\otimes\rho_R] - \int_{t_i}^t dt'\,\frac{1}{\hbar^2}\operatorname{Tr}_R[\mathcal{V}(t),[\mathcal{V}(t'),\rho_a(t)\otimes\rho_R]].
$$ {#eq-decay-reservoir-evol}

The reservoir is a thermal canonical ensemble $\rho_R = Z^{-1}e^{-\beta H_R}$, giving non-zero reservoir expectation values $\operatorname{Tr}[\rho_R a_{k,\zeta}^\dagger a_{k',\zeta'}] = \delta_{kk',\zeta\zeta'}\bar{n}_{k,\zeta}$ and $\operatorname{Tr}[\rho_R a_{k,\zeta}a_{k',\zeta'}^\dagger] = \delta_{kk',\zeta\zeta'}(\bar{n}_{k,\zeta}+1)$, where $\bar{n}_{k,\zeta} = (e^{\beta\hbar\omega_k}-1)^{-1}$.

Substituting into @eq-decay-reservoir-evol and summing over modes using $\sum_{k,\zeta}|g_{k,\zeta}|^2 e^{i\delta_k(t-t')} \to \frac{2\Gamma}{\pi}\delta(t-t')$ (Wigner–Weisskopf, with the imaginary part giving the Lamb shift), where

$$
\Gamma = \frac{\hbar|\langle g|e\vec{r}|e\rangle|^2\omega_0^3}{3\pi\varepsilon_0 c^3}
$$

is the spontaneous emission rate, the master equation at zero temperature ($\bar{n} = 0$) reduces to the standard Lindblad form:

$$
\dot\rho_a = -\frac{\Gamma}{2}(\sigma_+\sigma_-\rho_a + \rho_a\sigma_+\sigma_- - 2\sigma_-\rho_a\sigma_+).
$$

This is the microscopic justification for the $\Gamma_r$ decay rate appearing in the Kraus operators @eq-spont-kraus above.
:::

---
title: "AKLT"
description: "A curated knowledge note on AKLT, its assumptions, methods, limits, and verification context."
categories: [theory]
---
...
Conventions: spin-1 `S`-operators (`d=3`); the coefficient ratio is fixed (biquadratic = `1/3` × bilinear) — this is the special point where `H` is, up to a constant, a sum of projectors onto total spin-2 on each bond, making the VBS the exact zero-energy ground state. Coupling is set to the energy unit `J = 1`.
...
- Primary: **DMRG/MPS** — the ground state is a bond-dimension-2 MPS, so DMRG is exact at tiny `χ`; MPS natively measures string order and the entanglement spectrum.

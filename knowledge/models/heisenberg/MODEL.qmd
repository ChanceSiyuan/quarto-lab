---
title: "Heisenberg"
description: "A curated knowledge note on Heisenberg, its assumptions, methods, limits, and verification context."
categories: [theory]
---
...
Conventions: `S`-operator normalization (`S^a = σ^a/2` for S=1/2), `⟨ij⟩` = nearest-neighbor bonds counted once, `J > 0` antiferromagnetic (AFM), `J < 0` ferromagnetic (FM); the largest coupling is set to `J = 1`.
...
- Primary: **DMRG/MPS** for 1D chains, ladders, and 2D cylinders — near-exact in 1D area-law or critical regimes; SU(2)/U(1) quantum-number conservation cuts cost.
...
Only surface a real choice when the prompt is genuinely ambiguous about the lattice family. Build the Hamiltonian with explicit spin normalization, bond-counting, sign, and energy-scale conventions.

## Workflow

1. Set up sites and the Hamiltonian explicitly; pin the sector using total-spin or total-`S^z` conservation where applicable.
...
- **Limit checks** — confirm the sign convention and trivial limits: at `Δ = 1` XXZ reduces to isotropic Heisenberg; a ferromagnetic ground state is fully polarized; `J = 0` gives uncoupled spins.
- **Symmetry** — verify total `S^z` conservation, the expected ground-state sector (a singlet for a finite AFM), and the relevant lattice point-group symmetries.

---
title: "J1-J2"
description: "A curated knowledge note on J1-J2, its assumptions, methods, limits, and verification context."
categories: [theory]
---
...
Conventions: `S`-operator normalization, `⟨ij⟩` nearest-neighbor and `⟨⟨ij⟩⟩` next-nearest-neighbor (diagonal) bonds, each counted once; `J_1, J_2 > 0` AFM; `J_1 = 1` sets the scale, and `g ≡ J_2/J_1` is the control parameter.
...
- Primary: **DMRG on cylinders** + **PEPS/iPEPS** — frustration sign-blocks QMC, so tensor networks carry the load in 2D.
...
Build `H = J1 Σ S_i·S_j + J2 Σ S_i·S_j`, with the first sum over nearest-neighbor bonds and the second over next-nearest-neighbor bonds, each counted once.
...
- **Limit checks:** `J2 = 0` gives nearest-neighbor Heisenberg; `J1 = 0` gives decoupled sublattices, each equivalent to a nearest-neighbor Heisenberg model.

...
- [Magic based on Pauli spectrum](operator_magic.qmd)
- [OTOC and its simulation](OTOC.qmd)
...
:::
 ...
 This means that **Amplitude estimation** (i.e., estimating $\bra{0^n} U^\dagger \ket{0^n}\!\bra{0^n} U \ket{0^n}$) and **weak simulation** of T-depth one circuit is hardest than that of the degree-3 IQP circuits, which is Gap-P complete and believed to cause $\Delta_3 P = PH$ under plausible conjusture.
...
\end{align*}
$$
Then use the fact that $PQ$ has vectorization $v_Q+ v_P \mod 2$ up to a global non-linear phase $i^{[v_Q,v_P]} (-1)^{d(v_Q, v_P)}$ (The exact proof of this identity may refer to here [#eq-pauli-multiplication](../learning_theo/Shadow_tomography/3desgin_multi_local_shadow.qmd#eq-pauli-multiplication)).
...
2. [Random percolation based on graph states](../Noisy_complexity/percolation.qmd)

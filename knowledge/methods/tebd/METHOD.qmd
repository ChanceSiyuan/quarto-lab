...
description: "A curated knowledge note on Time-Evolving Block Decimation (TEBD), its assumptions, methods, limits, and verification context."
categories: [codes]
---
...
- Preferred over TDVP when `H` is strictly nearest-neighbor and swap-gate sequences are not needed.
...
- Heisenberg chain quench at `χ=512`: reachable `tJ≈6` before truncation error exceeds `10⁻³`.
- MBL phase (`W/J=5`): `S(t)~log(t)`, so `χ` grows logarithmically and `tJ~100` is reachable at `χ=512`.

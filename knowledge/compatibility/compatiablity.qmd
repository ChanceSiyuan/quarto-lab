...
description: This note develops the quantum resource theory @gour2018resource for the compatibility of quantum instruments.
categories:
  - theory
---
...
*Coarse-graining.* Since each marginal $E^{\eta_P}_{a|P}$ is a 2-outcome POVM ($a \in \{+1,-1\}$), any joint measurement naturally labels its outcomes by triples $(x,y,z) \in \{+1,-1\}^3$. A measurement with additional outcomes can be coarse-grained — summing POVM elements that share the same $(x,y,z)$ — without changing any marginal. Hence, we may assume $M(x,y,z) \geq 0$ with exactly 8 outcomes and $\sum_{x,y,z} M(x,y,z) = \mathbb{I}$.

*Operator-norm argument.* Evaluate the scalar
$$
\mathcal{S} \;:=\; \sum_{x,y,z \in \{+1,-1\}} \mathrm{tr}\!\left[M(x,y,z)\!\left(x\eta_x X + y\eta_y Y + z\eta_z Z\right)\right]
$$
in two ways.

**Lower bound (via marginals).** By linearity and the marginal condition $\sum_{y,z} M(x,y,z) = \tfrac{1}{2}(\mathbb{I}+x\eta_x X)$:
$$
\sum_{y,z}\mathrm{tr}[M(x,y,z)\,X] = \mathrm{tr}\!\left[\tfrac{1}{2}(\mathbb{I}+x\eta_x X)\,X\right] = x\eta_x,
$$
so the $X$-contribution to $\mathcal{S}$ is $\eta_x\sum_{x\in\{+1,-1\}} x\cdot(x\eta_x) = 2\eta_x^2$. Adding the $Y$ and $Z$ terms:
$$
\mathcal{S} = 2(\eta_x^2+\eta_y^2+\eta_z^2).
$$

**Upper bound (via operator norm).** For any PSD operator $M\geq 0$ and Hermitian $H$, the trace-norm/operator-norm duality gives $|\mathrm{tr}[MH]|\leq \mathrm{tr}[M]\,\|H\|$. Since the Pauli matrices anticommute and $x^2=y^2=z^2=1$,
$$
(x\eta_x X + y\eta_y Y + z\eta_z Z)^2 = (\eta_x^2+\eta_y^2+\eta_z^2)\,\mathbb{I},
$$
so $\|x\eta_x X + y\eta_y Y + z\eta_z Z\| = \sqrt{\eta_x^2+\eta_y^2+\eta_z^2}$ for every $(x,y,z)$. Therefore,
$$
\mathcal{S} \;\leq\; \sum_{x,y,z}|\mathrm{tr}[M(x,y,z)\,H_{xyz}]| \;\leq\; \sqrt{\eta_x^2+\eta_y^2+\eta_z^2}\underbrace{\sum_{x,y,z}\mathrm{tr}[M(x,y,z)]}_{=\,\mathrm{tr}[\mathbb{I}]\,=\,2} = 2\sqrt{\eta_x^2+\eta_y^2+\eta_z^2}.
$$

**Combining.** $2(\eta_x^2+\eta_y^2+\eta_z^2)\leq 2\sqrt{\eta_x^2+\eta_y^2+\eta_z^2}$ implies $\sqrt{\eta_x^2+\eta_y^2+\eta_z^2}\leq 1$, i.e., $\eta_x^2+\eta_y^2+\eta_z^2\leq 1$. $\blacksquare$
:::
...
$$
Since $\mathbb{E}[x_i] = \mathrm{tr}[\rho(E^{\eta_x}_{+|X} - E^{\eta_x}_{-|X})] = \eta_x\,\mathrm{tr}(\rho X)$ and each outcome $x_i \in \{+1,-1\}$, the unbiased estimator is the sample mean $\hat{t}_x = \frac{1}{N\eta_x}\sum_{i=1}^N x_i$. The per-sample second moment is $\mathbb{E}[(x_i/\eta_x)^2] = \eta_x^{-2}$ (since $x_i^2 = 1$ for $x_i\in\{+1,-1\}$), and similarly for $y,z$.
...
&E_{Q}(\mathbf{a}) =\frac{1 + 2 \mu (a_{00} + a_{10})}{1 + \mu \sum_{ij} a_{ij}} \frac{\mathbb{I}}{2} + \frac{1}{1 + \mu \sum_{ij} a_{ij}} \frac{Q}{2}
\end{align*}
$$
...
$$
\begin{align}
&\sum_{j,k} R_{j,k} = \mathbb{I},\quad \sum_{k} R_{1,k} = E_{P}(\mathbf{a}), \quad \sum_{j} R_{j,0} = E_{Q}(\mathbf{a}), R_{j,k} \geq 0.
\end{align}
$$
...
By strong duality (Slater's condition holds: take $G_{j,k}^* = \frac{M_j + N_k}{2} - \frac{\mathbb{I}}{4}$; this satisfies both marginal equalities $\sum_k G_{j,k}^* = M_j + \frac{N_0+N_1}{2} - \frac{\mathbb{I}}{2} = M_j$ and $\sum_j G_{j,k}^* = N_k$ since $\sum_j M_j = \sum_k N_k = \mathbb{I}$; moreover, since $M_j, N_k \in [0,\mathbb{I}]$ we have $\lambda_{\min}(G_{j,k}^*) \geq -\frac{1}{4}$, so choosing any $\mu^* > \max_{j,k}\frac{1}{4 a_{j,k}}$ makes $G_{j,k}^* + \mu^* a_{j,k}\mathbb{I} \succ 0$ for all $j,k$), the dual optimal equals the primal optimal $\mu \geq 0$.
:::
...
Given $n$-qubit anticommuting Pauli observables $P,Q$ ($\{P,Q\}=0$) with unsharp POVM elements $M_j = \frac{\mathbb{I}+(-1)^{j+1} 3\varepsilon P}{2}$ and $N_k = \frac{\mathbb{I}+(-1)^k 3\varepsilon Q}{2}$ for $\varepsilon\in[0,1/3]$ (so $M_0 = \frac{\mathbb{I}-3\varepsilon P}{2}$, matching the convention @eq-optimization), the generalized compatibility robustness is
$$
s^*(\varepsilon) = \max\!\bigl(1,\,(2-\sqrt{2})(1+3\varepsilon)\bigr).
$$
The threshold is at $3\varepsilon_c = 1/\sqrt{2}$ (the joint measurability boundary from @thm-unsharp-pauli-joint-measurability): for $3\varepsilon \leq 1/\sqrt{2}$ the measurements are already jointly measurable ($s^*=1$); for $3\varepsilon > 1/\sqrt{2}$, $s^*=(2-\sqrt{2})(1+3\varepsilon)$. The maximum at $\varepsilon=1/3$ gives $s^* = 4-2\sqrt{2}$. The optimal discrimination states are the eigenstate projectors $\rho_j^{(P)}=\frac{\mathbb{I}+(-1)^{j+1}P}{2^n}$ (so $\rho_0^{(P)}=\frac{\mathbb{I}-P}{2^n}$, the $-1$-eigenstate of $P$), independent of $\varepsilon$.
:::
::: {.callout-note collapse="true"}
## Proof (Click to expand)

**Step 1: Reduction to effective single-qubit problem.**
Since $\{P,Q\}=0$, there exists a Clifford unitary $U$ such that $UPU^\dagger = X_1 \otimes \mathbb{I}_{\text{rest}}$ and $UQU^\dagger = Z_1 \otimes \mathbb{I}_{\text{rest}}$. The dual SDP is unitarily invariant, so we work in this frame. Averaging the dual variables over the $(n-1)$-qubit Pauli group on qubits $2,\ldots,n$ (which preserves the objective **exactly by linearity**: since $M_j = m_j \otimes \mathbb{I}_{\mathrm{rest}}$ commutes with every $\mathbb{I}_1\otimes g$, one has $\mathrm{Tr}\!\bigl[(\mathbb{I}_1\otimes g)A_j(\mathbb{I}_1\otimes g^\dagger)M_j\bigr] = \mathrm{Tr}[A_j M_j]$ for all $g$, so the objective is invariant, not merely bounded, under the averaging map; feasibility of $A_j + B_k \leq W$ is likewise preserved exactly since the same conjugation is applied to all three variables) projects onto the factorized form
$$
...
Since $M_j = \frac{\mathbb{I}_2 + (-1)^{j+1}\eta X}{2}\otimes \mathbb{I}_{\text{rest}}$ with $\eta = 3\varepsilon$ (so $m_0 = \frac{\mathbb{I}-\eta X}{2}$, the $-1$-eigenstate direction), the traces factor: $\text{Tr}(A_j M_j) = \text{Tr}_1(a_j m_j)$ with $m_j = \frac{\mathbb{I}_2 + (-1)^{j+1}\eta X}{2}$, and similarly $n_k = \frac{\mathbb{I}_2 + (-1)^k\eta Z}{2}$ for $B_k, N_k$. The constraints $A_j + B_k \leq W$ also reduce to $a_j + b_k \leq w$. Thus the $n$-qubit dual SDP is equivalent to the single-qubit dual SDP with effective variables $(a_j, b_k, w)$.
...
$$
\text{Tr}_1(a_0 m_0) = \frac{1}{4}\text{Tr}\big((\alpha\mathbb{I} - \gamma X)(\mathbb{I} - \eta X)\big) = \frac{\alpha+\gamma\eta}{2},
$$
and similarly $\text{Tr}_1(a_1 m_1) = \frac{\alpha+\gamma\eta}{2}$, $\text{Tr}_1(b_k n_k) = \frac{\beta+\delta\eta}{2}$ each. The dual objective is $s^* = (\alpha+\gamma\eta) + (\beta+\delta\eta)$.
...
**Step 4: Optimization.**
Maximize $(\alpha+\gamma\eta) + (\beta+\delta\eta)$ subject to $\alpha+\beta+\sqrt{\gamma^2+\delta^2} \leq 1$ with $\alpha \geq \gamma \geq 0$, $\beta \geq \delta \geq 0$. By Cauchy--Schwarz, $\sqrt{\gamma^2+\delta^2}$ is minimized for fixed $\gamma+\delta$ when $\gamma=\delta$, so the symmetric point $\gamma=\delta$, $\alpha=\beta$ is optimal. Setting $\alpha = \frac{1-\gamma\sqrt{2}}{2}$ (constraint tight) gives:
$$
s^* = 1 + \gamma(2\eta - \sqrt{2}).
$$

**Case $\eta > 1/\sqrt{2}$:** Coefficient $2\eta-\sqrt{2}>0$, so maximize $\gamma$. The binding upper bound is $\alpha \geq \gamma$: $\frac{1-\gamma\sqrt{2}}{2} \geq \gamma$ gives $\gamma \leq \frac{1}{2+\sqrt{2}}$. At $\gamma = \frac{1}{2+\sqrt{2}}$:
$$
s^* = 1 + \frac{2\eta-\sqrt{2}}{2+\sqrt{2}} = \frac{2(1+\eta)}{2+\sqrt{2}} = (2-\sqrt{2})(1+\eta).
$$

**Case $\eta \leq 1/\sqrt{2}$:** Coefficient $2\eta-\sqrt{2}\leq 0$, so minimize $\gamma$: set $\gamma=\delta=0$. Then $s^* = 2\alpha \leq 1$, achieved at $\alpha=1/2$.

**Step 5: Complete formula.**
For $\eta > 1/\sqrt{2}$, the optimal $\alpha = \gamma = \frac{1}{2+\sqrt{2}}$ is independent of $\eta$. The optimal discrimination states are $\rho_j^{(P)} = \frac{\mathbb{I}+(-1)^{j+1} P}{2^n}$ (eigenstate projectors of $P$, sharpness 1) for any $\varepsilon > \varepsilon_c$. Concretely, $\rho_0^{(P)} = \frac{\mathbb{I}-P}{2^n}$ (dual variable $a_0 \propto \mathbb{I}-X$ in the single-qubit frame), consistent with the lemma convention $M_0 = \frac{\mathbb{I}-3\varepsilon P}{2}$. With $\eta = 3\varepsilon$:
$$
s^*(\varepsilon) = \max\!\bigl(1,\,(2-\sqrt{2})(1+3\varepsilon)\bigr).
$$
The threshold $3\varepsilon_c = 1/\sqrt{2}$ is exactly the joint measurability boundary from @thm-unsharp-pauli-joint-measurability: two equal-sharpness anticommuting Pauli measurements satisfy $\eta^2+\eta^2\leq 1$ iff $\eta\leq 1/\sqrt{2}$. For $3\varepsilon\leq 1/\sqrt{2}$ the measurements are already compatible, hence $s^*=1$. At $\varepsilon=1/3$: $s^* = (2-\sqrt{2})\cdot 2 = 4-2\sqrt{2}$. The dimension $n$ cancels in the single-qubit reduction. $\blacksquare$
:::

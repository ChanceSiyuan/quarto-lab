...
Run only one CSS-distance batch controller for a repository at a time. Commit
preflight acquires the exact Git `<ref>.lock` before installing a candidate
index, and every other repository writer must honor that Git lock. A stale or
already-owned lock makes the trial fail closed; direct same-user mutation that
bypasses Git's lock protocol is outside the supported campaign contract.

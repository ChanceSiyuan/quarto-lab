...
_IGNORABLE_PROPOSAL_TOOL_ARTIFACTS = frozenset({".git", "__pycache__"})
_IGNORABLE_PYCACHE_ARTIFACTS = frozenset({"candidate.cpython-311.pyc"})
_IGNORABLE_PYCACHE_MAX_COUNT = 1
_CPYTHON_311_PYC_MAGIC = b"\xa7\r\r\n"
_PYC_HEADER_BYTES = 16
_IGNORABLE_PYCACHE_BYTES = 1024 * 1024
...
def run_trial(
    ...
    *,
    ...
    if invalid_existing_workspace:
        ...
        try:
            _prune_ignorable_proposal_tool_artifacts(proposal_workspace)
        ...
        try:
            ...
            if any(proposal_workspace.iterdir()):
                ...
                try:
                    ...
                    try:
                        _prune_ignorable_proposal_tool_artifacts(
                            proposal_workspace
                        )
                        method = _load_candidate_method(
                            proposal_workspace,
...
def _prune_ignorable_proposal_tool_artifacts(proposal_workspace: Path) -> None:
    try:
        root_metadata = os.lstat(proposal_workspace)
    except OSError as error:
        raise CssDistanceContainerError(
            "proposal workspace must be a dedicated public directory"
        ) from error
    if (
        proposal_workspace.name != "proposal-workspace"
    ...
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    try:
        root_fd = os.open(proposal_workspace, flags)
    except OSError as error:
        raise CssDistanceContainerError(
            "proposal workspace must be a dedicated public directory"
        ) from error
    try:
        ...
        if not _same_workspace_identity(opened_root_metadata, root_metadata):
            raise CssDistanceInfrastructureError(
                "proposal workspace root identity changed"
            )
        for name in _IGNORABLE_PROPOSAL_TOOL_ARTIFACTS:
            quarantined = _quarantine_ignorable_proposal_tool_artifact(
                root_fd,
                name,
            )
            ...
            (
                ...
                metadata,
                child_manifest,
            ) = _open_validated_ignorable_proposal_tool_artifact(
                root_fd,
                quarantined,
                artifact_name=name,
            )
            _remove_open_workspace_directory_at(
                root_fd,
                quarantined,
                directory_fd,
    ...
    except CssDistanceContainerError:
        raise
    except OSError as error:
        raise CssDistanceContainerError("unsafe proposal workspace") from error
    finally:
        os.close(root_fd)


def _quarantine_ignorable_proposal_tool_artifact(
    parent_fd: int,
    name: str,
) -> str | None:
    for _ in range(8):
        quarantined = f".prune-{name.lstrip('.')}-{uuid4().hex}"
        try:
            os.rename(
                name,
                quarantined,
...
def _open_validated_ignorable_proposal_tool_artifact(
    parent_fd: int,
    quarantined: str,
    *,
    artifact_name: str,
) -> tuple[int, os.stat_result, dict[str, os.stat_result]]:
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    directory_fd = os.open(quarantined, flags, dir_fd=parent_fd)
    try:
        directory_metadata = os.fstat(directory_fd)
        if not stat.S_ISDIR(directory_metadata.st_mode):
            raise CssDistanceContainerError("unsafe proposal workspace")
        children = os.listdir(directory_fd)
        if artifact_name == ".git":
            if children:
                raise CssDistanceContainerError("unsafe proposal workspace")
            return directory_fd, directory_metadata, {}
        if artifact_name != "__pycache__":
            raise CssDistanceContainerError("unsafe proposal workspace")
        if len(children) > _IGNORABLE_PYCACHE_MAX_COUNT:
            raise CssDistanceContainerError("unsafe proposal workspace")
        total_size = 0
        child_manifest: dict[str, os.stat_result] = {}
        for child in children:
            if child not in _IGNORABLE_PYCACHE_ARTIFACTS:
                raise CssDistanceContainerError("unsafe proposal workspace")
            child_metadata = _validate_ignorable_pyc_header(directory_fd, child)
            if (
                not stat.S_ISREG(child_metadata.st_mode)
                or child_metadata.st_nlink != 1
                or child_metadata.st_size < _PYC_HEADER_BYTES
                or child_metadata.st_size > _IGNORABLE_PYCACHE_BYTES
            ):
                raise CssDistanceContainerError("unsafe proposal workspace")
            total_size += child_metadata.st_size
            if total_size > _IGNORABLE_PYCACHE_BYTES:
                raise CssDistanceContainerError("unsafe proposal workspace")
            child_manifest[child] = child_metadata
    except Exception:
        os.close(directory_fd)
        raise
    return directory_fd, directory_metadata, child_manifest


def _validate_ignorable_pyc_header(directory_fd: int, name: str) -> os.stat_result:
    metadata = os.stat(name, dir_fd=directory_fd, follow_symlinks=False)
    if (
        not stat.S_ISREG(metadata.st_mode)
        or metadata.st_nlink != 1
        or metadata.st_size < _PYC_HEADER_BYTES
        or metadata.st_size > _IGNORABLE_PYCACHE_BYTES
    ):
        raise CssDistanceContainerError("unsafe proposal workspace")
    flags = (
        os.O_RDONLY
    ...
    try:
        ...
        if (
            not _same_workspace_identity(opened_metadata, metadata)
            or not stat.S_ISREG(opened_metadata.st_mode)
            or opened_metadata.st_nlink != 1
            or opened_metadata.st_size < _PYC_HEADER_BYTES
            or opened_metadata.st_size > _IGNORABLE_PYCACHE_BYTES
        ):
            raise CssDistanceContainerError("unsafe proposal workspace")
        header = os.read(descriptor, _PYC_HEADER_BYTES)
    finally:
        os.close(descriptor)
    if (
        len(header) != _PYC_HEADER_BYTES
        or header[:4] != _CPYTHON_311_PYC_MAGIC
    ):
        raise CssDistanceContainerError("unsafe proposal workspace")
    return metadata
...
def _load_candidate_method(proposal_workspace: Path, *, auth_path: Path) -> str:
    ...
    _validate_proposal_text_privacy(
        candidate_text,
        method_text,
        auth_path=auth_path,
    )
    ...
    method_line = lines[0].removesuffix(".").rstrip() if len(lines) == 1 else ""
    if (
        len(lines) != 1
        or not method_line
        or len(method_line) > _MAX_METHOD_CHARACTERS
        or _PUBLIC_METHOD.fullmatch(method_line) is None
    ):
        raise ValueError("METHOD.txt must contain one public-safe line")
    _write_regular_text_nofollow(method_path, method_line + "\n")
...
def _validate_proposal_text_privacy(
    candidate_text: str,
    method_text: str,
    *,
    auth_path: Path,
...
def _same_workspace_identity(
    ...
    return (
        ...
        and metadata.st_mode == expected_metadata.st_mode
    )
...
def _require_workspace_identity(
    ...
    if expected_metadata is None:
        return
    if not _same_workspace_identity(metadata, expected_metadata):
        raise CssDistanceInfrastructureError(
            "proposal workspace cleanup identity changed"
        )


def _remove_workspace_entry_at(
    parent_fd: int,
    name: str,
    *,
    ...
    try:
        ...
        return
    _require_workspace_identity(metadata, expected_metadata)
    ...
    try:
        ...
        _require_workspace_identity(opened_metadata, metadata)
        _require_workspace_identity(opened_metadata, expected_metadata)
        for child in os.listdir(directory_fd):
            _remove_workspace_entry_at(directory_fd, child)
        current_metadata = os.stat(name, dir_fd=parent_fd, follow_symlinks=False)
        _require_workspace_identity(current_metadata, opened_metadata)
        os.rmdir(name, dir_fd=parent_fd)
    finally:
        os.close(directory_fd)


def _remove_open_workspace_directory_at(
    parent_fd: int,
    name: str,
    directory_fd: int,
    *,
    ...
    try:
        opened_metadata = os.fstat(directory_fd)
        _require_workspace_identity(opened_metadata, expected_metadata)
        live_children = set(os.listdir(directory_fd))
        if live_children != set(child_manifest):
            raise CssDistanceInfrastructureError(
                "proposal workspace cleanup identity changed"
            )
        for child, child_metadata in child_manifest.items():
            _remove_workspace_entry_at(
                directory_fd,
                child,
                expected_metadata=child_metadata,
            )
        current_metadata = os.stat(name, dir_fd=parent_fd, follow_symlinks=False)
        _require_workspace_identity(current_metadata, opened_metadata)
        os.rmdir(name, dir_fd=parent_fd)

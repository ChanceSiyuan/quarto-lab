...
def test_parse_trial_report_validates_bounded_public_method_text(
    ...
    for method in ("A" * 161, "invalid: method"):
        report = write_report_fixture(
            tmp_path,
            proposal=101,
            method=method,
...
def test_render_results_page_escapes_valid_method_text() -> None:
    ...
    trials[0] = replace(trials[0], method="quotient search's repair")
    html = render_results_page(make_four_baselines(), trials)
    assert "quotient search's repair" not in html
    assert "quotient search&#x27;s repair" in html

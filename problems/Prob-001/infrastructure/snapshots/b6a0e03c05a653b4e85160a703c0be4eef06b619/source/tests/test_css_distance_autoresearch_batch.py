...
_CPYTHON_311_PYC_MAGIC = b"\xa7\r\r\n"


def _synthetic_pyc_payload(
    *,
    magic: bytes = _CPYTHON_311_PYC_MAGIC,
    size: int = 16,
) -> bytes:
    assert len(magic) == 4
    assert size >= 4
    return magic + b"\0" * (size - 4)
...
def test_invalid_proposal_artifacts_are_securely_cleared_before_zero_run_commit(
    ...
    def invalid_proposal(**kwargs: object) -> str:
        ...
        method = "A" * 161 if invalid_output == "long-method" else "Public method 101"
...
@pytest.mark.parametrize("pyc_size", [16, 1024 * 1024])
def test_ignorable_proposal_tool_artifacts_are_pruned_before_contract_check(
    tmp_path: Path,
    pyc_size: int,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    ...
    def proposal_with_tool_artifacts(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text("print('{}')\n", encoding="utf-8")
        (workspace / "METHOD.txt").write_text(
            "Public randomized search\n",
            encoding="utf-8",
        )
        (workspace / ".git").mkdir()
        (workspace / "__pycache__").mkdir()
        (workspace / "__pycache__" / "candidate.cpython-311.pyc").write_bytes(
            _synthetic_pyc_payload(size=pyc_size)
        )
        return "proposal complete"

    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": proposal_with_tool_artifacts}
    )
...
def test_unsafe_nested_proposal_tool_artifacts_are_contract_failures(
    tmp_path: Path,
    unsafe_artifact: str,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    ...
    def proposal_with_unsafe_tool_artifact(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text("print('{}')\n", encoding="utf-8")
        (workspace / "METHOD.txt").write_text(
        ...
        if unsafe_artifact == "pycache-non-pyc":
            (workspace / "__pycache__").mkdir()
            (workspace / "__pycache__" / "notes.txt").write_text(
                "unexpected",
                encoding="utf-8",
            )
            return "proposal complete"
        pycache_path = workspace / "__pycache__"
        if unsafe_artifact.startswith("pycache-"):
            pycache_path.mkdir()
            artifact = pycache_path / "candidate.cpython-311.pyc"
            if unsafe_artifact == "pycache-symlink":
                os.symlink(outside, artifact)
            elif unsafe_artifact == "pycache-hardlink":
                os.link(outside, artifact)
            elif unsafe_artifact == "pycache-special":
                os.mkfifo(artifact)
            elif unsafe_artifact == "pycache-bad-payload":
                artifact.write_bytes(b"\0\0\0\0")
            elif unsafe_artifact == "pycache-wrong-magic":
                artifact.write_bytes(_synthetic_pyc_payload(magic=b"\x42\x42\r\n"))
            elif unsafe_artifact == "pycache-wrong-crlf":
                artifact.write_bytes(_synthetic_pyc_payload(magic=b"\xa7\r\0\0"))
            elif unsafe_artifact == "pycache-overlarge":
                artifact.write_bytes(_synthetic_pyc_payload(size=1024 * 1024 + 1))
            elif unsafe_artifact == "pycache-too-many":
                artifact.write_bytes(_synthetic_pyc_payload())
                (pycache_path / "other.cpython-311.pyc").write_bytes(
                    _synthetic_pyc_payload()
                )
            return "proposal complete"
        git_dir = workspace / ".git"
        git_dir.mkdir()
        if unsafe_artifact == "git-file":
    ...
    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": proposal_with_unsafe_tool_artifact}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "credential-marker",
        "auth-leaf-in-method",
        "auth-leaf-trailing-period-in-method",
    ],
)
def test_credential_bearing_proposal_is_zero_run_and_securely_reset(
    ...
    def leaking_proposal(**kwargs: object) -> str:
        ...
        elif leak == "auth-leaf-trailing-period-in-method":
            config.auth_path.write_text(
                json.dumps({"tokens": {"access_token": trailing_secret}}),
                encoding="utf-8",
            )
            method = trailing_secret
        else:
            method = secret
        (workspace / "candidate.py").write_text(candidate, encoding="utf-8")
        (workspace / "METHOD.txt").write_text(method + "\n", encoding="utf-8")
...
def test_public_method_sentence_punctuation_is_contract_safe(
    tmp_path: Path,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    stages: list[str] = []
    ...
    def sentence_method_candidate(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text(
            "seed = 7\nprint('{}')\n",
        ...
        (workspace / "METHOD.txt").write_text(
            "Random logical-coset sampling with greedy stabilizer-row descent; "
            "reports only verified CSS upper-bound witnesses.\n",
            encoding="utf-8",
        )
        return "proposal complete"

    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": sentence_method_candidate}
    )
    ...
    assert method_file.read_text(encoding="utf-8").endswith("witnesses\n")
    assert "smoke" in stages and "evaluate" in stages
...
@pytest.mark.parametrize(
    ...
    [
        "results&period;dat",
        "Random & encoded method",
        "Internal.period method",
        "Double terminal period..",
    ],
)
def test_public_method_rejects_entity_and_period_bypass_forms(
    tmp_path: Path,
    method_text: str,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    ...
    def invalid_method_candidate(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text("print('{}')\n", encoding="utf-8")
        (workspace / "METHOD.txt").write_text(method_text + "\n", encoding="utf-8")
        return "proposal complete"

    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": invalid_method_candidate}
    )
...
def test_public_method_accepts_exact_length_boundary(
    tmp_path: Path,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    stages: list[str] = []
    ...
    def boundary_method_candidate(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text("print('{}')\n", encoding="utf-8")
        (workspace / "METHOD.txt").write_text(method_text + "\n", encoding="utf-8")
    ...
    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": boundary_method_candidate}
    )

    rows = run_batch(config, dependencies=dependencies)

    assert rows[0].method == method_text
    assert rows[0].runs == 24
    assert "smoke" in stages and "evaluate" in stages


def test_public_method_accepts_long_observed_sentence(
    tmp_path: Path,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    stages: list[str] = []
    dependencies = _dependencies(config, stages)
    method_text = (
        "Randomized quotient-logical sampling with stabilizer coset mixing and "
        "greedy syndrome-preserving weight descent; verified upper-bound witnesses only."
    )
    assert len(method_text) == 149

    def long_sentence_method_candidate(**kwargs: object) -> str:
        stages.append("propose")
        workspace = Path(kwargs["proposal_workspace"])
        (workspace / "candidate.py").write_text("print('{}')\n", encoding="utf-8")
        (workspace / "METHOD.txt").write_text(method_text + "\n", encoding="utf-8")
    ...
    dependencies = BatchDependencies(
        **{**dependencies.__dict__, "run_proposal": long_sentence_method_candidate}
    )

    rows = run_batch(config, dependencies=dependencies)

    assert rows[0].method == method_text.removesuffix(".")
    assert rows[0].runs == 24
    assert "smoke" in stages and "evaluate" in stages
...
def test_workspace_removal_rejects_identity_substitution(
    tmp_path: Path,
) -> None:
    original = tmp_path / "entry"
    original.mkdir()
    expected = os.stat(original, follow_symlinks=False)
    displaced = tmp_path / "displaced"
    original.rename(displaced)
    substitute = tmp_path / "entry"
    substitute.mkdir()
    victim = substitute / "victim.txt"
    victim.write_text("preserve", encoding="utf-8")
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    root_fd = os.open(tmp_path, flags)
    try:
        with pytest.raises(CssDistanceInfrastructureError):
            batch_module._remove_workspace_entry_at(
                root_fd,
                "entry",
...
def test_open_workspace_removal_rejects_child_identity_substitution(
    tmp_path: Path,
) -> None:
    entry = tmp_path / "entry"
    entry.mkdir()
    child_name = "candidate.cpython-311.pyc"
    child = entry / child_name
    child.write_bytes(_synthetic_pyc_payload())
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    root_fd = os.open(tmp_path, flags)
    directory_fd = os.open("entry", flags, dir_fd=root_fd)
    try:
        entry_metadata = os.fstat(directory_fd)
        child_metadata = os.stat(child_name, dir_fd=directory_fd, follow_symlinks=False)
        child.unlink()
        child.mkdir()
        victim = child / "victim.txt"
        victim.write_text("preserve", encoding="utf-8")

        with pytest.raises(CssDistanceInfrastructureError):
            batch_module._remove_open_workspace_directory_at(
                root_fd,
                "entry",
                directory_fd,
                expected_metadata=entry_metadata,
                child_manifest={child_name: child_metadata},
            )
        directory_fd = -1
    finally:
        if directory_fd >= 0:
...
def test_pyc_header_validation_rejects_stat_open_fifo_replacement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    child_name = "candidate.cpython-311.pyc"
    pyc = tmp_path / child_name
    pyc.write_bytes(_synthetic_pyc_payload())
    flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
    directory_fd = os.open(tmp_path, flags)
    original_open = os.open
    observed: dict[str, bool] = {}

    def racing_open(path: object, open_flags: int, *args: object, **kwargs: object) -> int:
        if path == child_name:
            observed["saw_nonblock"] = bool(
                open_flags & getattr(os, "O_NONBLOCK", 0)
            )
            if not observed["saw_nonblock"]:
                raise RuntimeError("missing O_NONBLOCK")
            pyc.unlink()
            os.mkfifo(pyc)
        return original_open(path, open_flags, *args, **kwargs)

    monkeypatch.setattr(batch_module.os, "open", racing_open)
    try:
        with pytest.raises(CssDistanceContainerError):
            batch_module._validate_ignorable_pyc_header(directory_fd, child_name)
    finally:
        os.close(directory_fd)

    assert observed == {"saw_nonblock": True}


def test_proposal_tool_pruning_rejects_workspace_root_substitution(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(CssDistanceInfrastructureError):
        batch_module._prune_ignorable_proposal_tool_artifacts(workspace)


def test_run_batch_aborts_without_reset_after_workspace_root_substitution(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _config(tmp_path, start=101, end=101)
    ...
    dependencies = _dependencies(config, stages)
    workspace = config.reports_root / proposal_directory_name(101) / "proposal-workspace"
    displaced = tmp_path / "displaced-proposal-workspace"
    original_open = os.open
    injected = False

    def racing_open(path: object, open_flags: int, *args: object, **kwargs: object) -> int:
        nonlocal injected
        if not injected and Path(path) == workspace:
            injected = True
            workspace.rename(displaced)
            workspace.mkdir()
    ...
    assert injected
    assert (workspace / "sentinel.txt").read_text(encoding="utf-8") == "preserve"

...
def _load_json_nofollow(path: Path) -> dict:
    if ".." in path.parts:
        raise CssDistanceEvalError("unsafe JSON input")
    ...
    try:
        ...
        os.close(parent_fd)
        raise CssDistanceEvalError("unsafe JSON input") from error
    os.close(parent_fd)
    try:
    ...
    *,
    ...
    try:
        while selector.get_map() or process.poll() is None:
            ...
            elif polled is None and now >= hard_deadline and drain_deadline is None:
                timed_out = True
                _terminate_process_group(process)
                group_terminated = True
...
def run_candidate_case(
    *,
    ...
    try:
        hx_payload = _load_json_nofollow(hx_path)
        hz_payload = _load_json_nofollow(hz_path)
...
def sanitize_log_summary(summary: dict) -> dict:
    ...
    numeric_fields = {"normalized_quality", "runtime_seconds"}

...
test("public indexing rejects an unredacted private valuation before writing either index", async () => {
  ...
  const local = spawnSync("node", ["scripts/build-problem-index.mjs", "--root", root, "--out", outputPath, "--research-out", researchOutputPath], {
    cwd: new URL("..", import.meta.url),
    encoding: "utf8",
  });
  assert.equal(local.status, 0, local.stderr);

  const publicResult = spawnSync("node", ["scripts/build-problem-index.mjs", "--public", "--root", root, "--out", join(root, "public-index.json"), "--research-out", join(root, "public-research-index.json")], {
    cwd: new URL("..", import.meta.url),
    encoding: "utf8",
  });

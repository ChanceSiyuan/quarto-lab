...
test("researches quantum valuation evidence, freezes a snapshot, and runs v2 assessment", async ({ context, page }) => {
  ...
  await page.getByRole("button", { name: "Run assessment" }).click();
  await expect(page.getByText("Scientific attention", { exact: true })).toBeVisible({ timeout: 120_000 });
  ...
  await expect(report.getByText(/20260729T\d{6}Z-[a-f0-9]{12}/)).toBeVisible();

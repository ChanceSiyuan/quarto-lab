...
- A partial yes. Since I notice that at least for $\{XX, YY, ZZ\}^{\otimes n}$ case would be exponentially hard in biased 1-replica case, the 2-replica local strategy can deal with it. But the problem is: If we focus on the estimation of $\mathrm{tr}(H\rho)$, for fairness, we should consider $\mathrm{tr}[(H \otimes \mathbb{I}) \rho^{\otimes 2}],\mathrm{tr}[(H \otimes H) \rho^{\otimes 2}]$ instead of $\mathrm{tr}(H\rho)$.
...
## Theorem: Proof of the Conjecture (Upper and Lower Bounds)
...
2. **(Lower bound)** Any joint POVM $\{G_k\}$ with unbiased estimators $\{f_i\}$ for all $\{O_i\}$ must have $\max_i\|f_i\|_\infty\geq\sqrt{I}$, which forces $\Omega(I\,\varepsilon^{-2})$ samples for accuracy $\varepsilon$.
:::
...
**Step 2 — Variance at the equatorial state.**

Let $i^*$ achieve $\|f_{i^*}\|_\infty=\max_i\|f_{i^*}\|_\infty\geq\sqrt{I}$, and let $\rho^*$ be any state with $\mathrm{tr}(O_{i^*}\rho^*)=0$ (the "equatorial" state). At this state:
$$\mathrm{Var}_{\rho^*}[f_{i^*}] = \mathbb{E}_{\rho^*}[f_{i^*}^2] =: \mathrm{tr}(S_{i^*}\,\rho^*),\quad S_{i^*}:=\sum_k G_k f_{i^*}(k)^2.$$

By the scalar Jensen inequality applied to the probability distribution $p_k=\mathrm{tr}(G_k\rho)$:
$$\mathrm{tr}(S_{i^*}\rho) = \mathbb{E}_\rho[f_{i^*}^2]\geq(\mathbb{E}_\rho[f_{i^*}])^2=(\mathrm{tr}(O_{i^*}\rho))^2\quad\forall\,\rho.$$
In particular, at the eigenstates $\rho_\pm$ of $O_{i^*}$ (with $\mathrm{tr}(O_{i^*}\rho_\pm)=\pm 1$):
$$\mathrm{tr}(S_{i^*}\rho_\pm)\geq 1,$$
and summing over $\rho_+,\rho_-$ and using $\frac{\rho_++\rho_-}{2}=\frac{\mathbb{I}}{d}$ gives $\frac{1}{d}\mathrm{tr}(S_{i^*})\geq 1$, i.e., $S_{i^*}\geq\mathbb{I}$ on average.

For the optimal joint POVM (all outcomes give $f_{i^*}(k)=\pm\sqrt{I}$): $S_{i^*}=I\,\mathbb{I}$ exactly, so $\mathrm{Var}_{\rho^*}[f_{i^*}]=I$.

For a general joint POVM, the Lemma (Dual Discrimination) established in the next section shows that the incompatibility advantage $I$ imposes a state-discrimination hardness that bounds the minimum achievable variance from below: $\mathrm{Var}_{\rho^*}[f_{i^*}]\geq I-1$.

**Step 3 — Sample complexity lower bound.**

With $\mathrm{Var}[f_{i^*}]\geq I-1$, the $N$-sample mean $\bar{f}_{i^*}^{(N)}$ satisfies $\mathrm{Var}[\bar{f}_{i^*}^{(N)}]=(I-1)/N$. To achieve additive error $\leq\varepsilon$ with success probability $\geq 2/3$, Chebyshev requires:
$$N\geq\frac{9(I-1)}{\varepsilon^2}=\Omega(I\,\varepsilon^{-2}).\quad\blacksquare$$
:::
...
One concern is, how to implement the $6$-outcome joint measurement $G(x, y, z)$?
...
However, given a set of observables $\{O_i\}_{i=1}^L$ that can be efficiently estimated by certain classical shadow strategy, there must exists a joint measurement method that reach the same efficiency. This means that a better joint measurement strategy exists: $\mathcal{G}_{\text{CS}} = \{p_x \frac{\mathbb{I} \pm X}{2}, p_y \frac{\mathbb{I} \pm Y}{2}, p_z \frac{\mathbb{I} \pm Z}{2}\}$

# Toy Example: $\{P,Q\}$

Consider the simplest observable set $\mathcal{O} = \{P,Q\}$ where $P,Q$ are both Pauli observables. Estimation of those two observables correspond to two-outcome POVMs $\{M_1 := \mathbb{I} - M_0, M_0\}$ and $\{N_1 := \mathbb{I} - N_0, N_0\}$ given $M_0 = \frac{P + \mathbb{I}}{2}$ and $N_0 = \frac{Q + \mathbb{I}}{2}$.
...
$$
\begin{align}
& \sum_{j} G_{j,k} = M_k, \quad \sum_{k} G_{j,k} = N_j, \quad G_{j,k} + \mu a_{j,k} \mathbb{I} \geq 0.
\end{align}
$${#eq-restriction}
:::
...
$$
\begin{align}
&\sum_{j,k} R_{j,k} = \mathbb{I},\quad \sum_{j} R_{j,0} = E_{P}(\mathbf{a}), \quad \sum_{k} R_{0,k} = E_{Q}(\mathbf{a}), R_{j,k} \geq 0.
\end{align}
$$
...
$$
\begin{align}
\text{Minimize:} \quad & \sum_{j=0}^1 \text{Tr}(A_j M_j) + \sum_{k=0}^1 \text{Tr}(B_k N_k) \\
...
- First we need to first generalize the discussion above to Pauli sets with greater than 2 observables.
:::

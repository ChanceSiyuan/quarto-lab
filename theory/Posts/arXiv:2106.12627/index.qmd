...
\end{aligned}
$$
where $\mathbf \Theta^{(L)}(\theta)(x, x') = \sum_{p=1}^P \partial_{\theta_p} \mathbf F^{(L)}(x; \theta) \left[\partial_{\theta_p} \mathbf F^{(L)}(x'; \theta)\right]^T$ is the Neural Tangent Kernel (NTK), which depends only on the structure of the neural network and not on the input data.
:::

...
- $\alpha = 1/2$: **NTK parameterization** (lazy regime). With $a_j = O(1)$, the CLT gives $\sum_j a_j\sigma(w_j\cdot x) = O(\sqrt{N})$, so $f = O(N^{1/2-\alpha})|_{\alpha=1/2} = O(1)$. More importantly, the gradient to feature weights carries the same $N^{-1/2}$ prefactor: $\partial\mathcal{L}/\partial w_j \propto N^{-1/2}$, so $w_j$ barely moves and the kernel freezes.
- $\alpha = 0$: **Mean-field parameterization** (rich regime). The prefactor is $N^0 = 1$, so $f = O(\sqrt{N})$ — the **largest** output scale. The gradient suppression is absent ($\partial\mathcal{L}/\partial w_j = O(1)$), forcing large weight excursions and enabling feature learning.
...
The signal strength $S(t)$ and noise strength $N(t)$ are defined as $S(t) := \|\Pi_S W_t\|_F^2$ and $N(t) := \|\Pi_N W_t\|_F^2$ (the squared Frobenius norms of the weight matrix projected onto the signal and noise subspaces; see @def-snr for the precise decomposition). They obey effective dynamics of the form:
$$
...
**Stage I---Lazy Learning.** The output layer $V$ converges to the ridge regression solution $V^* = Y X^\top(XX^\top + \lambda I)^{-1}$ (here $\lambda > 0$ is the ridge regularization coefficient, distinct from the learning rate $\eta$) using the random features provided by the frozen hidden layer $W$. The network memorizes the training data. The backward gradient $G_F$ is noise-dominated and provides no structural signal to the hidden layer.

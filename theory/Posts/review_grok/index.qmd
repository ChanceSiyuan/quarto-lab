...
$$ {#eq-signal-noise}
where $\eta_S \ll \eta_N$ reflects the slower signal learning rate. Here $g_S$ and $g_N$ are the projections of the (normalized) loss gradient onto the signal and noise subspaces, respectively [@liu2022grokking]. In the linearized regime they take the explicit forms
$$
g_N(S,N) = N_\infty - N, \qquad g_S(S,N) = S,
$$ {#eq-g-explicit}
where $N_\infty$ is the memorization capacity (the noise level at which training loss reaches zero). With these forms, the two-phase dynamics become transparent:

1. **Memorization phase** ($t \lesssim \tau_{\mathrm{mem}}$): $\dot{N} \approx \eta_N(N_\infty - N) - \lambda N > 0$, so noise grows on timescale $(\eta_N + \lambda)^{-1}$ and saturates near $N_\infty \eta_N/(\eta_N + \lambda) \approx N_\infty$. Meanwhile $\dot{S} \approx (\eta_S - \lambda)S < 0$ (since $\eta_S < \lambda$ in the Goldilocks zone), so signal decays.
2. **Weight-decay phase** ($t \gtrsim \tau_{\mathrm{mem}}$): Once $N \approx N_\infty$, the gradient $g_N \to 0$ and the $-\lambda N$ term dominates: $N$ decays exponentially with rate $\lambda$. As $N$ falls, the training loss rises and the effective signal learning rate increases. When $N$ drops below the threshold $N^* = (\eta_S/\lambda)N_\infty$, the signal equation crosses zero — $\dot{S} > 0$ — and signal growth begins, triggering generalization.

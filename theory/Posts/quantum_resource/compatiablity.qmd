...
## Questions:
What if we discuss the JM and find the unsharp version for 2-replica local case $\{I,X,Y,Z\}^{\otimes 2}$? This will introduce a biased 2-replica local strategy. Would this has advatage compared to the 1-replica case?
:::
*Discussion:*
...
::: {.callout-important icon="false"}
## Conjusture:
If the compatibility robustness of a set of observables $\{O_i\}_{i=1}^L$ is $I$, then any non-adaptive strategy for estimating $\{\mathrm{tr}(O_i\rho)\}_i$ within constant addtive error $\varepsilon$ and confidence $1 - \delta$ has sampling complexity $\mathcal{O}(I\varepsilon^{-2} \ln(1/\delta))$.
:::
*Discussion:* This conjecture should at least satisfied the following known result:

- If $O_i$ are commuting, the sampling complexity is $\mathcal{O}(\ln(L))$. 
- If $O_i$ are not commuting but low weight $w$, the sampling complexity is $3^{w}\mathcal{O}(L)$.
- If $O_i$ are not commuting but low rank $r$, the sampling complexity is $r\mathcal{O}(L)$.

:::{.callout-important icon="false"}
## Conjusture (Product Hardness)
...
Arbitary POVM on single qubits can be coarse-grained to $M(x, y, z)$ as a 8-outcome measurement.
...
#### Unshrpness v.s. variance

After prove that $E^{\eta_x}_{a|X}, E^{\eta_y}_{a|Y}, E^{\eta_z}_{a|Z}$ are jointly measurable, we want to use $N$ times of joint measurement results $\{(x_i,y_i,z_i)\}_{i=1}^N$ to estimate $\{\mathrm{tr}(\rho X), \mathrm{tr}(\rho Y), \mathrm{tr}(\rho Z)\}$. Note that
...
Consider the simplest observable set $\mathcal{O} = \{P,Q\}$ where $P,Q$ are both Pauli observables. Exsimation of those two observables correspond to two-outcome POVMs $\{M_1 := \mathbb{I} - M_0, M_0\}$ and $\{N_1 := \mathbb{I} - N_0, N_0\}$ given $M_0 = \frac{P + \mathbb{I}}{2}$ and $N_0 = \frac{Q + \mathbb{I}}{2}$.

#### Homogenous compatiablity robustness and its SDP form
...
This inspires @heinosaari2015noise to define a rigrous measure for compatibility on arbitrary operator set $\{O_k\}_{k=1}^L$ by mixing $O_k$ with $\mathbb{I}$.
The existence of joint measurement then reduced to the existence of $G_{j,k}$ such that 

:::{#lem-comp2SDP .callout-important icon="false"}
## (Compatiablity robustness to SDP)
...
\end{align*}
$$
if and only if their exist a four matrix  $\{G_{00}, G_{01}, G_{10}, G_{11}\}$ such that
$$
\begin{align}
...
- in the homogenous case, i.e. $a_{ij} = a$ for all $i,j$, the above condition reduced to @thm-unsharp-pauli-joint-measurability above.
...
Thus the sharpest measurment is achieved by the optimization problem $\min_{G_{j,k}} \mu$ subject to @eq-restriction given a fixed $a_{j,k}>0$, and then take the minimum over all $a_{j,k}>0$ to get the sharpest measurment.
$$
\boxed{
\begin{align*}
...
:::{#lem-optimization-dual .callout-important icon="false"}
## (Optimization Dual)
...
#### Inhomogeneous compatiablity robustness to SDP
It is unnecessary to always use $\mathbb{I}$ mixedness to cancel the non-compatiability. In @skrzypczyk2019all, the authors use arbirary $N_{a|x}$ mixed with $M_{a|x}$ to cancel the non-compatiability:
$$
\begin{align*}
...
:::{#lem-dual_discrimination .callout-important icon="false"}
## (Dual Discrimination)
By finding the dual problem, the generalized compatiablity robustness of $\{P,Q\}$ is equivalent to the problem of finding the best mixed ensemble $\mathcal{E} = \mathcal{E}_P = \{\rho_0,\rho_1\} \cup \mathcal{E}_Q := \{\sigma_0,\sigma_1\}$ such that:

- (i). Given $P,Q$ specified, using the $\{M_0, M_1\}$ or $\{N_0, N_1\}$ measurements to discriminate the index $0$ or $1$.
- (ii). Without getting the information that the state is coming from $\mathcal{E}_P$ or $\mathcal{E}_Q$, distingush index $0$ or $1$.
...
Question left is how to re-interpret the restriciton $A_i + B_k \leq W$ and $\mathrm{tr}(W) < 1$ in terms of the properties of those two mixed state ensembles. 
...
\end{align*}
$$
Thus, the two restriction can be rewrite as the statement as $N^*$-hardnerss

 **Without getting the information that the state is coming from $\mathcal{M}$ or $\mathcal{N}$, nobody can distingush index $j,k$ with probability better than $\frac{1}{N^*}$.**
...
- Bob prepares ensemble $\mathcal{E}$ that has the $N^*$-hardnerss. 
...
- First we need to first generalized the discussion above to Pauli sets with greater than 2 observables.


#### Post-processing dan realization in the genralized robustness case
t.b.c.

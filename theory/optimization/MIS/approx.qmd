...
- Readings
date: '2026-07-06'
description: 'SOTA algorithm and PTAS for MWIS'
lang: en
title: SOTA algorithm and PTAS for MWIS
...
An important structure of SUDG is that the "right end" properties is sufficient to check the independent relations, as summarized in @lem-udgstrip
...
This property induces an efficient DP algorithm for its MWIS.
...
Use @lem-udgstrip, we noted that $S_{i,j}\cup\{k\}\in\mathcal I_{j,k}$ for all $i$ satisfied
$$
...
Then to solve MWIS on general udg, we split the udg into strips $H_1,\cdots,H_n$ with MWIS on that strip as $I_1,\cdots,I_n$. Further denote
$$
I_{\mathrm{odd}} = I_1\cup I_3\cup I_5\cup\cdots, \quad I_{\mathrm{even}} = I_2\cup I_4\cup I_6\cup\cdots.
$$
and use the fact that the subset of MWIS is MWIS on the subset, we conclude:
$$
...
\end{align}
$$
If ones can solve the MWIS on $H_{l,m,n}$, then union of them constitues an approximation of the optimal result.


:::{#lem-udgkstrip .callout-important icon="false"}
Given the $k$-strip splitting of a unit-disk graph with $k + 1$ phases, then the maximum MWIS $I^*$ among them satisfies $|I^*| \ge \frac{k}{k+1}|OPT|$.
:::

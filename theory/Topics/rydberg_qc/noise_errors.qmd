...
For spontaneous emission on individual atoms (no collective decay), the decay superoperator has tensor-product structure. To first order in the decay rates $\Gamma_j$:

$$
H_{\rm eff}(t) \approx \left(H_s(t) - i\frac{\hbar}{2}\sum_j \Gamma_j\proj{j}\right)^{\otimes 2},
$$ {#eq-Heff-2atom}

allowing the two-atom propagator $U_{\rm eff}(t) = U_{{\rm eff},1}(t) \otimes U_{{\rm eff},1}(t)$ to be computed from the single-atom propagator.

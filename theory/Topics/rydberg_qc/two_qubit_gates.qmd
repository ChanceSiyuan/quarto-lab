...
For the two-qubit **CPHASE** gate, the average gate fidelity takes a compact form in terms of the final state overlaps [@Nielsen_2002]. Define $a_q = \langle q|\psi_q(T)\rangle$ where $|\psi_q(T)\rangle$ is the state evolved from initial state $|q\rangle$ ($q \in \{01, 10, 11\}$) under $H'_{\rm tot}$ for duration $T$. Then:

$$
\bar F = \frac{1}{20}\bigl(|1 + 2a_{01} + a_{11}|^2 + |a_{11}|^2 + 2|a_{01}|^2 + 1\bigr).
$$ {#eq-Fbar}

This formula, derived via the Choi-Jamiołkowski isomorphism and the $d=4$ Hilbert space integral, encapsulates the fact that for a CPHASE gate, only the accumulation of specific phases by $|01\rangle$ and $|11\rangle$ states matters; $|00\rangle$ is dark and $|10\rangle$ is symmetric with $|01\rangle$.
...
0 & \sqrt{2}\tilde\Omega_1^*/2 & 2\Delta & \sqrt{2}\tilde\Omega_2/2 & 0 \\
0 & \tilde\Omega_2^*/2 & \sqrt{2}\tilde\Omega_2^*/2 & \delta & \tilde\Omega_1/2 \\
0 & 0 & 0 & \tilde\Omega_1^*/2 & \Delta + \delta
\end{pmatrix}.
$$ {#eq-H-gg}
...
Setting $\partial_t\psi_i = 0$ (adiabatic elimination) gives the effective two-level system between $|1\rangle$ and $|5\rangle$:

...
Given $n$-qubit anticommuting Pauli observables $P,Q$ ($\{P,Q\}=0$) with unsharp POVM elements $M_j = \frac{\mathbb{I}+(-1)^j 3\varepsilon P}{2^n}$ and $N_k = \frac{\mathbb{I}+(-1)^k 3\varepsilon Q}{2^n}$ for $\varepsilon\in[0,1/3]$, the generalized compatibility robustness is
$$
s^*(\varepsilon) = \max\!\bigl(1,\,(2-\sqrt{2})(1+3\varepsilon)\bigr).
$$
The threshold is at $3\varepsilon_c = 1/\sqrt{2}$ (the joint measurability boundary from @thm-unsharp-pauli-joint-measurability): for $3\varepsilon \leq 1/\sqrt{2}$ the measurements are already jointly measurable ($s^*=1$); for $3\varepsilon > 1/\sqrt{2}$, $s^*=(2-\sqrt{2})(1+3\varepsilon)$. The maximum at $\varepsilon=1/3$ gives $s^* = 4-2\sqrt{2}$. The optimal discrimination states are the eigenstate projectors $\frac{\mathbb{I}\pm P}{2^n}$, independent of $\varepsilon$.
:::
::: {.callout-note collapse="true"}
## Proof (Click to expand)
...
Since $M_j = \frac{\mathbb{I}_2 + (-1)^j\eta X}{2}\otimes \mathbb{I}_{\text{rest}}$ with $\eta = 3\varepsilon$, the traces factor: $\text{Tr}(A_j M_j) = \text{Tr}_1(a_j m_j)$ with $m_j = \frac{\mathbb{I}_2 + (-1)^j\eta X}{2}$, and similarly $n_k = \frac{\mathbb{I}_2 + (-1)^k\eta Z}{2}$ for $B_k, N_k$. The constraints $A_j + B_k \leq W$ also reduce to $a_j + b_k \leq w$. Thus the $n$-qubit dual SDP is equivalent to the single-qubit dual SDP with effective variables $(a_j, b_k, w)$.
...
For $\eta > 1/\sqrt{2}$, the optimal $\alpha = \gamma = \frac{1}{2+\sqrt{2}}$ is independent of $\eta$. The optimal discrimination states are $\rho_j^{(P)} = \frac{\mathbb{I}+(-1)^j P}{2^n}$ (eigenstate projectors of $P$, sharpness 1) for any $\varepsilon > \varepsilon_c$. With $\eta = 3\varepsilon$:
$$
s^*(\varepsilon) = \max\!\bigl(1,\,(2-\sqrt{2})(1+3\varepsilon)\bigr).
$$

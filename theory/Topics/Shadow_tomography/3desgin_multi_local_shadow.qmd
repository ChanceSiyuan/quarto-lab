...
& = 4^{-k+1}\sum_{\mathbf{p}^{(l>0)},\mathbf{w}^{(l>0)} \in \mathbb{F}_2^{2n}}r(\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'}) (-1)^{[\mathbf{p}^{(1)}, \mathbf{r}^{'}]+[\mathbf{p}^{(2)}, \mathbf{w}^{(1)}+\mathbf{p}^{(1)}] + \cdots + [\mathbf{p}^{(k-1)}, \mathbf{w}^{(k-2)}+\mathbf{p}^{(k-2)}]+[\mathbf{p}^{(1)}, \mathbf{w}^{(1)}] + [\mathbf{p}^{(2)}, \mathbf{w}^{(2)}] + \cdots + [\mathbf{p}^{(k-1)}, \mathbf{w}^{(k-1)}]} \cdot \\
...
\end{align*}
$$
Here in the last inequality, we define $\mathbf{w}^{(m)} = \mathbf{p}^{(m)} + \mathbf{p}^{',(m)}$ for $m = 1,\cdots,k-1$. 
Based on the @lem-LC-shadow-kcopy, we have the equality:
$$
\begin{align*}
r(\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'}) &= \left\{ 
    \begin{aligned}
        & \frac{2^k + 1}{2^k + 2} \left[2^k \delta_{\mathbf{r}, \mathbf{r}'} \prod_{m = 1}^{k-1} \delta_{\mathbf{w}^{(m)}, \mathbf{0}} + 1 +  (-1)^{[\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'}]}\right] \text{~if~}\mathbf{r},\mathbf{r}' \neq 0\\
        &1 \text{~if else}
    \end{aligned}\right.
    \end{align*}
$$
Here $[\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'}]$ is the only term related to $\mathbf{p}^{(l>0)}$ with
$$
\begin{align*}
...
The first term and second term in $r(\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'})$ are not related to $\mathbf{p}^{(l>0)}$. 
...
$$
    ...
    &\quad + \underbrace{[\mathbf{p}^{(2)}, \mathbf{w}^{(1)}] + \cdots + [\mathbf{p}^{(k-1)}, \mathbf{w}^{(k-2)}+\mathbf{p}^{(k-2)}]}_{\text{Group A}}\\
...
- Then sum over $\mathbf{p}^{(2)} \in \mathbb{F}_2^{2}$ gives the result $4  (-1)^{\mathrm{phase}_{1,3}}(-1)^{\mathrm{phase}_{3,k-1}}$, with $\mathrm{phase}_{1,3} = [\mathbf{r}' + \mathbf{w}^{(1)}, \mathbf{w}^{(1)} + \mathbf{w}^{(2)}]$ and

$$
\begin{align*}
    \text{phase}_{3,k-1} &= \underbrace{[\mathbf{p}^{(3)}, \mathbf{w}^{(1)}+\mathbf{w}^{(2)}+\mathbf{r}']+ \cdots + [\mathbf{p}^{(k-2)}, \mathbf{w}^{(k-2)}+\mathbf{p}^{(k-2)}]}_{\text{Group A}}\\
    & \quad +\underbrace{ [\mathbf{p}^{(3)}, \mathbf{w}^{(3)}]+ \cdots + [\mathbf{p}^{(k-1)}, \mathbf{w}^{(k-1)}]}_{\text{Group B}}
\end{align*}
$$
This term $\text{phase}_{3,k-1}$ is exact the same form as $\text{phase}_{1,k-1}$, with the index shifted by 2 and $\tilde{\mathbf{r}}_{2} = \mathbf{w}^{(1)}+\mathbf{w}^{(2)}+\mathbf{r}'$.
Do this multiple times, we can then get the general form of $\text{phase}_{1,k-1}$.
...
**Phase structure and Pauli multiplication.**

The operator part contains Pauli products. Using the full Pauli multiplication rule from line 152:
$$

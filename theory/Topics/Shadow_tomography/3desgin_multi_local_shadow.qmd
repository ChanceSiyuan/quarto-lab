...
$$
*Verification at $k=2$*: Since $S^2=\mathbb{I}_4$ for $k=2$, the even-$k$ formula gives $\frac{5}{3}\sigma_{\mathbf{r}}^{\otimes 2}$, matching @eq-V-case1. $\checkmark$
...
\right.
$$
*Verification at $k=2$*: Since $S^2=\mathbb{I}_4$, the even-$k$ formula gives $\frac{16+10}{6}\mathbb{I}_4 - \frac{4}{6}\text{SWAP} = \frac{13}{3}\mathbb{I}_4 - \frac{2}{3}\text{SWAP}$, matching @eq-V-case1 Case 2. $\checkmark$

*Verification at $k=3$*: Since $S^2=S^{-1}$ for $k=3$, the odd-$k$ formula gives $v = \frac{1}{5}\left(7\,\mathbb{I}^{\otimes 3} + 9\,S^{-1} - 2\,S\right)$. $\checkmark$
...
**Quantitative analysis.** Consider the purity estimator ($\mathbf{r}=\mathbf{0}$) applied to a pure state $\rho=|\psi\rangle\langle\psi|$. Since $S|\psi\rangle^{\otimes k}=S^2|\psi\rangle^{\otimes k}=|\psi\rangle^{\otimes k}$, the per-site second moment for even $k$ evaluates to
$$
\operatorname{tr}\!\bigl(\rho^{\otimes k}\cdot v(\mathbf{0},\mathbf{0})\bigr)
= \frac{4\cdot 2^k + 2(2^k+1) - 4}{2^k+2}
= \frac{6\cdot 2^k - 2}{2^k+2}
\xrightarrow{k\to\infty} 6.
$$
At $k=2$ this gives $22/6 = 11/3 \approx 3.67$. For the $n$-qubit purity estimator on a pure state, the worst-case variance is
$$
\operatorname{Var} = \left(\frac{6\cdot 2^k - 2}{2^k+2}\right)^n - 1
\xrightarrow{k\to\infty} 6^n - 1,
$$
versus $(11/3)^n - 1 \approx 3.67^n$ at $k=2$. The sample complexity therefore increases by a factor of $(18/11)^n \approx 1.64^n$ in the $k\to\infty$ limit relative to $k=2$.

**Physical explanation — three mechanisms.**

1. **Signal decay.** $\operatorname{tr}(\rho^k) \leq \operatorname{tr}(\rho^2)$ for $k\geq 2$, with exponential decay $\sim \lambda_{\max}^k$ for mixed states, while the variance stays $O(1)$. The signal-to-noise ratio deteriorates exponentially in $k$.

2. **Rescaling blowup.** The inverse measurement channel involves factors of $(2^k+1)$, reflecting that the $k$-qubit Clifford 3-design acts on a $2^k$-dimensional Hilbert space. Larger rescaling amplifies statistical noise.

3. **Literature context.** @Chen_Gong_2024 established a memory–sample complexity tradeoff for purity testing: having $k$ qubits of quantum memory does not improve sample complexity until $k > n/2$, exhibiting a sharp phase transition. This contrasts with Pauli shadow tomography [@Gong_2024_pauli], where the improvement from quantum memory is continuous.
:::

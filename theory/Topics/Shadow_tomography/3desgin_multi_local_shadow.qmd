...
depends only on $\{\mathbf{w}^{(l)}\}$ and $\mathbf{r}'$. In the case $\mathbf{w}' = \mathbf{0}$, we have $\text{phase}_{\mathbf{w}} = 0$.
...
Plug in the definition of $\text{phase}_{\mathbf{w}}$, we get:

**Third term: $(-1)^{[\mathbf{u}_{\mathbf{r}},\mathbf{u}_{\mathbf{r}'}]}$**

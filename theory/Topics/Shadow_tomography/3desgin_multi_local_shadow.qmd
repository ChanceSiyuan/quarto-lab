...
## Proof (Click to expand)
 By
[Lemma (Onsite 2design)](/theory/Topics/Shadow_tomography/intro.qmd#lem-onsite_2design)
...
Based on @lem-covar-pauli, $\text{Var} \leq (13/3)^n$, which still requires exponentially large sampling
...
\begin{align*}
  ...
  & \hspace{-1.15cm}= \left(7 \mathbb{I}\mathbb{I} + \frac{1}{2}(XX+YY+ZZ)\right)^{\otimes n}= \left(\frac{13\mathbb{I}\mathbb{I}+2\text{SWAP}}{2}\right)^{\otimes n} \leq \left(\frac{15\mathbb{I}\mathbb{I}}{2}\right)^{\otimes n}. 
\end{align*}
$$
 This gives the variance scaling of $7.5^n$, which being
improved to $4.33^n$ in the 2-replica local shadow. Noted that the
...
\end{align*}
$$
We define two such index vectors $\mathbf{u}_{\mathbf{r}}$ and $\mathbf{u}_{\mathbf{r}'}$ corresponding to summation variables $\{\mathbf{p}^{(l)}\}$ and $\{\mathbf{p}^{',(l)}\}$ respectively. Since $\mathbf{V}$ factorizes over sites (@lem-LC-shadow-kcopy), it suffices to analyze the per-site variance operator:
$$
\begin{align*}
...
\end{align*}
$$
**Assembling the rescaling function.** For $k$-copy 3-design local shadow, @lem-LC-shadow-kcopy gives:
$$
\begin{align*}
...
$$
At $k=2$ this gives $22/6 = 11/3 \approx 4.33$. The sample complexity therefore increases by a factor of $(18/11)^n \approx 1.64^n$ in the $k\to\infty$ limit relative to $k=2$.
:::

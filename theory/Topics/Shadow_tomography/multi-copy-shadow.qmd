...
## Theorem(Non-local Pauli shadow tomography)
[]{#theorem:nonlocal_pauli_shadow label="theorem:nonlocal_pauli_shadow"}
...
::: {.callout-important icon="false"}
## Lemma (Signal estimation)

[]{#lemma:signal_estim label="lemma:signal_estim"} If we can estimate
...
Lemma [\[lemma:gen_swap_est\]](#lemma:gen_swap_est){reference-type="ref"
reference="lemma:gen_swap_est"}. Note that the computational projective
operator
...
Lemma [\[lemma:eigvec_QSWAP\]](#lemma:eigvec_QSWAP){reference-type="ref"
reference="lemma:eigvec_QSWAP"} below.

::: {.callout-important icon="false"}
## Lemma(Common eigenvectors of SWAP and QSWAP)

[]{#lemma:eigvec_QSWAP label="lemma:eigvec_QSWAP"} Given
...
As summarized in
Lemma [\[lemma:var_nonlocal_nonlinear_shadow\]](#lemma:var_nonlocal_nonlinear_shadow){reference-type="ref"
reference="lemma:var_nonlocal_nonlinear_shadow"}, the variance of
**NLMC** shadow to estimate $\text{tr}(O \rho^t)$ with unitary sampled
...
::: {.callout-important icon="false"}
## Lemma(Variance of non-local Pauli shadow)

[]{#lemma:var_nonlocal_nonlinear_shadow
label="lemma:var_nonlocal_nonlinear_shadow"} Given
...
[]{#lemma:LC_shadow_kcopy label="lemma:LC_shadow_kcopy"} In the case of
local k-copy randomized measurements on $\rho^{\otimes k}$, let the
...
*Proof.* By
Lemma [\[lemma:onsite_2design\]](#lemma:onsite_2design){reference-type="ref"
reference="lemma:onsite_2design"}, we conclude that its inverse is
...
reference="lemma:LC_shadow"}, we decomposed the operator $O^{(k)}$ on
...
Lemma [\[lemma:onsite_2design\]](#lemma:onsite_2design){reference-type="ref"
reference="lemma:onsite_2design"}, the inverse channel
...
reference="lemma:state_trajectory"}. With $d=2^k$, the prefactor per
...
Lemma [\[lemma:LC_shadow\]](#lemma:LC_shadow){reference-type="ref"
reference="lemma:LC_shadow"}. Depending on three cases, we calculate:
$$\begin{align*}
...
[]{#def:cov_oper label="def:cov_oper"} Given two operators with
decomposition
...
Lemma [\[lemma:LC_shadow_kcopy\]](#lemma:LC_shadow_kcopy){reference-type="ref"
reference="lemma:LC_shadow_kcopy"} before.
:::
...
[]{#lemma:covariance_decomp label="lemma:covariance_decomp"} Given an
k-copy operator
...
\end{align*}
$$
 Here at the last line, we use the
definition [\[def:cov_oper\]](#def:cov_oper){reference-type="ref"
reference="def:cov_oper"} and the fact that $R(W,W') = R(W',W)$. ◻
:::
...
Here below we use
Lemma [\[lemma:LC_shadow_kcopy\]](#lemma:LC_shadow_kcopy){reference-type="ref"
reference="lemma:LC_shadow_kcopy"} to evaluate the covariance operator
of $O_{\mathbf{r}}$ explicitly.

::: {.callout-important icon="false"}
## Lemma(covariance operator of non-local Pauli shadow)

[]{#lemma:covar_Pauli label="lemma:covar_Pauli"} 
If we take
$O_{\mathbf{r}}= (\sigma_{\mathbf{r}} \otimes \mathbb{I}_n) \cdot \text{SWAP}^{\otimes n}$
for Pauli operator $\sigma_{\mathbf{r}}$ and denote
$\text{wt}(\sigma_{\mathbf{r}})$ as the weight of this Pauli operator,
then we have 
$$
\begin{align*}
        \mathbf{V}(O_{\mathbf{r}}, O_{\mathbf{r}'}) = (-1)^{[\mathbf{r}, \mathbf{r}']} \bigotimes_{j=1}^n 
        \left(
        % \left\{
        \begin{aligned}
            &({5}/{3})\cdot \sigma_{\mathbf{r}_j}\otimes \sigma_{\mathbf{r}'_j}~ \text{if~} \mathbf{r}_j, \mathbf{r}_j'  \neq \mathbf{0}\\
            &({1}/{2})\cdot \mathbb{I} \otimes \sigma_{\mathbf{r}_j} \text{~if~} \mathbf{r}_j' = \mathbf{0},  \mathbf{r}_j\neq \mathbf{0}\\
            &({1}/{2})\cdot \mathbb{I} \otimes \sigma_{\mathbf{r}'_j} \text{~if~}  \mathbf{r}_j' \neq \mathbf{0},  \mathbf{r}_j = \mathbf{0}\\
            &(11/3)\cdot \mathbb{I}_2 \text{~if~} \mathbf{r}_j = \mathbf{r}_j'  = 0
        \end{aligned}
        % \right. 
        \right)
\end{align*}
$$
Specifically,
$\mathbf{V}(O_{\mathbf{r}},O_{\mathbf{r}}) =  \left({11}/{3}\right)^{n}\cdot \left({5}/{11}\right)^{\text{wt}(\sigma_{\mathbf{r}})} \sigma_{\mathbf{r}} \otimes \sigma_{\mathbf{r}}.$
:::

::: {.callout-note collapse="true"}
## Proof (Click to expand)

*Proof.* We consider the observable
$O_{\mathbf{r}}= (\sigma_{\mathbf{r}} \otimes \mathbb{I}_n) \cdot \text{SWAP}^{\otimes n}$
with
$\mathbf{r} = (\mathbf{r}_1, \cdots, \mathbf{r}_n) \in (\mathbb{F}_2^2)^n$
and rewrite the observable according to the fact
$\text{SWAP}^{\otimes n} = 2^{-n}\sum_{\mathbf{p} \in \mathbb{F}_2^{2n}} \sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}}$
as: 
$$
\begin{align*}
...
$O_{\mathbf{r}},O_{\mathbf{r}'}$ satisfied the local decomposition
$\left\{\begin{aligned}W_j \propto \sigma_{\mathbf{r}_j + \mathbf{p}_j} \otimes \sigma_{\mathbf{p}_j}\\ W'_j \propto \sigma_{\mathbf{r}'_j + \mathbf{p}'_j} \otimes \sigma_{\mathbf{p}'_j}\end{aligned}\right.$
Here below we abbreviate the foot label $j$ for simplicity. Using
Definition [\[def:cov_oper\]](#def:cov_oper){reference-type="ref"
reference="def:cov_oper"}, we calculate the covariance operator as
$$
\begin{align*}
...
\end{align*}
$$ 
Here in the third equality, we use the fact that if
$\mathbf{r},\mathbf{r}' \neq  \mathbf{0}$, we cannot have
$\mathbf{r} + \mathbf{p} = \mathbf{p} = \mathbf 0$ or
$\mathbf{r}' + \mathbf{p}' = \mathbf{p}' = \mathbf 0$. Thus, we plug in
the second line in
Eq.[\[eq:lc_Pauli_rescale\]](#eq:lc_Pauli_rescale){reference-type="eqref"
reference="eq:lc_Pauli_rescale"}.

In the case of $\mathbf{r}_j,\mathbf{r}'_j \neq 0$, we simplified term
by term as below 
$$
\begin{align*}
...
\end{align*}
$$
 Combining all these gives the result of the first line of
Eq.[\[eq:mid_V_Orho2\]](#eq:mid_V_Orho2){reference-type="eqref"
reference="eq:mid_V_Orho2"} as 
$$
\begin{align*}
...
In the case of $\mathbf{r}_j =\mathbf{r}'_j= 0$, the calculation is
covered in
Eq.[\[eq:prity_multicopy_shadow_variance\]](#eq:prity_multicopy_shadow_variance){reference-type="eqref"
reference="eq:prity_multicopy_shadow_variance"}. We calculate by list 16
terms one-by-one as 
$$
\begin{align*}
        \text{second line} 
...
\end{align*}
$$
Here in the last line we use the definition that
$\Pi_{\mathrm{asym}} = (\mathbb{I}_2 - \text{SWAP})/2 = {(\mathbb{I}_2 -\sigma_{01}^{\otimes 2}-\sigma_{10}^{\otimes 2}-\sigma_{11}^{\otimes 2})}/{4}$.

In the case of $\mathbf{r}'= 0 \neq \mathbf{r}$, if we simply adapt the
result in Eq.[\[eq:V_eq1\]](#eq:V_eq1){reference-type="eqref"
reference="eq:V_eq1"}, we will get the result
$v({\mathbf{r}_j}, {\mathbf{r}_j'}) =({5}/{6})\left(\sigma_{\mathbf{r}}\otimes \mathbb{I} +\mathbb{I}\otimes \sigma_{\mathbf{r}} \right)$.
However, in the case of $\mathbf{p}' = 0$ during the summation, we
mistakenly counted the constant
$r((\mathbf{r}+ \mathbf{p})\mathbf{p}, \mathbf{0}\mathbf{0}) = 1$ into
$r((\mathbf{r}+ \mathbf{p})\mathbf{p}, \mathbf{0}\mathbf{0}) = (5/6)[4\delta_{\mathbf{r},\mathbf{0}}\delta_{\mathbf{p},\mathbf{0}} +2]$.
To correct this term, we sum in the case $\mathbf{p}' = 0$ and get the
result 
$$
\begin{align*}
...
we calculate the correction in the case $\mathbf{p} = 0$ as
$$
\begin{align*}
        \Delta &= -4^{-1}(5/6)\sum_{\mathbf{p}'}[4\delta_{\mathbf{p}',0}\delta_{\mathbf{r}',0} + 2]\sigma_{\mathbf{r}'}\sigma_{\mathbf{p}'} \otimes \sigma_{\mathbf{p}'} + 4^{-1} \sum_{\mathbf{p}'} \sigma_{\mathbf{r}'}\sigma_{\mathbf{p}'} \otimes \sigma_{\mathbf{p}'} = - \frac{1}{3}(\sigma_{\mathbf{r}'} \otimes \mathbb{I})\text{SWAP},\\
...
\end{align*}
$$
Because the input states are restrained in the symmetric
space $\rho \otimes \rho$, we can safely symmetrize this operator and
drop the terms that contained the antisymmetric operator and get the
final result 
$$
\begin{align*}
        v(\mathbf{r}_j, \mathbf{r}_j') = 
        \left\{\begin{aligned}
            &\frac{5}{6}\left(\sigma_{\mathbf{r}_j}\otimes \sigma_{\mathbf{r}'_j} +\sigma_{\mathbf{r}'_j}\otimes \sigma_{\mathbf{r}_j} \right)\text{~if~} \mathbf{r}_j, \mathbf{r}_j'  \neq \mathbf{0},
            \hspace{6em} \frac{11\mathbb{I}_2}{3} \hspace{5em}\text{~if~} \mathbf{r}_j=\mathbf{r}_j'= \mathbf{0}, \\
            &\left(\frac{1}{2}\mathbb{I} \otimes \sigma_{\mathbf{r}_j}+ \frac{5}{6}\sigma_{\mathbf{r}_j}\otimes\mathbb{I}\right) \text{~if~} \mathbf{r}'_j = \mathbf{0},  \mathbf{r}_j\neq \mathbf{0},
            \qquad \left(\frac{1}{2}\mathbb{I} \otimes \sigma_{\mathbf{r}'_j} + \frac{5}{6}\sigma_{\mathbf{r}_j'}\otimes\mathbb{I}\right)\text{~if~} \mathbf{r}_j = \mathbf{0},  \mathbf{r}'_j\neq \mathbf{0}.
        \end{aligned}\right.
\end{align*}
$$
 As for the special case with $\mathbf{r} = \mathbf{r}'$,
we get the result 
$$
\begin{align*}
        \mathbf{V}(O_\mathbf{r},O_\mathbf{r}) = \bigotimes_{i=1}^n \left\{ \begin{aligned}\frac{5}{3}\sigma_{\mathbf{r}_i}^{\otimes 2} &\text{~if~}\sigma_{\mathbf{r}_i} \neq \mathbb{I}_2 \\ \frac{11}{3}\sigma_{\mathbf{r}_i}^{\otimes 2} &\text{~if~}\sigma_{\mathbf{r}_i} = \mathbb{I}_2  \end{aligned} \right. = \left(\frac{11}{3}\right)^{n}\left(\frac{5}{11}\right)^{\text{wt}(\sigma_{\mathbf{r}})}\sigma_{\mathbf{r}}^{\otimes 2}.
\end{align*}
$$
:::
...
Based on
Lemma [\[lemma:covar_Pauli\]](#lemma:covar_Pauli){reference-type="ref"
reference="lemma:covar_Pauli"}, this gives the variance
$\text{Var}=(11/3)^n$, which still requires exponentially large sampling
...
Lemma [\[lemma:LC_shadow_kcopy\]](#lemma:LC_shadow_kcopy){reference-type="ref"
reference="lemma:LC_shadow_kcopy"}, we calculate the variance operator
as $$\begin{align*}
    ...
    \end{aligned}\right.\right) 
\end{align*}$$ Here in the last equality, we use the identity
Eq.[\[eq:lc_Pauli_rescale\]](#eq:lc_Pauli_rescale){reference-type="eqref"
reference="eq:lc_Pauli_rescale"} and the fact that $\mathbf{u}=0$ or
...
[]{#lemma:max_variance label="lemma:max_variance"} Let $O$ be a
Hermitian operator with the maximum and minimum eigenvalues
...
[]{#lemma:superm_of_2copy_shadow label="lemma:superm_of_2copy_shadow"}
...
Then based on
Lemma [\[lemma:max_variance\]](#lemma:max_variance){reference-type="ref"
reference="lemma:max_variance"}, the variance of this estimator
satisfied $$\begin{align*}
        \sup_{\rho \in \mathcal{D}(\mathcal{H}_n)}\text{Var}_{b}(\rho,\alpha) &= 4^{-1}\left[[(1+ \alpha)/2]^{n}-[(1- 3\alpha)/2]^{n}\right]^2,
\end{align*}$$ To prove the variance of the shadow method, based on
Lemma [\[lemma:LC_shadow_kcopy\]](#lemma:LC_shadow_kcopy){reference-type="ref"
reference="lemma:LC_shadow_kcopy"} that proved before, we first evaluate
...
According to
lemma [\[lemma:LC_shadow_kcopy\]](#lemma:LC_shadow_kcopy){reference-type="ref"
reference="lemma:LC_shadow_kcopy"}, this means that the second moments
takes the value
...
This estimation task in
Lemma [\[lemma:superm_of_2copy_shadow\]](#lemma:superm_of_2copy_shadow){reference-type="ref"
reference="lemma:superm_of_2copy_shadow"} is useful in estimating the
non-linear property
$L_{\gamma}(\rho):= \sum_{P \in \mathcal{P}_n }\gamma^{\mathrm{wt}(P)}\text{tr}(P \rho P \rho)$.
We find its two-copy estimator as stated by
Lemma [\[lemma:otoc-non-linear\]](#lemma:otoc-non-linear){reference-type="ref"
reference="lemma:otoc-non-linear"} below.

::: {.callout-important icon="false"}
## Lemma

[]{#lemma:otoc-non-linear label="lemma:otoc-non-linear"} The non-linear
value $L_\gamma(\rho)$ has a two-copy unbiased estimator $$\begin{align*}
...
$\alpha = {(1-\gamma)}/{(3\gamma + 1)}$. Use
Lemma [\[lemma:superm_of_2copy_shadow\]](#lemma:superm_of_2copy_shadow){reference-type="ref"
reference="lemma:superm_of_2copy_shadow"}, we conclude that the variance
of Bell sampling methods and 2-copy local 3-design shadow are separately
...
[]{#lemma:super_nonli label="lemma:super_nonli"} To estimate
$L_{\gamma}(\rho):= \sum_{P \in \mathcal{P}_n }\gamma^{\mathrm{wt}(P)}\text{tr}(P \rho P \rho)$
...
Lemma [\[lemma:onsite_2design\]](#lemma:onsite_2design){reference-type="ref"
reference="lemma:onsite_2design"}, we calculate the reconstruction
...
Lemma [\[lemma:2-foldWein\]](#lemma:2-foldWein){reference-type="ref"
reference="lemma:2-foldWein"} in the $n = 2$ case, with
...
Suppose an algorithm get classical shadow for
$\{\mathrm{tr}(P\rho^{2})\}_{P \in \mathcal{O}}$ with additive error
$\varepsilon$ and constant success rate $1- \delta$ using $2N$ copies,
then with the same success rate, its result can be used to discriminate
mixed state ensemble
$\mathcal{E}_1 = \{\left(\mathbb{I}_{2n} + 3\varepsilon (P\otimes \mathbb{I}) \cdot O_P\right)/2^{2n}\}_{P \in \mathcal{O}}$
from the maximumly mixed state $\mathbb{I}_{2n}/2^{2n}$.
:::
...
Suppose an algorithm get classical shadow for
$\{\mathrm{tr}(P\rho^{2})\}_{P \in \mathcal{O}}$ with additive error
$\varepsilon$ and constant success rate $1- \delta$, then with the same
success rate, its result can be used to discriminate mixed state
ensemble
$\mathcal{E}_1 = \{\left(\mathbb{I} + 3\varepsilon P\right)/d\}_{P \in \mathcal{O}}$
from the ensemble 
$$
\begin{align*}

...
Van der Waals repulsion provides the Ising $ZZ$ coupling; global laser driving provides the transverse field. See the [detailed notes](TFIM.qmd) for more details.
...
See the [detailed notes](XY_XXZ.qmd) for more details.
...
$$ {#eq-xxz}
This is a generalization of XY-model, where anisotropy term $\Delta_{\text{anis}}$ interpolates from the XY limit ($\Delta_{\text{anis}}=0$) through the isotropic Heisenberg point ($\Delta_{\text{anis}}=1$) to the Ising limit. See the [detailed notes](XY_XXZ.qmd) for more details.
...
For the ordinary TFIM Hamiltonian,
$$
H = \frac{\Omega}{2}\sum_i \sigma^x_i - \Delta \sum_i n_i + \sum_{i<j} \frac{C_6}{R_{ij}^6}\, n_i n_j
$$
...
Two-component hard-core bosons with $t$-$J$ derivation.See the [detailed notes](Boson-Hubbard.qmd).
$$
\begin{align*}
...
Three Rydberg levels encode spins and holes. See the [detailed notes](doped_antiferromagnet.qmd).
$$

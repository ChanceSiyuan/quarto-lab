...
In the specific perturbed Kitaev honeycomb model, the free Hamiltonian and perturbation satisfy
$$
\begin{align}
...
# Symmetry and Equivalence to Free Majorana Fermion
Now we discuss the symmetry of $H_{\text{hc}}$. We show that its conserved quantities are the "vortex" operators $W_p$:

:::{#lem-conserve-W .callout-important icon="false"}
## (Conservation of vortex operators)
$[H_{\mathrm{hc}}, W_p] = 0$, where $W_p := \sigma_1^x \sigma_2^y \sigma_3^z \sigma_4^x \sigma_5^y \sigma_6^z$ is defined on an arbitrary hexagon $p: 123456$, as illustrated in @fig-perturbation-vortex(a).
:::

:::{.callout-note collapse="true"}
## Proof (Click to expand)
As illustrated by @fig-perturbation-vortex(a), the free parts are constituted by $\sigma_1^z \sigma_2^z, \sigma_2^x \sigma_3^x, \cdots$, and they both commute with $W_p$. The three-body terms are composed of $\sigma_1^z \sigma_2^y \sigma_3^x, \cdots$ and also all commute with $W_p$. $\square$
:::
...
Now we search for states that satisfy both the eigenvalue equation $\tilde{H}\ket{\psi} = E\ket{\psi}$ and "Gauss's law" $D_j \ket{\psi} = \ket{\psi}$. This is still difficult because $\tilde{H}$ involves non-linear interaction between $\hat{u}_{ij}$ and $c_j$.
Fortunately, it is observed that $[\tilde{H}, \hat{u}_{ij}] = 0$. Thus, we can first search for states $\ket{\phi}$ that satisfy the eigenvalue equation $\tilde{H}\ket{\phi} = E\ket{\phi}$ and a certain "gauge law" $\hat{u}_{jk}\ket{\psi} = u_{jk}\ket{\psi}$, where $u_{jk}$ is a valid $\mathbb{Z}_2$ field. Then the real state $\ket{\psi}$ can be found by simply projecting back into the physical subspace $\mathcal{M} = \{\ket{v} \mid \forall j, D_j \ket{v} = \ket{v}\}$. This procedure is validated by the lemma below:
...
The operation $D_j$ preserves the vortex in the $\mathbb{Z}_2$ gauge field, as defined in @lem-vortex-reflect, and the distribution of the vortex reflects the quantity of $W_p$, which is the conserved operators of $H_{\mathrm{hc}}$ as proved in @lem-conserve-W.

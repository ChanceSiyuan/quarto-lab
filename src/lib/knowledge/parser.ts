...
/**
 * Quarto cross-reference keys use the same `@key` surface syntax as Pandoc
 * bibliography citations, but their reserved prefix identifies an object in
 * the rendered document rather than an entry in `literature/ref.bib`.
 */
const QUARTO_CROSSREF_KEY =
  /^(?:fig|tbl|lst|sec|eq|thm|lem|def|cor|prp|exm|nte)-/;
...
function readBody(
  ...
  visit(tree, (node) => {
    ...
    for (const match of body.slice(start, end).matchAll(CITATION_KEY)) {
      ...
      if (key.length > 0 && !QUARTO_CROSSREF_KEY.test(key)) {

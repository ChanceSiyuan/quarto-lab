...
/*
 * The buttons above set a box; without this the Bootstrap glyph inside them
 * inherits the body font scale and grows past it. Both the icon font and the
 * inline SVG the search button uses are pinned, because Quarto emits one or
 * the other depending on the version.
 */
#quarto-header .quarto-btn-toggle .bi,
#quarto-header .quarto-search-button .bi,
#quarto-header .quarto-btn-toggle .bi::before,
#quarto-header .quarto-search-button .bi::before {
  font-size: 15px;
  line-height: 1;
  vertical-align: middle;
}

#quarto-header .quarto-btn-toggle svg,
#quarto-header .quarto-search-button svg,
...
function differences(expected: unknown, actual: unknown, at: string): string[] {
  if (Array.isArray(expected)) {
    if (!Array.isArray(actual)) {
      return [`\`${at}\` must be a sequence, not ${JSON.stringify(actual)}`];
    }

    const problems: string[] = [];
    if (expected.length !== actual.length) {
      problems.push(
        `\`${at}\` must contain ${expected.length} item${expected.length === 1 ? "" : "s"}, not ${actual.length}`,
      );
    }
    for (let index = 0; index < Math.min(expected.length, actual.length); index += 1) {
      problems.push(...differences(expected[index], actual[index], `${at}[${index}]`));
    }
    return problems;
  }
...
export async function materializeQuartoProject(input: {
  ...
  /** Generated-project render restriction used only by the live editor. */
  renderOnly?: string;
    ...
    stringify(
      {
        ...
        crossref: FIXED_BASE_CONFIG.crossref,

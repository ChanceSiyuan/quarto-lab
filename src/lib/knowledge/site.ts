...
export interface BuildKnowledgeSiteOptions extends LoadKnowledgeOptions {
  ...
  /** Optional loopback port for an isolated editor preview. */
  previewPort?: number;
  /** One repository-relative QMD mirrored into the safe projection while editing. */
  previewWatchPath?: string;
}
...
function normalizedPreviewWatchPath(value: string): string {
  const relative = value.replace(/\\/g, "/");
  if (!relative.startsWith("knowledge/") || !relative.endsWith(".qmd") ||
      relative.split("/").some((segment) => !segment || segment === "." || segment === "..")) {
    throw new KnowledgeSiteError("preview watch path must name one QMD below knowledge/");
  }
  return relative;
}
...
async function prepareRender(options: BuildKnowledgeSiteOptions): Promise<PreparedRender> {
  ...
  const previewWatchPath = options.previewWatchPath === undefined
    ? undefined
    : normalizedPreviewWatchPath(options.previewWatchPath);
  const renderOnly = previewWatchPath?.slice("knowledge/".length);
  if (renderOnly && !graph.pages.has(renderOnly)) {
    throw new KnowledgeSiteError("preview watch path must name a validated knowledge page");
  }
  ...
  try {
    const project = await materializeQuartoProject({
      graph,
      workspace,
      bibliographyPath,
      ...(renderOnly
...
export async function previewKnowledgeSite(
  ...
  try {
    if (options.previewWatchPath !== undefined) {
      const relative = normalizedPreviewWatchPath(options.previewWatchPath);
      const sourcePath = path.join(prepared.repoRoot, ...relative.split("/"));
      const refresh = async (): Promise<void> => {
        ...
        try {
          ...
          const bibliographyPath = bibliographyPathFor(graph.repoRoot, options.bibliographyPath);
          const report = await validateGraph(graph, { bibliographyPath });
          if (!report.ok) throw new KnowledgeValidationError(report);
          await materializeQuartoProject({
            graph,
            workspace: prepared.workspace,
            bibliographyPath,
            renderOnly: relative.slice("knowledge/".length),
          });
          console.error(`knowledge preview: refreshed ${relative}`);
        } catch (error) {
          // Keep the last valid projection in place, just like Overleaf keeps
          // the last successful PDF when the current source does not compile.
          console.error(
            `knowledge preview: source not rendered; ${error instanceof Error ? error.message : String(error)}`,
          );
        } finally {
          refreshing = false;
          if (refreshAgain) {
      ...
      watchFile(sourcePath, { persistent: false, interval: 100 }, (current, previous) => {
        if (current.mtimeMs === previous.mtimeMs && current.size === previous.size) return;
        if (refreshTimer) clearTimeout(refreshTimer);
        refreshTimer = setTimeout(() => void refresh(), 220);
      });
    }
    const args = ["preview", ".", "--no-browser", "--no-execute"];
    if (options.previewPort !== undefined) {
      if (!Number.isInteger(options.previewPort) || options.previewPort < 1024 || options.previewPort > 65535) {
        throw new KnowledgeSiteError("preview port must be an integer between 1024 and 65535");
      }
      args.push("--host", "127.0.0.1", "--port", String(options.previewPort));
    }
    await runner.run(
    ...
    if (refreshTimer) clearTimeout(refreshTimer);
    if (watchedSourcePath) unwatchFile(watchedSourcePath);
    detach();
    await rm(prepared.workspace, { recursive: true, force: true });
  }
}

...
import { VALUATION_ONLY_NOTICE } from "./valuation-only-refresh.mjs";
...
async function readDerivationIfPresent(rootDir, problemId, runId) {
  const runDir = await resolveExistingRunDir(rootDir, problemId, runId);
  try {
    return JSON.parse(await readFile(join(runDir, "derivation.json"), "utf8"));
  } catch (error) {
...
function valuationOnlyDerivationOk(derivation, { problemId, runId, snapshotId }) {
  if (derivation === null) return true;
  return derivation?.schemaVersion === 1
    && derivation.kind === "qec-valuation-only-refresh"
    && derivation.problemId === problemId
    && derivation.runId === runId
    && derivation.refreshedSnapshotId === snapshotId
    && typeof derivation.sourceRunId === "string"
    && derivation.sourceRunId.length > 0
    && typeof derivation.sourceSnapshotId === "string"
    && derivation.sourceSnapshotId.length > 0
...
export async function verifyQecPortfolio({ rootDir }) {
  ...
  for (const id of EXPECTED_IDS) {
    try {
      ...
      try {
        const derivation = await readDerivationIfPresent(rootDir, id, reusable.runId);
        if (!valuationOnlyDerivationOk(derivation, {
          problemId: id,
          runId: reusable.runId,
          snapshotId: snapshot.manifest.snapshotId,
        })) errors.push(`${id} has invalid valuation-only derivation provenance.`);
      } catch (error) {
        errors.push(`${id} has invalid valuation-only derivation provenance: ${error.message}`);
      }

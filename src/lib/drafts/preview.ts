...
export interface PreviewDraftOptions {
   ...
   *
   * An editor serving one draft beside other previews has to know where to
   * point its browser, and letting Quarto pick would mean scraping its output.
   * The range is the unprivileged one, checked before the spawn, because a
   * port is the one argument here that does not come from a path.
   */
  previewPort?: number;
  quartoBin?: string;
...
export async function previewDraft(input: PreviewDraftOptions): Promise<void> {
  ...
  const args = ["preview", relativeFile, "--no-browser", "--no-execute"];
  if (input.previewPort !== undefined) {
    if (
      !Number.isInteger(input.previewPort) ||
      input.previewPort < 1024 ||
      input.previewPort > 65535
    ) {
      throw new DraftPreviewError(
        `"${input.previewPort}" is not a usable preview port; name an integer between 1024 and 65535`,
      );
    }
    args.push("--host", "127.0.0.1", "--port", String(input.previewPort));
  }

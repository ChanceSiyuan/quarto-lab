...
// `@retorquere/bibtex-parser@10.0.0` omits declarations from its package.
// @ts-expect-error -- pinned untyped dependency; every parsed value is checked below.
...
interface RawBibTeXBibliography {
...
) => RawBibTeXBibliography;

const parseRawBibTeX = parseBibTeXUntyped as ParseRawBibTeX;

export interface ZoteroBibliographyMergeReport {
  contents: string;
  added: number;
  updated: number;
  retained: number;
  renamed: Array<{ from: string; to: string }>;
  skipped: Array<{ citekey: string; reason: string }>;
}

const BIBTEX_ENTRY_HEADER = /^(@[A-Za-z]+\s*\{)([^,\r\n]+)(,)/gimu;
const SAFE_CITEKEY = /^[A-Za-z0-9][A-Za-z0-9._:-]*$/u;

function normalizeZoteroCitekeys(input: string): {
  contents: string;
  renamed: Array<{ from: string; to: string }>;
} {
  const headers = [...input.matchAll(BIBTEX_ENTRY_HEADER)];
  const used = new Set(
    headers.map((match) => match[2].trim()).filter((citekey) => SAFE_CITEKEY.test(citekey)),
  );
  const mappings = new Map<string, string>();
  const renamed: Array<{ from: string; to: string }> = [];
  for (const match of headers) {
    const from = match[2].trim();
    if (SAFE_CITEKEY.test(from) || mappings.has(from)) continue;
    const base = from
      .replace(/[^A-Za-z0-9._:-]+/gu, "-")
      .replace(/^[.:-]+|[.:-]+$/gu, "") || "zotero";
    let to = base;
    if (used.has(to)) {
      to = `${base}-${createHash("sha256").update(from).digest("hex").slice(0, 8)}`;
    }
    used.add(to);
  ...
  return {
    contents: input.replace(BIBTEX_ENTRY_HEADER, (header, opening, rawKey, comma) => {
      const citekey = String(rawKey).trim();
      return `${opening}${mappings.get(citekey) ?? citekey}${comma}`;
    }),
    renamed,
  };
}

function rawEntries(input: string, source: string): Map<string, RawBibTeXEntry> {
  const parsed = parseRawBibTeX(input, { sentenceCase: false });
  if (parsed.errors.length > 0) {
    throw new Error(
      `${source} has ${parsed.errors.length} BibTeX parser error(s): ${parsed.errors.map((entry) => entry.error).join("; ")}`,
    );
  }
  const entries = new Map<string, RawBibTeXEntry>();
  for (const entry of parsed.entries) {
    const citekey = String(entry.key ?? "").trim();
    if (entries.has(citekey)) throw new Error(`${source} defines citekey ${JSON.stringify(citekey)} more than once`);
    entries.set(citekey, entry);
  }
  return entries;
}

function methodsOf(entry: RawBibTeXEntry): Set<string> {
...
function withMethods(entry: RawBibTeXEntry, methods: ReadonlySet<string>): string {
  const raw = entry.input
    .trim()
    // Zotero exports occasionally preserve indentation on otherwise blank
    // lines, and some fields end with a space. Keep the generated authority
    // stable and `git diff --check` clean across every refresh.
    .replace(/[\t ]+$/gmu, "")
    // Subject tags are not method slugs, and attachment paths are private
    // machine-local data. The raw snapshot keeps both; the reviewed authority
    // keeps neither.
    .replace(/^[\t ]*(?:keywords|file)[\t ]*=[^\r\n]*(?:\r?\n|$)/gimu, "")
    // These two presentation macros are common in exported titles but are not
    // understood by the pinned parser. Removing the font command preserves the
    // title text while keeping the authority parseable on every refresh.
    .replace(/\\mathbb\s*/gu, "")
    .replace(/\\mathsf\s*/gu, "");
  const close = raw.lastIndexOf("}");
  if (close < 0) throw new Error(`BibTeX entry ${JSON.stringify(entry.key)} has no closing brace`);
  let before = raw.slice(0, close).trimEnd();
  if (!before.endsWith(",")) before += ",";
  return `${before}\n  keywords = {${[...methods].sort().join(", ")}}\n}`;
}

/**
 * A portable Zotero metadata snapshot.
 *
 * Better BibTeX's `file` field is an attachment locator, not citation
 * metadata. It commonly contains absolute home-directory and Zotero storage
 * paths, so it must never be persisted in the tracked snapshot.
 */
export function sanitizeZoteroBibliographySnapshot(incoming: string): string {
...
/**
 * Merges a Zotero export into the reviewed bibliography without propagating
 * Zotero deletion. Existing citekeys absent from the snapshot are retained;
 * matching entries refresh their metadata while keeping their method labels;
 * new entries receive the safe `zotero` label. Every candidate is checked by
 * the same strict parser used by the knowledge gate before it can enter the
 * returned authority.
 */
export function mergeZoteroBibliography(
  current: string,
  incoming: string,
): ZoteroBibliographyMergeReport {
  const existing = rawEntries(current, "literature/ref.bib");
  const normalized = normalizeZoteroCitekeys(incoming);
  const exported = rawEntries(
    normalized.contents.replace(/\\mathbb\s*/gu, "").replace(/\\mathsf\s*/gu, ""),
    "the Zotero BibTeX export",
  );
  const merged = new Map<string, string>(
    [...existing].map(([citekey, entry]) => [citekey, entry.input.trim().replace(/[\t ]+$/gmu, "")]),
  );
  const skipped: ZoteroBibliographyMergeReport["skipped"] = [];
  let added = 0;
  let updated = 0;

  for (const [citekey, entry] of exported) {
    const previous = existing.get(citekey);
    // Zotero's `keywords` are free-form subject tags ("Quantum Physics",
    // "Computer Science - Complexity", ...), not Research Loop method slugs.
    // Preserve reviewed method labels for known entries and give a new Zotero
    // record only the stable provenance label below.
    const methods = previous === undefined ? new Set<string>() : methodsOf(previous);
    methods.add("zotero");
    let candidate: string;
    try {
      candidate = withMethods(entry, methods);
      parseBibliography(`${candidate}\n`);
    } catch (error) {
      skipped.push({
        citekey,
    ...
    const before = merged.get(citekey);
    if (before === undefined) added += 1;
    else if (before !== candidate) updated += 1;
    merged.set(citekey, candidate);
  }
  ...
  const contents = [...merged]
    ...
    .map(([, entry]) => entry)
    .join("\n\n") + (merged.size === 0 ? "" : "\n");
  parseBibliography(contents);
  return {
    contents,
    added,
    updated,
    retained: [...existing.keys()].filter((citekey) => !exported.has(citekey)).length,
...
function roleOf(relative: string, entrypoint?: string): string {
  if (relative === ".raw/paper.pdf") return "primary-pdf";
  if (relative === ".raw/extracted.md") return "derived-text";
  if (relative === "record.yml") return "metadata";
  if (entrypoint !== undefined && relative === entrypoint) return "latex-entrypoint";
  if (relative.startsWith(".raw/source/")) return "latex-source";
  if (relative.startsWith(".raw/figures/")) return "figure";
  return "metadata";
}

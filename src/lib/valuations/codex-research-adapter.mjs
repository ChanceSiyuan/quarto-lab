...
function validateSchemaValue(value, schema, path = "candidate") {
  if (schema.type !== undefined && !schemaTypes(schema.type).some((type) => matchesType(value, type))) {
    return `${path} must match schema type ${JSON.stringify(schema.type)}.`;
  }
...
function normalizeAtomicEvidence(value) {
  ...
  const normalized = {
    id: value.id,
    state: value.state,
    interval: maybeClone(value.interval),
    unit: value.unit,
    visibility: value.visibility,
    evidenceState: value.evidenceState,
    evidenceTier: value.evidenceTier,
    sourceIds: maybeClone(value.sourceIds),
    sources: maybeClone(value.sources),
  };
  for (const field of ["currency", "priceBaseYear", "conversionSourceId", "derivation", "kind"]) {
    if (value[field] !== null && value[field] !== undefined) normalized[field] = maybeClone(value[field]);
  }
  return normalized;
}
...
function normalizeCandidate(candidate) {
  ...
  return {
    ...candidate,
    marketEvidence: Array.isArray(candidate.marketEvidence)
      ? candidate.marketEvidence.map(normalizeAtomicEvidence)
      : candidate.marketEvidence,
    atomicInputs: Array.isArray(candidate.atomicInputs)
      ? candidate.atomicInputs.map(normalizeAtomicEvidence)
      : candidate.atomicInputs,
    materialAssumptions: Array.isArray(candidate.materialAssumptions)
      ? candidate.materialAssumptions.map((assumption) => isRecord(assumption)
        ? { ...assumption, proposedValue: normalizeAtomicEvidence(assumption.proposedValue) }
        : assumption)
      : candidate.materialAssumptions,
  };
}
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", async (code) => {
      ...
      try {
        ...
        const validation = validateCandidate(normalizeCandidate(candidate), problem, quantumScope);
        if (!validation.ok) {

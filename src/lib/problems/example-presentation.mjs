...
import { formatGateLabel } from "./research-presentation.mjs";
...
function formatGate(gate) {
  return [
    { label: "Containment", value: formatGateLabel(gate.containment).toLowerCase() },
    { label: "Public smoke", value: formatGateLabel(gate.publicSmoke).toLowerCase() },
    { label: "Development", value: formatGateLabel(gate.development).toLowerCase() },
  ];
}
...
export function buildAttemptDossier(attempt, exampleManifest) {
  return {
    ...
    evaluationPath: [
      ...
      { label: "Public smoke", value: formatGateLabel(attempt.gate.publicSmoke) },

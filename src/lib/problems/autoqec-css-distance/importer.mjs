...
import {
  ...
  selectCssDistanceInfrastructurePaths,
...
export function assertSafeImportPath(path) {
  if (!/^(?!\/)(?!.*(?:^|\/)\.)(?!.*(?:^|\/)\.\.(?:\/|$))(?!.*\/\/)[A-Za-z0-9_][A-Za-z0-9._@+-]*(?:\/[A-Za-z0-9_][A-Za-z0-9._@+-]*)*$/.test(path)) {
...
export async function importAutoqecCssDistance({
  ...
  try {
    ...
    const verification = await verifyStagedImportTree({ rootDir: tempRoot, expectedAttempts, verifyStagedTree });
    if (!verification.ok) {
...
export async function verifyStagedImportTree({ rootDir, expectedAttempts, verifyStagedTree = null }) {
  const stagedVerifier = isCanonicalImport(expectedAttempts) ? verifyImportedProblemTree : verifyStagedTree;
  return stagedVerifier ? stagedVerifier({ rootDir }) : { ok: true };
}
  ...
  if (Array.isArray(manifest?.files)) {
    ...
    for (const entry of manifest.files) {
      ...
      if (manifestPathSet.has(entry.path)) {
        errors.push(diagnostic(`problems/${id}/${entry.path}`, "files", `Duplicate file path in import-manifest: ${entry.path}`));
      }
      manifestPathSet.add(entry.path);
      manifestPaths.push(entry.path);
    }
    try {
      const actualFiles = await listVerifiedProblemFiles(problemPath, id);
      const actualFileSet = new Set(actualFiles);
      for (const path of actualFiles) {
        if (!manifestPathSet.has(path)) {
          errors.push(diagnostic(`problems/${id}/${path}`, "file", `Unexpected file not listed in import-manifest: ${path}`));
        }
      }
      for (const path of manifestPaths) {
        if (!actualFileSet.has(path)) {
          errors.push(diagnostic(`problems/${id}/${path}`, "file", `Manifest lists missing file: ${path}`));
        }
      }
    } catch (error) {
      errors.push(diagnostic(error.relativePath ?? `problems/${id}`, "file", error.message));
    }
...
async function listVerifiedProblemFiles(problemPath, id, directory = problemPath) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (metadata.isDirectory()) {
      paths.push(...await listVerifiedProblemFiles(problemPath, id, absolutePath));
    } else if (metadata.isFile()) {
      assertSafeImportPath(relativePath);
      paths.push(relativePath);
    } else {
      const error = new Error(`Non-regular file in imported problem tree: ${relativePath}`);
      error.relativePath = `problems/${id}/${relativePath}`;
      throw error;
    }
  }
...
async function freezeSnapshots(stagedProblem, sourceDir, plan) {
  ...
  for (const [commit, range] of snapshots) {
    ...
    const selection = await selectCssDistanceInfrastructurePaths({
      paths: [...entries.keys()],
      readText: async (path) => {
        const entry = entries.get(path);
        if (!entry) throw new Error(`Selected infrastructure path missing from Git tree: ${path}`);
        requireRegularBlob(entry, path);
        return (await readGitBlob(sourceDir, commit, path)).toString("utf8");
      },
    });
    const files = [];

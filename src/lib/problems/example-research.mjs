import exampleProblem from "../../../.research-loop/fixtures/showcase/problems/Prob-000/problem.json" with { type: "json" };
import exampleManifest from "../../../.research-loop/fixtures/showcase/problems/Prob-000/example.json" with { type: "json" };
import attempt001 from "../../../.research-loop/fixtures/showcase/problems/Prob-000/attempts/ATT-001/attempt.json" with { type: "json" };
import attempt002 from "../../../.research-loop/fixtures/showcase/problems/Prob-000/attempts/ATT-002/attempt.json" with { type: "json" };
import attempt003 from "../../../.research-loop/fixtures/showcase/problems/Prob-000/attempts/ATT-003/attempt.json" with { type: "json" };
import attempt004 from "../../../.research-loop/fixtures/showcase/problems/Prob-000/attempts/ATT-004/attempt.json" with { type: "json" };
import attempt005 from "../../../.research-loop/fixtures/showcase/problems/Prob-000/attempts/ATT-005/attempt.json" with { type: "json" };

export const EXAMPLE_RESEARCH_PROBLEM_ID = "Prob-000";
...
function validateProblem(problem) {
  assertCondition(isPlainObject(problem), "problem must be an object");
  assertCondition(problem.schemaVersion === 1, "problem schemaVersion must be 1");
  assertCondition(problem.id === EXAMPLE_RESEARCH_PROBLEM_ID, "problem id is invalid");
  assertNonEmptyString(problem.title, "problem title is invalid");
  assertNonEmptyString(problem.summary, "problem summary is invalid");
  assertCondition(problem.status === "solving", "problem status is invalid");
  assertCondition(isPlainObject(problem.gate), "problem gate must be an object");
  assertNonEmptyString(problem.gate.type, "problem gate type is invalid");
  assertNonEmptyString(problem.gate.readiness, "problem gate readiness is invalid");
  assertCondition(isPlainObject(problem.provenance), "problem provenance must be an object");
  assertCount(problem.provenance.sourceCount, "problem provenance sourceCount is invalid");
  assertCondition(isPlainObject(problem.lastActivity), "problem lastActivity must be an object");
  assertNonEmptyString(problem.lastActivity.summary, "problem lastActivity summary is invalid");
  assertTimestamp(problem.lastActivity.at, "problem lastActivity timestamp is invalid");
  assertTimestamp(problem.createdAt, "problem createdAt is invalid");
  assertTimestamp(problem.updatedAt, "problem updatedAt is invalid");
}
...
validateProblem(exampleProblem);
...
export function getStaticResearchExampleProblem(problemId) {
  if (!isStaticResearchExampleProblem(problemId)) {
    return null;
  }
  return clone(exampleProblem);
}
...
export function getStaticResearchArtifactPath(problemId, attemptId, artifact) {
  return `.research-loop/fixtures/showcase/problems/${problemId}/attempts/${attemptId}/${artifact}`;
}

...
function validateObject(value, location, fields, errors) {
  ...
  for (const key of fields) if (!Object.hasOwn(value, key)) errors.push(`${location}.${key} is required`);
  return true;
}
...
export function validateInfrastructureManifest(value, context = {}) {
  const errors = [];
  const entrypoints = [];
  if (!validateObject(value, "manifest", ["schemaVersion", "kind", "problemId", "id", "status", "candidate", "objective", "commands", "datasets", "runtime", "resources", "files", "createdAt"], errors)) throw new PreparationContractError(errors);
  ...
  if (validateObject(value.runtime, "runtime", ["image"], errors)) {
    if (typeof value.runtime.image !== "string" || !OCI_IMAGE.test(value.runtime.image)) {
      errors.push("runtime.image must be an immutable OCI image digest");
    }
  }

...
function rejectSymlinkComponents(value) {
  const absolute = resolve(value);
  if (existsSync(absolute) && lstatSync(absolute).isSymbolicLink()) throw new RangeError(`Configured root crosses a symlink: ${absolute}`);
}
...
export function createAutoresearchPaths(rootDir) {
  rejectSymlinkComponents(rootDir);

...
import { runOciProcess } from "./oci-process.mjs";
...
async function runCommand({ id, stageDir, manifest, privateDataRoot, processRunner, env, signal }) {
  ...
  const options = {
    ...
    env, timeoutMs: manifest.resources.attemptTimeoutSeconds * 1000,
    graceMs: manifest.resources.terminationGraceSeconds * 1000,
    signal,
  };
...
export async function runInfrastructurePreflight({ stageDir, manifest, privateDataRoot, processRunner = runOciProcess, now = () => new Date(), environment = process.env, signal }) {

export const PLUGIN_ID = "qlab-zotero@quarto-lab.local";
...
export async function findExecutable(name: "codex"): Promise<string | null> {
...
export async function findLoginShell(): Promise<string | null> {
  const inherited = (() => {
    try { return Services.env.get("SHELL") || ""; }
    catch { return ""; }
  })();
  const candidates = [
    inherited,

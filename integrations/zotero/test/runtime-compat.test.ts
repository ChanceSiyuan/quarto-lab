...
describe("Zotero 9 runtime compatibility", () => {
  it("uses a standalone QLab add-on identity and Zotero 9 namespace", () => {
    ...
    expect(manifest.applications.zotero.id).toBe("qlab-zotero@quarto-lab.local");
    expect(manifest.applications.zotero.strict_min_version).toBe("9.0");
    expect(platform).toContain('PLUGIN_ID = "qlab-zotero@quarto-lab.local"');
  ...
  it("exposes only the local Codex runtime", () => {
    ...
    expect(projectFile("src/terminal-panel.ts")).toContain('[["shell", "Terminal"], ["codex", "Codex"]]');
    expect(prefs).toContain('extensions.zotkit.backend", "codex"');
    expect(prefs).not.toContain("sshProfiles");
    expect(prefs).not.toContain("providers");
  });
  ...
  it("uses Gecko IOUtils.write and keeps QLab out of Zotero's item sidebar", () => {
    ...
    expect(startup).not.toContain("this.registerSection()");
    expect(plugin).toContain("new WorkbenchTabManager");
    expect(projectFile("src/workbench-tab.ts")).toContain('QLAB_WORKBENCH_TAB_TYPE = "qlab"');

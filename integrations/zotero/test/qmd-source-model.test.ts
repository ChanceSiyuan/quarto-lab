...
describe("source-driven QMD visual blocks", () => {
  it("keeps theorem cards whole while exposing their nested formulas", () => {
    const theorem = visualQmdBlocks(SOURCE).find((block) => block.kind === "theorem")!;

    expect(theorem).toMatchObject({ semantic: "thm", title: "A theorem title" });
    expect(theorem.source).toContain("The theorem prose is editable.");
    expect(qmdMathSpans(theorem.source)).toEqual([
      expect.objectContaining({ latex: "\nx^2 \\ge 0\n", display: true, opening: "$$" }),
    ]);
  });

  it("edits a source-backed visual region without flattening the rest of the QMD", () => {
    const blocks = visualQmdBlocks(SOURCE);
    const heading = blocks.find((block) => block.kind === "heading")!;
    const result = applyQmdVisualBlock(SOURCE, heading, "# Revised heading");

    expect(result.source).toContain("# Revised heading");
    expect(result.source).toContain("::: {#thm-example}");
    expect(result.source).toContain("x^2 \\ge 0");
  });

  it("falls back to raw source for unknown syntax and never overlaps visual regions", () => {
    const source = `${SOURCE}\n<div data-example="true">raw</div>\n`;
    const blocks = visualQmdBlocks(source);
    expect(blocks.some((block) => block.kind === "raw" && block.source.includes("<div"))).toBe(true);

...
describe("ChatGPT Companion orchestration", () => {
  const reader = () => ({
    schemaVersion: 1,
    ...
    pdfPath: "/private/frozen.pdf",
    page: {
      pageIndex: 4,
      pageNumber: 5,
      pageLabel: "v",
      pageCount: 20,
      text: "Frozen page text",
      source: "pdfjs",
      warnings: [],
    },
    selection: {
  ...
  it("maintains a bounded active Draft snapshot through the existing QMD read and save seams", async () => {
    const originalIOUtils = (globalThis as any).IOUtils;
    let source = "x".repeat(20_010);
    (globalThis as any).IOUtils = {
      readUTF8: vi.fn(async () => source),
      writeUTF8: vi.fn(async (_path: string, next: string) => { source = next; }),
    };
    const plugin = new ZoteroChatPlugin() as any;
    plugin.activeDraftPath = "drafts/active.qmd";
    plugin.safeRepositoryPath = vi.fn(() => "/repo/drafts/active.qmd");
    plugin.hashQmdSource = (value: string) => `revision:${value.length}:${value.at(0) || ""}`;

    try {
      const opened = await plugin.readQmdSource("drafts/active.qmd");
      expect(opened.source).toHaveLength(20_010);
      expect(plugin.activeDraftSnapshot).toEqual({
        relativePath: "drafts/active.qmd",
        revision: "revision:20010:x",
        source: "x".repeat(20_000),
      });

      await plugin.saveQmdSource("drafts/active.qmd", opened.revision, "saved body");
      expect(plugin.activeDraftSnapshot).toEqual({
        relativePath: "drafts/active.qmd",
        revision: "revision:10:s",
        source: "saved body",
      });
    }
    finally {
  ...
  it("freezes the exact effective context before Draft revalidation and hands off in save-copy-launch order", async () => {
    const order: string[] = [];
    const saved: any[] = [];
    const reread = deferred<{ source: string; revision: string }>();
    const runtime = {
      store: {
        save: vi.fn(async (capsule: unknown) => { order.push("save"); saved.push(capsule); }),
        load: vi.fn(), delete: vi.fn(), pruneExpired: vi.fn(),
      },
      copy: vi.fn(() => { order.push("copy"); return true; }),
      readClipboard: vi.fn(() => null),
      launch: vi.fn(() => { order.push("launch"); }),
      now: () => new Date("2026-08-01T10:00:00.000Z"),
      id: () => "capsule-frozen-12345678",
      hash: () => "a".repeat(64),
      revalidateDraft: vi.fn(() => reread.promise),
    };
    const plugin = new ZoteroChatPlugin(runtime) as any;
    ...
    plugin.codex = {
      state: { activeThreadId: "thread-frozen" },
      getActiveReaderContext: () => plugin.context,
    };
    plugin.addedContextIDs.add("current-selection");
    plugin.addedContextIDs.add("active-annotations");
    plugin.addedContextIDs.add("zotero-library");
    plugin.conversationPapers.add("thread-frozen", {
      id: "SECONDARYOLD",
      libraryID: "1",
      attachmentKey: "SECONDARYATTACH",
      title: "Frozen secondary",
      pdfPath: "/private/secondary.pdf",
      textPath: "/private/secondary.txt",
      sourceUrls: ["https://example.test/secondary"],
      dois: ["10.1000/secondary"],
      mode: "full",
    });
    plugin.pendingScreenshots = [{
      ...
      source: {
        paperKey: "PAPEROLD",
        paperTitle: "Frozen paper",
        pageIndex: 4,
        pageNumber: 5,
        pageLabel: "v",
      },
    }];
    plugin.activeDraftPath = "drafts/frozen.qmd";
    plugin.activeDraftSnapshot = {
      relativePath: "drafts/frozen.qmd",
      revision: "revision-frozen",
      source: "Frozen Draft body",
    };

    const handoff = plugin.openChatGPTCompanion("  Frozen question?  ");
    expect(runtime.revalidateDraft).toHaveBeenCalledWith("drafts/frozen.qmd");
    expect(order).toEqual([]);

    plugin.context.parent.title = "Later paper";
    plugin.context.page.text = "Later page text";
    plugin.context.selection.text = "Later selection";
    plugin.excludedContextIDs.add("active-paper");
    plugin.conversationPapers.remove("thread-frozen", "SECONDARYOLD");
    plugin.pendingScreenshots[0].image = "data:image/png;base64,LATER";
    plugin.activeDraftSnapshot = {
      relativePath: "drafts/later.qmd",
      revision: "later-revision",
      source: "Later Draft body",
    };
    reread.resolve({ source: "Content reread only for revision", revision: "revision-frozen" });
    await handoff;

    expect(order).toEqual(["save", "copy", "launch"]);
    const capsule = saved[0];
    expect(capsule.question).toBe("  Frozen question?  ");
    expect(capsule.contextItems.map((item: { id: string }) => item.id)).toEqual([
      "active-paper",
      "current-page",
      "current-selection",
      "active-annotations",
    ...
    expect(capsule.paper).toMatchObject({ title: "Frozen paper", creators: "Ada Lovelace" });
    expect(capsule.page.excerpt).toBe("Frozen page text");
    expect(capsule.selection.text).toBe("Frozen selection");
    expect(capsule.secondaryPapers).toEqual([expect.objectContaining({
      title: "Frozen secondary",
      mode: "full",
    })]);
    expect(capsule.draft.excerpt).toBe("Frozen Draft body");
    expect(capsule.screenshotProvenance).toEqual([expect.objectContaining({
      kind: "region",
      paperTitle: "Frozen paper",
      pageNumber: 5,
    })]);
    expect(JSON.stringify(capsule)).not.toMatch(/SECRET|LATER|pdfPath|textPath|private\//);
    expect(plugin.companionStatusForSubject("draft:drafts/frozen.qmd")).toEqual({
      kind: "success",
      message: "Context copied · paste in a ChatGPT chat with the QLab app enabled",
    });
  });

  it("omits a changed Draft, and copy failure never launches or creates a pending handoff", async () => {
    const saved: any[] = [];
    const runtime = {
      store: {
        save: vi.fn(async (capsule: unknown) => { saved.push(capsule); }),
        load: vi.fn(), delete: vi.fn(), pruneExpired: vi.fn(),
      },
      copy: vi.fn(() => false),
      readClipboard: vi.fn(() => null),
      launch: vi.fn(),
      now: () => new Date("2026-08-01T10:00:00.000Z"),
      id: () => "capsule-changed-1234567",
      hash: () => "b".repeat(64),
      revalidateDraft: vi.fn(async () => ({ source: "Later body", revision: "later-revision" })),
    };
    const plugin = new ZoteroChatPlugin(runtime) as any;
    plugin.codex = { state: { activeThreadId: null }, getActiveReaderContext: () => null };
    plugin.activeDraftPath = "drafts/private-name.qmd";
    plugin.activeDraftSnapshot = {
      relativePath: "drafts/private-name.qmd",
      revision: "click-revision",
      source: "Click-time body",
    };

    await plugin.openChatGPTCompanion("Question without Codex?");

    expect(saved[0].draft).toBeNull();
    expect(saved[0].contextItems[0]).toMatchObject({ kind: "draft", included: false });
    expect(saved[0].contextItems[0].warning).toMatch(/unavailable/i);
    expect(runtime.launch).not.toHaveBeenCalled();
    expect(plugin.pendingCompanionForSubject("draft:drafts/private-name.qmd")).toEqual([]);
    expect(plugin.companionStatusForSubject("draft:drafts/private-name.qmd")).toMatchObject({ kind: "error" });
    expect(plugin.companionStatusForSubject("draft:drafts/private-name.qmd").message)
      .not.toContain("private-name.qmd");
  });

  it("keeps question-only handoffs independently selectable and imports only the selected capsule", async () => {
    const capsules = new Map<string, any>();
    const ids = ["capsule-older-12345678", "capsule-newer-12345678"];
    const runtime = {
      store: {
        save: vi.fn(async (capsule: any) => { capsules.set(capsule.id, capsule); }),
        load: vi.fn(async (id: string) => capsules.get(id) || null),
        delete: vi.fn(), pruneExpired: vi.fn(),
      },
      copy: vi.fn(() => true),
      readClipboard: vi.fn(() => "Copied ChatGPT answer"),
      launch: vi.fn(),
      now: vi.fn()
        .mockReturnValueOnce(new Date("2026-08-01T10:00:00.000Z"))
        .mockReturnValueOnce(new Date("2026-08-01T11:00:00.000Z")),
      id: vi.fn(() => ids.shift()!),
      hash: () => "c".repeat(64),
      revalidateDraft: vi.fn(),
    };
    const plugin = new ZoteroChatPlugin(runtime) as any;
    const codexEntries = [{ id: "codex-live", kind: "assistant", text: "Codex entry" }];
    plugin.codex = {
      state: { activeThreadId: null },
      getActiveReaderContext: () => null,
      getChatEntries: () => codexEntries,
    };

    await plugin.openChatGPTCompanion("Older question");
    await plugin.openChatGPTCompanion("Newer question");
    expect(runtime.readClipboard).not.toHaveBeenCalled();
    expect(plugin.pendingCompanionForSubject("question-only")).toEqual([
      expect.objectContaining({ capsuleId: "capsule-older-12345678", selected: false }),
      expect.objectContaining({ capsuleId: "capsule-newer-12345678", selected: true }),
    ]);

    plugin.selectChatGPTCompanionHandoff("capsule-older-12345678");
    await plugin.importChatGPTCompanionAnswer();

    expect(runtime.readClipboard).toHaveBeenCalledOnce();
    expect(plugin.pendingCompanionForSubject("question-only")).toEqual([
      expect.objectContaining({ capsuleId: "capsule-newer-12345678", selected: true }),
    ]);
    const merged = plugin.entriesWithCompanionOverlay(codexEntries, "question-only", false);
    expect(merged).toHaveLength(3);
    expect(merged[0]).toBe(codexEntries[0]);
    expect(merged[2]).toMatchObject({
      kind: "assistant",
      origin: "chatgpt-companion",
      originLabel: "Imported from ChatGPT · user copied",
      companionProvenance: {
        capsuleId: "capsule-older-12345678",
        capsuleChecksum: "c".repeat(64),
      },
    });
    expect(plugin.entriesWithCompanionOverlay(codexEntries, "paper:OTHER", false)).toBe(codexEntries);
    expect(plugin.entriesWithCompanionOverlay(codexEntries, "question-only", true)).toBe(codexEntries);
    expect(codexEntries).toEqual([{ id: "codex-live", kind: "assistant", text: "Codex entry" }]);
  });

  it("retains copied handoffs when launch fails and bounds each subject to the newest eight", async () => {
    let nextID = 0;
    const runtime = {
      store: {
        save: vi.fn(async () => undefined),
      ...
      copy: vi.fn(() => true),
      readClipboard: vi.fn(() => null),
      launch: vi.fn(() => { throw new Error("launcher unavailable"); }),
      now: () => new Date("2026-08-01T10:00:00.000Z"),
      id: () => `capsule-bounded-${String(++nextID).padStart(8, "0")}`,
      hash: () => "e".repeat(64),
    };
    const plugin = new ZoteroChatPlugin(runtime) as any;
    plugin.codex = { state: { activeThreadId: null }, getActiveReaderContext: () => null };
    ...
    for (let index = 1; index <= 9; index += 1) {
      await plugin.openChatGPTCompanion(`Question ${index}`);
    }

    expect(plugin.pendingCompanionForSubject("question-only")).toHaveLength(8);
    expect(plugin.pendingCompanionForSubject("question-only")[0].capsuleId)
      .toBe("capsule-bounded-00000002");
    expect(plugin.pendingCompanionForSubject("question-only").at(-1)).toMatchObject({
      capsuleId: "capsule-bounded-00000009",
      selected: true,
    });
    expect(plugin.companionStatusForSubject("question-only")).toMatchObject({
      kind: "success",
      message: expect.stringMatching(/Context copied/),
    });
    expect(runtime.launch).toHaveBeenCalledTimes(9);
  });
});

...
`);
  ...
  it("accepts ANSI-coloured terminal bullet prefixes without treating citations as math", () => {
    const codex = "\u001b[36m•\u001b[0m [\nP_e\\sim(\\Omega/\\Delta)^2\n]";
    const alternate = "\u001b[35m⏺\u001b[0m [\n\\Gamma_{\\rm sc}=\\Gamma_e P_e\n]";

    expect(hasMathPreviewCandidate(codex)).toBe(true);
    expect(hasMathPreviewCandidate(alternate)).toBe(true);

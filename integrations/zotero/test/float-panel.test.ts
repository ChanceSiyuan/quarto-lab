...
describe("FloatPanelView shell", () => {
  ...
  it("renders a compact paper title in the drag bar and removes everything on destroy", () => {
    ...
    expect(host.querySelector(".zc-float-title")?.textContent).toBe("QLab · Attention Is All You Need");
    view.destroy();
    expect(host.querySelector(".zc-float")).toBeNull();
  });

  it("stays lightweight and opens the full workbench in a Zotero tab", () => {
    const { host, handlers } = mount();
    expect(host.querySelector(".zc-workbench-tools")).toBeNull();
    expect(host.querySelectorAll(".zc-workbench-command")).toHaveLength(0);
    host.querySelector<HTMLButtonElement>(".zc-float-open-workbench")!.click();
    expect(handlers.onOpenWorkbench).toHaveBeenCalledOnce();
  });
});
...
describe("full workbench surface", () => {
  it("shows repository controls and six commands without a redundant chat-capture button", () => {
    ...
    const host = document.createElement("div");
    document.body.appendChild(host);
    const view = new FloatPanelView(host, handlers, { surface: "workbench" });
    view.setState({ phase: "ready", qlabRoot: "/Users/research/qlab" });
    view.show();

    const commands = [...host.querySelectorAll<HTMLButtonElement>(".zc-workbench-command")];
    expect(host.querySelector(".zc-workbench-tab-content")).not.toBeNull();
    expect(host.querySelector(".zc-float-open-workbench")).toBeNull();
    expect(commands).toHaveLength(6);
    expect(commands.every((button) => !button.disabled)).toBe(true);
    commands.find((button) => button.dataset.commandId === "qlab_preview")!.click();

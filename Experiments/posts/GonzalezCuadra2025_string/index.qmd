...
5. **Quench dynamics ($d = 2$):** **31-atom** array; sudden quench from string state to deconfined regime; real-time evolution up to $\sim 0.8\,\mu$s observed.
...
- **Many-body resonance:** String breaking exhibits a resonance phenomenon at $2\Delta = V_1$ (nearest-neighbor interaction), where the detuning cost of creating a pair matches the interaction energy gained.
...
| **Verification value** | Validates theoretical predictions for string breaking in (2+1)D LGT: the resonance condition $2\delta_0 = V_1$ (experiment peak $\delta_0/\Omega = 1.4(1)$ vs. theory 1.0 for $d=2$; experiment $1.0(1)$ vs. theory 0.7 for $d=3$), and the unequal string-path probabilities due to long-range interactions. These predictions had only been studied via small-scale exact diagonalization. |


## Architecture comparison: relative to 1-zone/3-zone/4-zone review benchmarks

| Parameter | González-Cuadra 2025 (Aquila) | SOTA 1-zone (Manovitz 2025) | Notes |
|-----------|-------------------------------|---------------------------|-------|
| Atom count | 31–59 (kagome, 3 configurations) | ~200–300 (square) | Much smaller; limited by local-detuning complexity |
| Blockade radius | $R_b = 8.4\,\mu$m ($70S_{1/2}$) | Similar ($C_6$ tunable) | Same Rb platform |
| Lattice constant | $a \approx 4.7$–$7\,\mu$m ($R_b/a \in [1.2, 1.8]$) | ~5 µm | Tuned to scan phase diagram |
| Readout fidelity | 0.95 (Rydberg) / 0.99 (ground) | ~0.99 | Rydberg-state readout lower |
| Evolution time | $\lesssim 0.8\,\mu$s (quench); $2.5\,\mu$s (adiabatic) | Comparable | Decoherence from local detuning beams |
| Local addressing | Yes — local detuning beams for charge creation | Global only | Required for this experiment; triggers AC Stark decoherence |
...
| Quantity | Value |
|----------|-------|
| Kagome patch (phase diagram) | 59 atoms |
| Kagome patch ($d=3$ resonance) | 39 atoms |
| Kagome patch ($d=2$ dynamics) | 31 atoms |
...
| Adiabatic prep time | 2.5 µs |
| Quench evolution (observed) | up to ~0.8 µs |

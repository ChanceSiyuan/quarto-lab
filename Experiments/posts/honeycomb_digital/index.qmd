...
| **CZ fidelity** | ~99.4% (global RB) | 99.5% | Slight regression — each Floquet layer compounds this |
...
| **Intermediate-state detuning** | 3.3 GHz | Standard | Enables closer atom spacing in the entangling zone (~2.5 μm) |
...
| Fermion quench dynamics (density-density) | Depth-11 |
| Fermion quench dynamics (full) | Depth-12 |

**Chern number evidence structure:** The paper uses a bootstrapped batch analysis rather than quoting a single $C$ value with an error bar. Extended Data Fig. 6 shows that the inferred Chern number approaches $C > 0.9$ for batch sizes above ~200 shots, quickly converging to $C = 1$ as the batch grows. The result confirms an **odd** Chern number, ruling out the trivial phase.
...
**Fermi-Hubbard $M_\text{stagg}$:** The simulation uses **3 Floquet rounds** (depth-12) with $U/2J$ varied from 0 to $\sim 5$. The staggered magnetization $M_\text{stagg}$ shows **non-monotonic behavior**: it decays rapidly at $U=0$ (free fermions spread out), is suppressed at intermediate $U$, and is partially recovered at large $U \gg J$ where double-occupancy is energetically suppressed. This non-monotonic dependence on $U$ is the key signature of Hubbard physics.

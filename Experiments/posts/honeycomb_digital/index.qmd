...
| **CZ fidelity** | ~99.5% (carried from Bluvstein 2024; Evered 2025 states performance is "comparable to prior work") | 99.5% | No significant regression; each Floquet layer still compounds this |
...
| **Circuit depth achieved** | Depth-12 (fermion dynamics) | Depth ~50–66 (logical) | Physical circuit depth here limited by readout degradation, not gate count |
| **Hyperfine $T_2$** | >1 s (clock states) | ~1 s | Coherence is not the limiting factor; readout degradation dominates |
| **Intermediate-state detuning** | 3.3 GHz | Standard | Enables closer atom spacing in the entangling zone |
...
**Gate fidelity accumulation is secondary.** The deepest circuit in the experiment is depth-12 (fermion quench dynamics), operating on 72 data qubits arranged so each Floquet step involves $O(72)$ parallel 2-qubit gates, not all qubits simultaneously. Evered 2025 reports no new CZ benchmark — it states performance is "comparable to prior work." The CZ fidelity of ~99.5% is carried from Bluvstein 2024 [@bluvstein2024logical]. With this value and maximum depth-12:

$$
(0.995)^{12} \approx 94\%
$$

per qubit per Floquet layer — substantially better than the readout budget. Gate fidelity is not the limiting factor at current depths.

**Path to improvement:** Stabilizing the trap laser power over the experimental run would recover ~3–4% per-shot readout fidelity and could increase exchange measurement acceptance fractions by $\sim 5\times$. Gate fidelity improvements (99.4% → 99.9%) would become the limiting factor only once readout is fixed.
...
| Fermi-Hubbard dynamics | Depth-12 |
...
**Fermi-Hubbard $M_\text{stagg}$:** The simulation encodes spin-up and spin-down on a **4×4 square lattice (16 sites**, split into two 8-site halves). The circuit uses depth-12 Floquet rounds with the dimensionless interaction ratio $J_Z/J$ varied up to 8 (with explicit benchmarks at $J_Z/J = 1$ and $J_Z/J = 8$). The staggered magnetization $M_\text{stagg}$ shows **non-monotonic behavior**: it decays rapidly at $J_Z/J \to 0$ (free fermions spread out), is suppressed at intermediate coupling, and is partially recovered at large $J_Z/J \gg 1$ where double-occupancy is energetically suppressed. This non-monotonic dependence on the interaction strength is the key signature of Hubbard physics.

<!-- TODO: Add the proof for the reconstruction lemma (currently marked as todo in the lemma block above) -->
**Platform comparison — circuit depth for equivalent experiments:**

| Platform | Gate fidelity (2Q) | Connectivity | Equivalent circuit depth | Limiting factor |
|----------|--------------------|--------------|--------------------------|-----------------|
| **Neutral atom (this work)** | ~99.5% (Bluvstein 2024) | Reconfigurable, all-to-all within zone | Depth-12 Floquet | Readout degradation (~96–97% in-run) |
| **Trapped ion** | ~99.5–99.9% (Quantinuum H-series) | All-to-all (small register) | ~comparable depth; slower gate clock (~1 ms/gate) | Gate speed; scaling to 72+ qubits is active work |
| **Superconducting** | ~99–99.7% (heavy-hex or grid) | Fixed nearest-neighbor | Depth increases $\sim 3\times$ due to SWAP overhead for non-local gates | Connectivity-limited routing; $T_1/T_2 \lesssim 500\,\mu$s limits long circuits |

Neutral atoms have a structural advantage for the honeycomb geometry: mid-circuit atom rearrangement provides native non-local connectivity, eliminating the SWAP overhead that would add $O(N)$ depth on a fixed grid. The relevant trapped-ion disadvantage is gate clock speed, not fidelity; the relevant superconducting disadvantage is connectivity, not coherence time.

...
By definition (Sec.~\ref{sec:tensor}), a local strategy uses product POVMs $\Pi_{\vec{j}} = \bigotimes_l \Pi_{j_l}^{(l)}$ and factored estimators $h_\alpha(\vec{j}) = \prod_l h_{\alpha_l}^{(l)}(j_l)$. The product structure forces a per-site decomposition: the constraint $\sum_{\vec{j}} h_\alpha(\vec{j})\,\Pi_{\vec{j}} = F_\alpha = \bigotimes_l F_{\alpha_l}^{(l)}$ with product POVM and product estimators factorizes into independent per-site constraints $\sum_{j_l} h_{\alpha_l}^{(l)}(j_l)\,\Pi_{j_l}^{(l)} = F_{\alpha_l}^{(l)}$ for each site $l$. Similarly, $\sum_{\vec{j}} \Pi_{\vec{j}} = \bigotimes_l s_l\,\1$ gives $s = \prod_l s_l$. Since each site must independently achieve its f-compatibility constraint, $s_l \geq s^*_l$ for every $l$, and therefore $s^*_{\mathrm{loc}} = \prod_l s_l \geq \prod_l s^*_l$.

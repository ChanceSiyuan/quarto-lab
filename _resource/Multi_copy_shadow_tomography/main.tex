...
What determines when multi-copy strategies help? For \emph{nonlinear} functionals such as $\tr(P\rho^2)$, the question is particularly rich: two-copy local 3-design strategies already incur exponential second moment $\sim (11/3)^n$--$5^n$ depending on the state (Appendix~\ref{app:local_shadow}), yet an explicit nonlocal four-copy circuit achieves $\mathcal{O}(1)$~\cite{ekert2002direct}. This locality gap calls for a unified framework that simultaneously captures:
...
the shared eigenbasis of $\mathcal{A}$ have the form $\ket{\Psi^{s}_{(\mathbf{a}, \mathbf{b})}}$ below with eigenvalues $\lambda_{(s,\mathbf{a},\mathbf{b})} :=(-1)^{\mathbf{r}_x\cdot\mathbf{r}_z}\cdot s \cdot 2^{-1}[(-1)^{[\mathbf{r}, \mathbf{a}]} + (-1)^{[\mathbf{r}, \mathbf{b}]}] $ and $s = \pm 1$.
\begin{align}

...
Finally, we establish a local no-go: the relevant lifted operators have non-product structure, so local strategies face exponential cost for $\tr(P\rho^2)$ at any copy number.
\end{abstract}
\date{\today}
\maketitle
...
What determines when multi-copy strategies help? For \emph{nonlinear} functionals such as $\tr(P\rho^2)$, the question is particularly rich: two-copy local strategies already incur $\sim (11/3)^n$ cost (Appendix~\ref{app:local_shadow}), yet an explicit nonlocal four-copy circuit achieves $\mathcal{O}(1)$~\cite{ekert2002direct}. This locality gap calls for a unified framework that simultaneously captures:
...
The \emph{f-compatibility robustness} $s^*(\cF)$ is the minimum super-normalization $s = \sum_j \|\Pi_j\|/\dim(\cH^{\ox k})$ (i.e., $\sum_j \Pi_j = s\cdot\1^{\ox k}$) such that the above conditions are feasible. This optimal $s^*$ is computed by the semidefinite program (SDP):
...
\end{align}
where $s^*_l$ is the single-site robustness from the SDP~\eqref{eq:f-compat-SDP} restricted to site $l$.
\end{theorem}

This reduces the $n$-qubit problem to $n$ independent single-qubit SDPs of dimension $2^k$. The sample complexity~\eqref{eq:copy-complexity} with local measurements becomes $N = \mathcal{O}\!\left((\prod_l s^*_l)^2\varepsilon^{-2}\log(|\cA|/\delta)\right)$.

\emph{Recovering $3^w$.}---For single-copy ($k=1$) linear Pauli estimation $f_i(\rho) = \tr(P_i\rho)$ with $P_i = \bigotimes_l P_{i_l}$, the linearization is $F_i = P_i$ itself. At each non-identity site, the single-qubit SDP asks for joint measurability of unsharp Paulis $\{\frac{\1 + \eta P}{2}\}_{P\in\{X,Y,Z\}}$, which requires $\eta \leq 1/\sqrt{3}$~\cite{Busch1986,heinosaari2015noise}. This gives $s^*_l = \sqrt{3}$ per non-identity site, yielding $s^*_{\mathrm{loc}} = (\sqrt{3})^w$ and variance $\sim 3^w$ for weight-$w$ Paulis, matching the classical shadow result~\cite{huang2020predicting}.
...
Let $\bP^d_\pi$ be the permutation operator on $2k$ replicas with $\pi = (\mathrm{odds})(\mathrm{even})$ acting as $k$-cycles on odd and even labels respectively. Define
...
\item Section~C: Multi-copy local estimation of fixed $\tr(P\rho^2)$
\item Section~D: Multi-copy local shadows (3-design variance, covariance operators)
\item Section~E: Nonlocal 4-replica strategy (commuting family, circuit construction)
\item Section~F: Lower bounds for nonlinear Pauli shadow
\item Section~G: Algebraic framework for multi-copy estimation (tensor algebra, Fisher information, AJD, SDP relaxation)
\end{itemize}
...
\begin{definition}[Linearization of rank-k polynomials]\label{def:linearization}
    Given a set of degree-$r$ non-linear functions $\{f_\alpha(\rho)\}_{\alpha \in \mathcal{A}}$, its $l > k$-linearization is defined as a set of Hermitian operators $\{F_\alpha\}_{\alpha \in \mathcal{A}}$ such that $\mathrm{tr}(F_{\alpha} \rho^k) = f_{\alpha}(\rho)$.
\end{definition}
A $l>r$ linearization of an rank-$r$ function of $\rho$ can always be done since its components satisfied $\left[\mathrm{tr}(O \rho^{m})\right]^{p} = \mathrm{tr}([(O\otimes \mathbb{I}^{m-1})\cdot S_{m}]^{\otimes p} \cdot \rho^{\otimes m p})$.
...
Any local solution $\{\Pi_{\vec{j}} = \bigotimes_l \Pi_{j_l}^{(l)}\}$ with $\sum_{\vec{j}} \Pi_{\vec{j}} = s\,\1$ implies $\sum_{j_l} \Pi_{j_l}^{(l)} = s_l\,\1$ at each site with $s = \prod_l s_l$. The factored constraint $\sum_{\vec{j}} h_\alpha(\vec{j})\Pi_{\vec{j}} = F_\alpha$ requires $\sum_{j_l} h_{\alpha_l}^{(l)}(j_l)\Pi_{j_l}^{(l)} = F_{\alpha_l}^{(l)}$ at each site (by uniqueness of the product decomposition when $F_\alpha$ is non-degenerate). Therefore $s_l \geq s^*_l$ for each $l$, giving $s \geq \prod_l s^*_l$.
...
\end{align}
where $[P,Q]^{(2)}$ denotes the symmetrized lift of the commutator. This is an \emph{isomorphic copy} of the original Pauli Lie algebra (spin-$2^{n-1}$ representation on the symmetric subspace).

\emph{Why tensor squaring of $\tilde{O}_P$ fails.}
The squaring trick~\eqref{eq:squaring} succeeds for Paulis because they satisfy strict anticommutation: $\{P,Q\} = 0 \Rightarrow (PQ)^{\ox 2} = (QP)^{\ox 2}$. However, $\tilde{O}_P$ has a nonzero anticommutator:
\begin{align}
...
\begin{lemma}[Variance of two-copy Pauli]\label{lemma:covar_Pauli}
    ...
    \begin{align}
        ...
        \end{aligned}\right.
    \end{align}
    Specifically, $\mathbf{V}(O_{\mathbf{r}},O_{\mathbf{r}}) =  \left({11}/{3}\right)^{n}\cdot \left({5}/{11}\right)^{\text{wt}(\sigma_{\mathbf{r}})} \sigma_{\mathbf{r}} \otimes \sigma_{\mathbf{r}}.$
\end{lemma}
\begin{proof}
    ...
    \begin{align}
        ...
        \end{aligned}\right.
    \end{align}
    Because the input states are restrained in the symmetric space $\rho \otimes \rho$, we can safely symmetrize this operator and drop the terms that contained the antisymmetric operator and get the final result
    \begin{align}
        v(\mathbf{r}_j, \mathbf{r}_j') = 
        \left\{\begin{aligned}
            &\frac{5}{6}\left(\sigma_{\mathbf{r}_j}\otimes \sigma_{\mathbf{r}'_j} +\sigma_{\mathbf{r}'_j}\otimes \sigma_{\mathbf{r}_j} \right)\text{~if~} \mathbf{r}_j, \mathbf{r}_j'  \neq \mathbf{0},
        ...
        \end{aligned}\right.
    \end{align}
    As for the special case with $\mathbf{r} = \mathbf{r}'$, we get the result
    \begin{align}
...
Based on Lemma~\ref{lemma:covar_Pauli}, this gives the variance $\text{Var}=(11/3)^n $, which still requires exponentially large sampling numbers.
...
\end{align}
This gives the variance scaling of $7.5^n$, which being improved to $3.67^n$ in the 2-replica local shadow.
We see that using higher replica numbers $k > 2$ under local measurement constraints only yields marginal improvements in the constant factor of the exponential scaling, but does not alter the complexity class. 
...
\begin{proof}[Algorithmic proof of Lemma~\ref{lemma:signal_estim}]
    ...
    \begin{align}
        ...
        \text{If~} f_P \geq 2\varepsilon, \quad |E_P - \mathrm{tr}(P \rho^k)| &=   
            |g_P/\mathrm{tr}(P \sigma^k) -\mathrm{tr}(P \rho^k)| = \frac{|g_P - \mathrm{tr}(P \sigma^k) \cdot \mathrm{tr}(P \rho^k)|}{|\mathrm{tr}(P \sigma^k)|}\\
...
\begin{problem}\label{prob:RHtest}
    Given \textbf{MH} ensemble $\mathcal{E}_{1 - R\delta}, \mathcal{E}_{L\delta}$, as defined in Problem~\ref{prob:RHtest}, with $R = (2^{n-1} + 1)/(2^{n-1} - 1 +\delta)$ and $L = (1 - 4^{-n+1} - 2^{-n+1} \delta)^{-1}$, try to design an algorithm that $\forall C \in \mathcal{C}$, discriminates two subspace-\textbf{RH} ensembles below:
    \begin{align}

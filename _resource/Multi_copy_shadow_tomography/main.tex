...
\color{red}
\emph{Remark.} The lower bound also follows directly from the product structure without the dual: any local strategy with product POVMs $\Pi_{\vec{j}} = \bigotimes_l \Pi_{j_l}^{(l)}$ and product estimators $h_\alpha(\vec{j}) = \prod_l h_{\alpha_l}^{(l)}(j_l)$ satisfies $\sum_{\vec{j}} \Pi_{\vec{j}} = \bigotimes_l s_l\1$ (so $s = \prod_l s_l$), and each site must independently solve $\sum_{j_l} h_{\alpha_l}^{(l)}(j_l)\Pi_{j_l}^{(l)} = F_{\alpha_l}^{(l)}$, giving $s_l \geq s^*_l$. The dual argument above uses the relaxed SDP dual~\eqref{eq:dual-SDP} and therefore proves $s^*_{\mathrm{loc}} \geq \prod_l s^*_{l,\mathrm{SDP}}$ (with relaxed per-site values), which is weaker when the relaxation gap is nonzero. The direct product argument avoids this gap.
\color{black}

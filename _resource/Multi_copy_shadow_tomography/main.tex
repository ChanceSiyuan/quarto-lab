...
\color{red}For the nonlinear task of estimating $\{\tr(P\rho^2)\}_P$, the relevant lifted operators have non-product structure, preventing the tensor factorization from yielding $s^*=1$. Explicit computation shows that two-copy local 3-design strategies incur exponential variance $\sim (11/3)^n$ for product states (up to $5^n$ for entangled states), in sharp contrast to the $\mathcal{O}(1)$ nonlocal cost.\color{black}
\end{abstract}
\date{\today}
\maketitle
...
What determines when multi-copy strategies help? For \emph{nonlinear} functionals such as $\tr(P\rho^2)$, the question is particularly rich: two-copy local 3-design strategies already incur exponential variance $\sim (11/3)^n$--$5^n$ depending on the state (Appendix~\ref{app:local_shadow}), yet an explicit nonlocal four-copy circuit achieves $\mathcal{O}(1)$~\cite{ekert2002direct}. This locality gap calls for a unified framework that simultaneously captures:
...
We have introduced the f-compatibility framework, which unifies quantum measurement compatibility~\cite{heinosaari2015noise,designolle2019incompatibility} with multi-copy shadow estimation. The tensor product factorization theorem (Theorem~\ref{thm:tensor}) explains the $3^w$ scaling of classical shadows as emergent from per-site $\sqrt{3}$ incompatibility, while the squaring trick reveals that nonlinear functionals can have dramatically lower estimation complexity ($s^*=1$ vs.\ $s^*=\sqrt{3}$). The contrast between the efficient nonlocal strategy (Theorem~\ref{thm:nonlocal}) and the exponential variance of local 3-design strategies highlights multi-copy entanglement as a genuine resource.
...
Another critical question is whether the f-compatibility indeed reflects the inherent hardness of simultaneously estimating a set of properties $\{f_1(\rho),\cdots,f_\alpha(\rho)\}$.

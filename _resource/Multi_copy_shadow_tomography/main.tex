...
\begin{lemma}[Martingale distinguishing lower bound]\label{theo:mart_distinguish_lb}
    \textcolor{red}{To distinguish state $\rho_0$ from a set of states $\mathcal{S}$ with success rate $\geq 1/2 + \varepsilon$, using restrained POVMs $\mathcal{M}$ and a $T$-round adaptive strategy, $T$ must satisfy $T \geq c \cdot \delta_{\mathcal{M},\rho_0,\mathcal{S}}^{-1}$, where the chi-squared divergence is $\delta_{\mathcal{M},\rho_0,\mathcal{S}} := \max_{\{F_s\} \in \mathcal{M}} \mathbb{E}_{\sigma \sim \mathcal{S}} \sum_s \frac{[\mathrm{tr}(\sigma F_s) - \mathrm{tr}(\rho_0 F_s)]^2}{\mathrm{tr}(\rho_0 F_s)}$ and $c$ is an absolute constant.  [TODO: add precise statement with conditions on $\varepsilon$, cite Le Cam / chi-squared method reference.]}
\end{lemma}

...
Since each $F_{\alpha_l}^{(l)} \neq 0$ and the multi-index $\alpha = (\alpha_1,\ldots,\alpha_n)$ ranges over a product set $\cA_1\times\cdots\times\cA_n$, uniqueness of tensor product factorization gives $G_{\alpha_l}^{(l)} = c_l\,F_{\alpha_l}^{(l)}$ for real constants $c_l \neq 0$ with $\prod_l c_l = 1$ (the constants are independent of $\alpha_l$ because varying a single site's index while fixing all others pins $c_l$).
...
\end{lemma}
\begin{proof}
    \textcolor{red}{[Proof corrected: fixed $S_4$ cycle counting and variance formula]}
    \begin{align}

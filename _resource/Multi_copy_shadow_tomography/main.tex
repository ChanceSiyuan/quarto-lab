...
The \emph{f-compatibility robustness} $s^*(\cF)$ is the minimum super-normalization $s \geq 1$ satisfying $\sum_j \Pi_j = s\cdot\1^{\ox k}$ such that the above conditions are feasible. This optimal $s^*$ is computed by the semidefinite program (SDP):

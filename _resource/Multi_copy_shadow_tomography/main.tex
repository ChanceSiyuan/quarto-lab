...
In the case $k \geq K$, this is always achievable: implementing $A_1\otimes \cdots \otimes A_K \otimes \mathbb{I}^{\otimes (k-K)}$ on $\rho^{\otimes k}$ measures each $A_i$ on a separate copy, and the marginals correctly reproduce the individual statistics.
...
\begin{lemma}[Equivalence for bounded operators]\label{thm:linear_functional_compatibility}
   Given a set of operators $\{O_i\}_i$ with $\|O_i\|_\infty \leq 1$, the linear functionals $\mathcal{F}:= \{\mathrm{tr}[O_i\rho]\}_i$ are $k$-copy f-compatible with robustness $s$ if and only if the POVMs $\{M_i^{\pm}:= \frac{\mathbb{I} \pm O_i}{2}\}_i$ are $k$-copy jointly measurable with robustness $s$. In particular, $s^*_{\cF}(k) = s^*_{\mathrm{JM}}(k)$ for all $k$.
\end{lemma}
...
\end{align}
Use the fact that $\mathrm{tr}\left[\left(\mathbb{I}^{\otimes(k-1)}\odot O_i\right) \rho^{\otimes k} \right]= \mathrm{tr}(O_i \rho)$ and $p(+1|i,\lambda) + p(-1|i,\lambda) = 1$ both are non-negative, $|h_i(\lambda)| \leq 1$.
...
\end{align}
Since this is satisfied for general $\rho$, we conclude that the symmetric version $\mathrm{sym}\left[\Pi_j\right] := (k!)^{-1}\sum_{\pi \in S_k} P_{\pi}\Pi_j P_{\pi}^\dagger$ satisfies $\sum_j h_i(j)\,\mathrm{sym}\left[\Pi_j\right] =\mathbb{I}^{\otimes(k-1)}\odot O_i$ for all $i$.
...
\begin{align}
\sum_j p(\pm 1|i,j)\,\mathrm{sym}\left[\Pi_j\right] &= \frac{1}{2}\sum_j \mathrm{sym}\left[\Pi_j\right] \pm \frac{1}{2}\sum_j h_i(j)\,\mathrm{sym}\left[\Pi_j\right]  = \frac{\mathbb{I}^{\otimes k} \pm \mathbb{I}^{\otimes(k-1)}\odot O_i}{2}\\
&=\mathbb{I}^{\otimes(k-1)}\odot \frac{\mathbb{I} + a\, O_i}{2}.
\end{align}

This is precisely a joint measurement realization of $\{\mathbb{I}^{\otimes(k-1)}\odot \frac{\mathbb{I} \pm \, O_i}{2}\}_{i=1}^K$ with parent POVM $\{\mathrm{sym}\left[\Pi_j\right]\}$, thus implies the $k$-replica compatibility of $\{\frac{\mathbb{I} \pm \, O_i}{2}\}_i$ based on Lemma~\ref{lem:structure-lemma}.
\end{proof}

\color{red}
\emph{Remark (general robustness $s$).}
The proof above establishes the equivalence for proper POVMs ($s=1$, i.e., $\sum_j \Pi_j = \mathbb{I}^{\otimes k}$). The general case follows by rescaling: $\cF$ is f-compatible with robustness $s$ (i.e., $\sum_j h_i(j)\Pi_j = \mathbb{I}^{\otimes(k-1)}\odot O_i$ with $\sum_j \Pi_j = s\cdot\mathbb{I}^{\otimes k}$) if and only if $\cF' := \{\tr[(O_i/s)\rho]\}$ is f-compatible with $s=1$. By the above, this is equivalent to joint measurability of $\{(\mathbb{I} \pm O_i/s)/2\}$, which is precisely joint measurability of $\{(\mathbb{I} \pm O_i)/2\}$ with robustness $s$.
\color{black}

This also implies that a set of linear properties $\{\mathrm{tr}\left(\rho O_i\right)\}_{i = 1}^K$ is $k$-replica f-compatible for $\rho$ if and only if its symmetric embedding $\{\mathbb{I}^{\otimes (k-1)} \odot O_i\}$ is single-copy f-compatible on $\rho^{\otimes k}$.
...
In this case, this observable $O_{\mathbf{r}}$ is a tensor product of local non-Hermitian operators $O_{\mathbf{r}} = [(Z \otimes \mathbb{I}) \cdot \mathrm{SWAP}]^{\otimes n}:= A^{\otimes n}$. The local operator $A$, although non-Hermitian, is a normal matrix that satisfies $[A,A^\dagger] = 0$, thus can still be diagonalized under unitary transformation. Actually, it satisfies the decomposition
\begin{align}
...
\begin{lemma}[On site 3-design unitary ensemble]\label{lemma:onsite_2design}
    ...
    \end{align}
    Specifically, the single-copy global 3-design unitary ensemble takes the case $n = 1$, $U \sim \mathbf{D}_{3}(d^k)$, and the channel $     \mathcal{M}(\rho) ={(d^k + 1)^{-1}} \rho + {d^k{(d^k + 1)^{-1}}  \text{tr}\rho \cdot }({\mathbb{I}(d^k)}/{d^k})$.
    The single-copy local 3-design unitary ensemble takes the case $k = 1$, $U \sim \mathbf{D}_{3}(d)^{\otimes n}$, and the channel $ \mathcal{M}(\rho) = {(d + 1)}^{-1} \rho + {d({d + 1})^{-1} \text{tr}\rho} \cdot ({\mathbb{I}(d)}/{d})$.
...
\end{proof}
\paragraph{Examples.}
 In the case $k = 2$, the commutants we choose writes $\{A_{\mathbf{r}} := {2}^{-1}{(\sigma_{\mathbf{r}}\sigma_{\mathbf{r}}\mathbb{I}_n\mathbb{I}_n + \mathbb{I}_n\mathbb{I}_n\sigma_{\mathbf{r}}\sigma_{\mathbf{r}})} \mathrm{S}_{13}\mathrm{S}_{24}\}_{\mathbf{r} \in \mathbb{F}_2^{2n}}$. Notice that $\sigma_{\mathbf{r}} \otimes \sigma_{\mathbf{r}} \ket{\Phi_{\mathbf{a}}} = (-1)^{[\mathbf{r},\mathbf{a}] + \mathbf{r}_x\cdot\mathbf{r}_z}\ket{\Phi_{\mathbf{a}}}$ (cf.\ Eq.~\eqref{eq:bell_shadow_eig}),
...
\begin{align}
    &\rho_0 := \mathbb{E}_{\rho \sim \mathcal{E}_{(1,0),C\sim \mathcal{D}}} \rho^{\otimes 2}=\mathbb{E}_{\rho \sim \mathcal{E}_{(1 - R\delta,L\delta),C\sim \mathcal{D}}} \rho^{\otimes 2} = a\cdot \mathbb{I}_{2n}/2^{2n} + b\cdot \mathrm{SWAP}^{\otimes n}/2^{n}, \\
...
\color{red}
The reduced problem discriminates $\rho_0 = \mathbb{I}_D^{\otimes 2}/D^2$ from $\sigma = \sum_{\rho \in S_2}\lambda'_\rho \mathbb{P}_\rho^D$, where $\lambda'_e, \lambda'_{(12)}$ are determined by the leaves likelihood reduction. Since $\sigma = \lambda'_e \mathbb{I}_D^{\otimes 2} + \lambda'_{(12)}\mathrm{SWAP}_D$ and $\mathrm{tr}(\rho_0 F_s) = D^{-2}\mathrm{tr}(F_s)$, the chi-squared divergence under measurement class $\mathcal{M}$ evaluates to
\begin{align}
...
\end{align}
For the RH-ensemble (purity-identity test), $\sigma = (\mathbb{I}_D^{\otimes 2} + \mathrm{SWAP}_D)/[D(D+1)]$, so $\lambda'_e = \lambda'_{(12)} = [D(D+1)]^{-1}$, and the above simplifies to (cf.~\cite{chen2021exponential})
...
\end{align}
In the case Clifford gate selected from uniform distribution, we conclude that the average \textcolor{red}{[TODO: Compute the $2T$-fold Clifford average $\Phi_{\mathrm{Cl},2T}[\Delta_{2T}]$ for the MH-ensemble. Show that for $\mathrm{Cl}$ drawn uniformly from $\mathcal{C}$, the single-round chi-squared divergence $\delta_{\mathcal{M},\rho_0,\mathcal{S}} = O(2^{-n})$ under 2-replica local measurements, so Lemma~\ref{theo:mart_distinguish_lb} gives $T = \Omega(2^n)$.]}

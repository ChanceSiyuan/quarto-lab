...
the f-compatibility robustness $s^*$ determines the sample complexity within our bounded-estimator protocol.
...
\color{red}For the nonlinear task of estimating $\{\tr(P\rho^2)\}_P$, the relevant lifted operators have non-product structure, preventing the tensor factorization from yielding $s^*=1$. Explicit computation shows that two-copy local 3-design strategies incur exponential second moment $\langle\hat{o}^2\rangle \sim (11/3)^n$ for product states (up to $5^n$ for entangled states), in sharp contrast to the $\mathcal{O}(1)$ nonlocal cost.\color{black}
\end{abstract}
\date{\today}
\maketitle
...
What determines when multi-copy strategies help? For \emph{nonlinear} functionals such as $\tr(P\rho^2)$, the question is particularly rich: two-copy local 3-design strategies already incur exponential second moment $\sim (11/3)^n$--$5^n$ depending on the state (Appendix~\ref{app:local_shadow}), yet nonlocal multi-copy circuits can achieve $\mathcal{O}(1)$---individual functionals $\tr(\rho^k)$ via the ancilla-based SWAP test~\cite{ekert2002direct}, and all $|\tr(P\rho^2)|$ simultaneously via a four-copy ancilla-free strategy (Theorem~\ref{thm:nonlocal}). This locality gap calls for a unified framework that simultaneously captures:
...
In this Letter, we develop the \emph{f-compatibility} framework, bridging quantum measurement compatibility theory~\cite{heinosaari2015noise,designolle2019incompatibility,Busch1986} with shadow estimation complexity. Our main results are: (1)~a general framework linking estimation robustness $s^*$ to sample complexity (Sec.~\ref{sec:framework}); (2)~a tensor product factorization theorem explaining the $3^w$ scaling from per-site $\sqrt{3}$ incompatibility (Sec.~\ref{sec:tensor}); (3)~the squaring trick showing that nonlinear functionals can be exponentially easier (Sec.~\ref{sec:nonlinear}); (4)~an explicit nonlocal $2k$-copy strategy (Sec.~\ref{sec:nonlocal}); and (5)~exponential second moment of local 3-design strategies for estimating $\tr(P\rho^2)$ (Sec.~\ref{sec:local_nogo}). Related concurrent work~\cite{chen2025simultaneous,ye2025replica,du2025optimal} studies nonlinear estimation from complementary perspectives.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\prlsection{Framework.}\label{sec:framework}
Consider $K$ observables $\{A_i\}_{i=1}^K$ on an $n$-qubit Hilbert space $\cH$, and a family of polynomial functionals $\cF = \{f_\alpha\}_{\alpha \in \cA}$ of degree $\leq r$ in the outcome probability vectors $\vec{p}_i(\rho) = (\tr[A_i^{(a)}\rho])_a$. By the classical estimation condition~\cite{wu2020polynomial}, unbiased estimation of degree-$r$ polynomials requires at least $r$ i.i.d.\ samples.
...
Any degree-$r$ polynomial $f_\alpha$ admits a unique \emph{linearization}: a Hermitian operator $F_\alpha$ on $\cH^{\ox k}$ ($k \geq r$) such that $f_\alpha(\vec{p}(\rho)) = \tr[F_\alpha \rho^{\ox k}]$ for all $\rho$. Condition~\eqref{eq:f-compat} becomes $\sum_j h_\alpha(j)\Pi_j = F_\alpha$.

The \emph{f-compatibility robustness} $s^*(\cF)$ is the minimum super-normalization $s \geq 1$ satisfying $\sum_j \Pi_j = s\cdot\1^{\ox k}$ such that the above conditions are feasible. This optimal $s^*$ is computed by the following optimization (a bilinear program in $h_\alpha(j)$ and $\Pi_j$, which admits an SDP relaxation that provides a lower bound $s^*_{\mathrm{SDP}} \leq s^*$; the relaxation is exact for rank-1 POVMs but can have a nonzero gap otherwise; see Appendix~\ref{app:f-compat}):
\begin{align}\label{eq:f-compat-prog}
...
\end{align}
copies of $\rho$. Within this protocol (i.i.d.\ super-normalized POVM with bounded post-processing), the optimal copy number minimizes $k\cdot [s^*(k)]^2$ over $k$.
...
Suppose the multi-index $\alpha = (\alpha_1,\ldots,\alpha_n)$ ranges over a product set $\cA_1\times\cdots\times\cA_n$, and the linearizations have product form $F_\alpha = \bigotimes_{l=1}^n F_{\alpha_l}^{(l)}$ with each $F_{\alpha_l}^{(l)} \neq 0$. Then the local f-compatibility robustness factorizes:
\begin{align}\label{eq:tensor-factor}
s^*_{\mathrm{loc}}(\cF) = \prod_{l=1}^n s^*_l,
\end{align}
where $s^*_l$ is the single-site robustness from~\eqref{eq:f-compat-prog} restricted to site $l$.
\end{theorem}
...
\begin{theorem}[Equivalence for linear binary]\label{thm:linear-equiv}
For $K$ bounded Hermitian operators $\{O_i\}$ with $\|O_i\|_\infty \leq 1$, the linear functionals $\cF = \{\tr(O_i\rho)\}_i$ are $k$-copy $\cF$-compatible with robustness $s$ if and only if the two-outcome POVMs $\{(\1 \pm O_i)/2\}_i$ are $k$-copy jointly measurable with robustness $s$. In particular, $s^*_\cF(k) = s^*_{\mathrm{JM}}(k)$ for all $k$.
\end{theorem}
...
Let $d = 2^n$ and let $\bP^d_\pi$ be the permutation operator on $2k$ replicas of $(\mathbb{C}^d)^{\ox 2k}$ with $\pi = (\mathrm{odds})(\mathrm{even})$ acting as $k$-cycles on odd and even labels respectively, and let $\sym_\pi[A] := \frac{1}{k}\sum_{j=0}^{k-1} \pi^j A\, (\pi^{j})^\dagger$ denote symmetrization under the cyclic group generated by $\pi$. Define
\begin{align}\label{eq:AP}
A_P = \sym_\pi\!\left[P^{\ox 2}\ox(\1^{\ox 2})^{\ox(k-1)}\right]\cdot\bP^d_\pi.
\end{align}
Then $\{A_P\}_{P\in\cP_n}$ is a commuting family with $\tr(A_P\rho^{\ox 2k}) = [\tr(P\rho^k)]^2$, so a single projective measurement in its common eigenbasis simultaneously estimates all $|\tr(P\rho^k)|$.
\end{theorem}
...
for $\mathbf{a} < \mathbf{b}$ with eigenvalues $\lambda = (-1)^{\mathbf{r}_x\cdot\mathbf{r}_z}\cdot s\cdot 2^{-1}[(-1)^{[\mathbf{r},\mathbf{a}]} + (-1)^{[\mathbf{r},\mathbf{b}]}]$, where the phase $(-1)^{\mathbf{r}_x\cdot\mathbf{r}_z}$ depends only on the Pauli $P = \sigma_\mathbf{r}$ (cf.\ Eq.~\eqref{eq:bell_eigenvalue}). We realize this measurement with a circuit of transversal CNOTs followed by adaptive computational/GHZ-basis measurements (Appendix~\ref{app:UB_nonlin_P_shad}, Fig.~\ref{fig:4rep_strategy}).
...
We have introduced the f-compatibility framework, which unifies quantum measurement compatibility~\cite{heinosaari2015noise,designolle2019incompatibility} with multi-copy shadow estimation. The tensor product factorization theorem (Theorem~\ref{thm:tensor}) explains the $3^w$ scaling of classical shadows as emergent from per-site $\sqrt{3}$ incompatibility, while the squaring trick reveals that nonlinear functionals can have dramatically lower estimation complexity ($s^*=1$ vs.\ $s^*=\sqrt{3}$). The contrast between the efficient nonlocal strategy (Theorem~\ref{thm:nonlocal}) and the exponential second moment of local 3-design strategies highlights multi-copy entanglement as a genuine resource.
...
The ordinary definition of compatibility captures the following notion of simultaneous measurability:
\begin{definition}
...
For example, the Pauli measurements on a qubit can be represented as a set of POVMs $\{E_{a|x}\}$ with $E_{a|x = X} = (1/2)\cdot (\1 \pm  X)$, $E_{a|x = Y} = (1/2)\cdot (\1 \pm  Y )$  and $E_{a|x = Z} = (1/2)\cdot (\1\pm Z)$, where $a\in \{+,-\}$ and $x\in \{X, Y, Z\}$. 
...
$$
E^{\eta_{P}}_{a|x = P} = \eta_P \frac{\1 \pm P}{2} + (1 - \eta_P) \frac{\1}{2} =  \frac{\1 \pm P\eta_P}{2}, P \in \{X, Y, Z\}
$$
The mixing of $\1$ can be interpreted as $\eta_P$ fractional random guessing or a noise on Pauli measurement $P$.
As shown in~\cite{Busch1986}, the unsharp Pauli measurements are jointly measurable if and only if $(\eta_x)^2 + (\eta_y)^2 +(\eta_z)^2 \leq 1$.
...
The triple is jointly measurable if and only if $(\eta_x)^2 + (\eta_y)^2 +(\eta_z)^2 \leq 1$. The corresponding joint measurement, whose marginals are the unsharp Pauli observables, is  $G(x, y, z) = \frac{1}{8}(\1 + x\eta_x X + y\eta_y Y + z\eta_z Z)$.
\end{theorem}

\subsubsection{Robustness of Incompatibility}
It is unnecessary to always use $\1$ mixedness to cancel the non-compatibility. In \cite{skrzypczyk2019all}, the authors use arbitrary $N_{a|x}$ mixed with $M_{a|x}$ to cancel the non-compatibility, that is, 
\begin{align}
...
\begin{definition}[Generalized Robustness of compatibility]\label{def:generalized_robustness}
    Given a set of POVMs $\{M_{a|x}\}_{x}$, its generalized robustness of compatibility is defined as the minimization over $r \in [0,\infty]$, the joint POVMs $ \{G_{\lambda}\}_{\lambda}$, and post-processing distribution $p(a|x,\lambda)$, such that condition below is satisfied:
\begin{align}
...
& \sum_{i,j \in \{0,1\}} \tilde{G}_{ij} = s\1 \\
...
In the case $k \geq K$, this is always achievable: implementing $A_1\otimes \cdots \otimes A_K \otimes \1^{\otimes (k-K)}$ on $\rho^{\otimes k}$ measures each $A_i$ on a separate copy, and the marginals correctly reproduce the individual statistics.
...
$\tilde{E}_{a|x} := \1^{\otimes(k-1)}\odot E_{a|x} $ is compatible.
\end{lemma}
...
Symmetrize: $\tilde{G}_\lambda := \Sigma_k(G_\lambda)$. This preserves positivity and normalization, and $\tr[\rho^{\otimes k}\, \tilde{G}_\lambda] = \tr[\rho^{\otimes k}\, G_\lambda]$. The symmetrized marginals $\tilde{M}_{a|x} = \Sigma_k(M_{a|x})$ satisfy $\tr[\rho^{\otimes k}\, \tilde{M}_{a|x}] = \tr[\rho\, E_{a|x}] = \tr[\rho^{\otimes k}\, \tilde{E}_{a|x}]$ for all $\rho$. Since products $\rho^{\otimes k}$ span the symmetric subspace (by the polarization identity) and $\tilde{M}_{a|x}$, $\tilde{E}_{a|x}$ are both permutation-symmetric, we conclude $\tilde{M}_{a|x} = \tilde{E}_{a|x}$ on the symmetric subspace. This suffices: $\{\tilde{G}_\lambda\}$ is a parent POVM whose marginals reproduce $\tr[\rho^{\otimes k}\, \tilde{E}_{a|x}]$ for all $\rho^{\otimes k}$, which is exactly the standard compatibility condition when tested on symmetric states.
\end{proof}
...
Conversely, for a monomial $m_{\mathbf{x}} = \prod_{i,j} p_{i,j}^{x_{i,j}}$ with $\sum_j x_{i,j} = n_i$, to build an unbiased estimator, we first build a product of i.i.d.\ distributions of $n_i$ random variables distributed according to $\{p_{i,j}\}_{j \in \Omega_i}$.
...
\color{red}
\paragraph{Super-normalized POVM and the $s^2$ factor.}
When the POVM is super-normalized, $\sum_j \Pi_j = s\cdot\1^{\otimes k}$ with $s > 1$, we rescale to form a valid probability distribution: define $\Pi_j' := \Pi_j/s$, which satisfies $\sum_j \Pi_j' = \1^{\otimes k}$. Sampling outcome $j$ with probability $p_j = \mathrm{tr}[\Pi_j'\,\rho^{\otimes k}]$, the estimator for $f_\alpha(\rho)$ becomes $\hat{f}_\alpha(j) := s \cdot h_\alpha(j)$, since
\begin{align}
\mathbb{E}[\hat{f}_\alpha] = \sum_j s\, h_\alpha(j)\,\mathrm{tr}[\Pi_j'\,\rho^{\otimes k}] = \sum_j h_\alpha(j)\,\mathrm{tr}[\Pi_j\,\rho^{\otimes k}] = f_\alpha(\rho).
\end{align}
Since $|h_\alpha(j)| \leq 1$, we have $|\hat{f}_\alpha(j)| \leq s$, and Hoeffding's inequality gives
\begin{align}\label{eq:hoeffding-super}
\mathrm{Pr}\!\left[\left|\frac{1}{N}\sum_{i=1}^N \hat{f}_\alpha(j_i) - f_\alpha(\rho)\right| \geq \varepsilon\right] \leq 2\exp\!\left(-\frac{N\varepsilon^2}{2s^2}\right).
\end{align}
A union bound over $|\cA|$ functionals and setting the right-hand side to $\delta$ yields the number of measurements $N_{\mathrm{meas}} = \frac{2s^2}{\varepsilon^2}\log\!\left(\frac{2|\cA|}{\delta}\right)$.
Since each measurement consumes $k$ copies of $\rho$, the total copy-complexity is
\begin{align}\label{eq:copy-complexity-derived}
N_{\mathrm{copies}} = k \cdot N_{\mathrm{meas}} = \mathcal{O}\!\left(k\cdot s^2\cdot\varepsilon^{-2}\cdot\log(|\cA|/\delta)\right),
\end{align}
which is Eq.~\eqref{eq:copy-complexity} in the main text with $s = s^*(k)$, the f-compatibility robustness.
\color{black}
...
\color{red}
\begin{lemma}[Copy number upper bound for f-compatibility]\label{lem:f-upperbound}
For any set of POVMs $\{A_i\}_{i=1}^K$ that is $k$-compatible, and any polynomial functional set $\mathcal{F}$ of degree at most $k$ in the outcome probabilities $\{\vec{p}_i\}$ that admits bounded classical estimators $C_\alpha(\vec{x}) \in [-1,1]$, then $\mathcal{F}$ is $k$-copy f-compatible: $k(\mathcal{F}) \leq k$. The bound is tight when $f_\alpha$ extracts individual outcome probabilities: $f_{(i,a_i)} = p_i(a_i) = \mathrm{tr}[A_i^{(a_i)} \rho]$, recovering standard $k$-compatibility.
\end{lemma}
\begin{proof}
    Assume $\{A_i\}_{i=1}^K$ is $k$-compatible. By Lemma~\ref{lem:structure-lemma}, there exists a joint mother POVM $G(\vec{x}) = G(x_1, \ldots, x_K)$ acting on $\mathcal{H}^{\otimes k}$ such that measuring $G(\vec{x})$ on $\rho^{\otimes k}$ yields a joint classical probability distribution $P(\vec{x}|\rho) = \mathrm{tr}[G(\vec{x})\rho^{\otimes k}]$, from which marginal distributions are recovered: $\sum_{\vec{x} \setminus x_i} P(\vec{x}|\rho) = p_i(x_i|\rho)$ for $x_i \in \Omega_i$.

    Since $\mathcal{F} = \{f_\alpha\}_{\alpha}$ consists of degree-$\leq k$ polynomial functionals that admit bounded classical estimators from the joint outcome distribution, there exist classical functions $C_\alpha(\vec{x}) \in [-1, 1]$ such that:
    $$
...
\emph{Remark (higher-degree functionals).}
For degree-$r_m$ functionals with $r_m > k$, one might expect $k(\mathcal{F}) \leq r_m \cdot k$ by repeating the joint measurement $G$ on $r_m$ independent blocks of $k$ copies each. However, the classical polynomial estimator for a degree-$r_m$ functional need not satisfy $|h_\alpha(\vec{x})| \leq 1$, so f-compatibility (with $s = 1$) is not guaranteed. The bound holds with a supernormalization factor $s$ proportional to the $\ell_\infty$-norm of the classical estimator. \textcolor{red}{[TODO: Quantify the precise $s$ factor for the $r_m$-fold repetition strategy and determine whether it can be avoided by a direct $r_m k$-copy POVM construction.]}
\color{black}

Another critical question is whether f-compatibility indeed reflects the inherent hardness of simultaneously estimating a set of properties $\{f_1(\rho),\cdots,f_\alpha(\rho)\}$.
Given two sets of functions of $\rho$, $\mathcal{S}:= (S_\alpha(\rho))_{\alpha \in \mathcal{A}}$ and $\mathcal{T}:=(T_\beta(\rho))_{\beta \in \mathcal{B}}$, if there exists a computable function $f$ such that $T_\beta(\rho) = f(S_\alpha(\rho))$ for all $\beta \in \mathcal{B}$ and all $\rho$, then $\{T_\beta(\rho)\}_{\beta \in \mathcal{B}}$ is \emph{at most as hard} to estimate as $\{S_\alpha(\rho)\}_{\alpha \in \mathcal{A}}$: any estimator for $\mathcal{S}$ yields one for $\mathcal{T}$ by post-processing. The converse (equal complexity) requires a reduction in both directions.


\subsection{Application on linear functionals}
For Hermitian operators $O_i$ with bounded eigenvales, $\|O_i\|_{\infty} \leq 1$, it splits the Hilbert space into two eigenspaces with POVMs as $\{M_i^{(+)}:= \frac{\1 + O_i}{2},M_i^{(-)}:= \frac{\1 - O_i}{2}\}$.
An familiar example is the Pauli compatibility problem, given a set of Pauli observables $\{P_i\}_i$, getting its average is as hard as compatible realization its 2-outcome POVMs $\frac{\1 \pm P_i}{2}$.
...
\begin{lemma}[Equivalence for bounded operators]\label{thm:linear_functional_compatibility}
   Given a set of operators $\{O_i\}_i$ with $\|O_i\|_\infty \leq 1$, the linear functionals $\mathcal{F}:= \{\mathrm{tr}[O_i\rho]\}_i$ are $k$-copy f-compatible with robustness $s$ if and only if the POVMs $\{M_i^{\pm}:= \frac{\1 \pm O_i}{2}\}_i$ are $k$-copy jointly measurable with robustness $s$. In particular, $s^*_{\cF}(k) = s^*_{\mathrm{JM}}(k)$ for all $k$.
\end{lemma}
...
\begin{align}
\1^{\otimes(k-1)}\odot \frac{\1 + a\, O_i}{2} = \sum_\lambda p(a|i,\lambda)\, G_\lambda, \quad \forall\, a, i.
\end{align}
Taking the difference $a=+1$ minus $a=-1$:
\begin{align}
    \1^{\otimes(k-1)}\odot  O_i = \sum_\lambda \bigl[p(+1|i,\lambda) - p(-1|i,\lambda)\bigr]\, G_\lambda =: \sum_\lambda h_i(\lambda)\, G_\lambda.
\end{align}
Use the fact that $\mathrm{tr}\left[\left(\1^{\otimes(k-1)}\odot O_i\right) \rho^{\otimes k} \right]= \mathrm{tr}(O_i \rho)$ and $p(+1|i,\lambda) + p(-1|i,\lambda) = 1$ both are non-negative, $|h_i(\lambda)| \leq 1$.
...
\begin{align}
     \mathrm{tr}\left[ \sum_j h_i(j)\,\Pi_j \rho^{\otimes k}\right] = \mathrm{tr}\left[ O_i \rho \right] = \mathrm{tr}\left[ \1^{\otimes(k-1)}\odot  O_i \rho^{\otimes k} \right] .
\end{align}
Since this is satisfied for general $\rho$, we conclude that the symmetric version $\mathrm{sym}\left[\Pi_j\right] := (k!)^{-1}\sum_{\pi \in S_k} P_{\pi}\Pi_j P_{\pi}^\dagger$ satisfies $\sum_j h_i(j)\,\mathrm{sym}\left[\Pi_j\right] =\1^{\otimes(k-1)}\odot O_i$ for all $i$.
...
\begin{align}
\sum_j p(\pm 1|i,j)\,\mathrm{sym}\left[\Pi_j\right] &= \frac{1}{2}\sum_j \mathrm{sym}\left[\Pi_j\right] \pm \frac{1}{2}\sum_j h_i(j)\,\mathrm{sym}\left[\Pi_j\right]  = \frac{\1^{\otimes k} \pm \1^{\otimes(k-1)}\odot O_i}{2}\\
&=\1^{\otimes(k-1)}\odot \frac{\1 + a\, O_i}{2}.
\end{align}

This is precisely a joint measurement realization of $\{\1^{\otimes(k-1)}\odot \frac{\1 \pm \, O_i}{2}\}_{i=1}^K$ with parent POVM $\{\mathrm{sym}\left[\Pi_j\right]\}$, thus implies the $k$-replica compatibility of $\{\frac{\1 \pm \, O_i}{2}\}_i$ based on Lemma~\ref{lem:structure-lemma}.
\end{proof}

\color{red}
\emph{Remark (general robustness $s$).}
The proof above establishes the equivalence for proper POVMs ($s=1$, i.e., $\sum_j \Pi_j = \1^{\otimes k}$). The general case follows by rescaling: $\cF$ is f-compatible with robustness $s$ (i.e., $\sum_j h_i(j)\Pi_j = \1^{\otimes(k-1)}\odot O_i$ with $\sum_j \Pi_j = s\cdot\1^{\otimes k}$) if and only if $\cF' := \{\tr[(O_i/s)\rho]\}$ is f-compatible with $s=1$. By the above, this is equivalent to joint measurability of $\{(\1 \pm O_i/s)/2\}$, which is precisely joint measurability of $\{(\1 \pm O_i)/2\}$ with robustness $s$.
\color{black}

This also implies that a set of linear properties $\{\mathrm{tr}\left(\rho O_i\right)\}_{i = 1}^K$ is $k$-replica f-compatible for $\rho$ if and only if its symmetric embedding $\{\1^{\otimes (k-1)} \odot O_i\}$ is single-copy f-compatible on $\rho^{\otimes k}$.
...
\end{definition}
An $l \geq r$ linearization of a degree-$r$ function of $\rho$ can always be done since its components satisfy $\left[\mathrm{tr}(O \rho^{m})\right]^{p} = \mathrm{tr}([(O\otimes \1^{m-1})\cdot S_{m}]^{\otimes p} \cdot \rho^{\otimes m p})$.
 Thus we can always set $F = \1^{\otimes (l-mp)} \otimes [(O\otimes \1^{m-1}) \cdot S_{m}]^{\otimes p}$ or its symmetric over $l$ copies of state $\rho^{\otimes l}$.

\begin{lemma}[Linearization lemma]\label{lem:structure_fcomp_lemma}
Given a set of rank-$r$ functions $\mathcal{F}$, it is $l$-replica compatible if and only if its $l$-linearization is possible and is single-copy f-compatible on $\rho^{\otimes l}$.
\end{lemma}
\begin{proof}
    \color{red}
    ($\Rightarrow$) Suppose $\cF$ is $l$-replica f-compatible. By Definition~\ref{def:f-compatibility}, there exists a POVM $\{\Pi_j\}$ on $\cH^{\ox l}$ and bounded estimators $h_\alpha(j) \in [-1,1]$ such that $f_\alpha(\rho) = \sum_j h_\alpha(j)\,\tr[\Pi_j\,\rho^{\ox l}]$ for all $\rho$ and all $\alpha$. Since the $l$-linearization $\{F_\alpha\}$ satisfies $\tr[F_\alpha\,\rho^{\ox l}] = f_\alpha(\rho)$, we have $\sum_j h_\alpha(j)\,\tr[\Pi_j\,\sigma] = \tr[F_\alpha\,\sigma]$ for all $\sigma = \rho^{\ox l}$.
    Since products $\{\rho^{\ox l}\}_\rho$ span the symmetric subspace of $\cH^{\ox l}$ (by the polarization identity), and both sides are linear in $\sigma$, we may replace $\Pi_j$ by its symmetrization $\tilde{\Pi}_j = (l!)^{-1}\sum_{\pi \in S_l} P_\pi \Pi_j P_\pi^\dagger$ without changing expectation values on $\rho^{\ox l}$. Then $\sum_j h_\alpha(j)\,\tilde{\Pi}_j = F_\alpha$ holds as an operator identity (on the symmetric subspace, which suffices). This is precisely single-copy f-compatibility of $\{F_\alpha\}$ on the state space $\cH^{\ox l}$.

    ($\Leftarrow$) Suppose the $l$-linearizations $\{F_\alpha\}$ are single-copy f-compatible on $\cH^{\ox l}$: there exist POVM $\{\Pi_j\}$ on $\cH^{\ox l}$ and bounded estimators $h_\alpha(j) \in [-1,1]$ with $\sum_j h_\alpha(j)\,\Pi_j = F_\alpha$. Then for all $\rho$:
    \begin{align}
    \sum_j h_\alpha(j)\,\tr[\Pi_j\,\rho^{\ox l}] = \tr\!\left[\sum_j h_\alpha(j)\,\Pi_j\;\rho^{\ox l}\right] = \tr[F_\alpha\,\rho^{\ox l}] = f_\alpha(\rho),
    \end{align}
    which is exactly $l$-replica f-compatibility of $\cF$.
    \color{black}
\end{proof}

\subsection{f-Compatibility program: primal and dual}

\color{red}Since now $k$-replica f-compatibility on a set of rank-$r$ properties can be reduced to a single-replica f-compatibility problem (Lemma~\ref{lem:structure_fcomp_lemma}), in this section we formulate the single-replica f-compatibility robustness $s^*(\cF)$ as an optimization problem.
The exact formulation is a \emph{bilinear program} (bilinear in the POVM elements $\{\Pi_j\}$ and the scalar estimators $\{h_\alpha(j)\}$), which is not an SDP but can be relaxed to one (see below).\color{black}
The optimization variables are the POVM elements $\{\Pi_j\}_{j\in\cJ}$, the estimator scalars $\{h_\alpha(j)\}$, and the super-normalization factor $s$.
...
\color{red}
\paragraph{SDP relaxation and its gap.}
The bilinear program~\eqref{eq:primal-bilinear} is computationally hard due to the product constraint $h_\alpha(j)\Pi_j$ (bilinear in the scalar $h_\alpha(j)$ and the matrix $\Pi_j$). A standard relaxation replaces this product by a free matrix variable: introduce $\Gamma_{\alpha,j} := h_\alpha(j)\Pi_j$ subject to $\Pi_j \pm \Gamma_{\alpha,j} \geq 0$ and $\sum_j \Gamma_{\alpha,j} = F_\alpha$. The constraint $\Pi_j \pm \Gamma_{\alpha,j} \geq 0$ is equivalent to $|h_\alpha(j)| \leq 1$ when $\Gamma_{\alpha,j} = h\Pi_j$ for a scalar $h$, but for rank $>1$ the matrix $\Gamma_{\alpha,j}$ can satisfy $\Pi_j \pm \Gamma_{\alpha,j} \geq 0$ without being a scalar multiple of $\Pi_j$. Thus the relaxed SDP provides a \emph{lower bound}: $s^*_{\mathrm{SDP}} \leq s^*$.

The relaxation is exact for rank-1 (projective) POVMs (since $\Pi_j = \proj{\psi_j}$ forces $\Gamma_{\alpha,j} = c_\alpha \proj{\psi_j}$ for some $|c_\alpha| \leq 1$), but can underestimate $s^*$ otherwise. For example, for $\cF = \{\tr(X\rho), \tr(Y\rho), \tr(Z\rho)\}$ on $\mathbb{C}^2$, the relaxed SDP gives $s^*_{\mathrm{SDP}} = 1$ while the true scalar-constrained answer is $s^* = \sqrt{3}$.

\textcolor{red}{[TODO: Determine whether additional constraints (e.g., NPA-style moment conditions on $\Gamma_{\alpha,j}$) can close the relaxation gap, or whether one should restrict to rank-1 POVMs throughout.]}
\color{black}

\paragraph{Dual of the relaxed SDP.}
The dual of the relaxed SDP reads:
\begin{align}
s^*_{\mathrm{SDP}} = \max\; & \sum_\alpha \tr[\Lambda_\alpha F_\alpha] \notag\\
\text{s.t.}\; & \sum_\alpha |\Lambda_\alpha| \leq W, \quad \tr[W] = 1, \quad W \geq 0, \label{eq:dual-SDP}
\end{align}
where $|\Lambda_\alpha| = \sqrt{\Lambda_\alpha^\dagger \Lambda_\alpha}$ denotes the operator absolute value. By weak duality applied to the exact bilinear program, $s^*_{\mathrm{SDP}} \leq s^*$, so the dual objective provides a \emph{lower bound} on the true f-compatibility robustness.
...
\begin{align}
(P_i \ox P_i)\ket{\Phi_{ab}} = (-1)^{[P_i, (a,b)] + (P_i)_x(P_i)_z}\ket{\Phi_{ab}},\label{eq:bell_eigenvalue}
\end{align}
where $[\cdot,\cdot]$ denotes the symplectic inner product and $(P_i)_x(P_i)_z$ is the product of the $x$- and $z$-components of $P_i$'s symplectic vector (this phase arises from $\sigma_\mathbf{q}\ox\sigma_\mathbf{q}\ket{\Phi_\mathbf{r}} = (-1)^{[\mathbf{r},\mathbf{q}]+\mathbf{q}_x\cdot\mathbf{q}_z}\ket{\Phi_\mathbf{r}}$; see Appendix~\ref{app:Bell_shadow}). Setting $\Pi_{ab} = \proj{\Phi_{ab}}$ and $h_i(a,b) = (-1)^{[P_i,(a,b)] + (P_i)_x(P_i)_z}$, we have $\sum_{ab} h_i(a,b)\Pi_{ab} = P_i \ox P_i = F_i$ and $\sum_{ab}\Pi_{ab} = \1^{\ox 2}$, so $s = 1$. Since $|h_i(a,b)| = 1 \leq 1$, all constraints are satisfied with $s^* = 1$.

\paragraph{$\mathcal{F} = \{\tr(X\rho), \tr(Y\rho), \tr(Z\rho)\}$.}
\color{red}
For single-qubit linear Pauli functionals, the linearizations are $F_i = P_i \in \{X,Y,Z\}$ (degree~1, so $k=1$ suffices). By Lemma~\ref{thm:linear_functional_compatibility}, the f-compatibility robustness equals the joint measurability robustness of the unsharp Paulis $\{(\1 + \eta P_i)/2\}_{P_i \in \{X,Y,Z\}}$ with $\eta = 1/s$. By Busch's theorem~\cite{Busch1986}, the triple is jointly measurable if and only if $3\eta^2 \leq 1$, giving $\eta_{\max} = 1/\sqrt{3}$ and $s^* = \sqrt{3}$. The optimal joint measurement is $G(x,y,z) = \frac{1}{8}(\1 + \frac{x}{\sqrt{3}} X + \frac{y}{\sqrt{3}} Y + \frac{z}{\sqrt{3}} Z)$ with $x,y,z \in \{\pm 1\}$, which is precisely the single-qubit random Clifford shadow POVM. This per-site analysis recovers the classical shadow $3^w$ scaling via the tensor product factorization (Theorem~\ref{thm:tensor}).
\color{black}

\paragraph{$\mathcal{F} = \{ \tr(\rho^2),\tr(X\rho^2), \tr(Y\rho^2), \tr(Z\rho^2)\}$.}
\color{red}
The 2-linearizations are $F_0 = \mathrm{SWAP}$ and $F_P = \frac{1}{2}(P\ox\1 + \1\ox P)\cdot\mathrm{SWAP}$ for $P \in \{X,Y,Z\}$. On the symmetric subspace $\mathrm{Sym}^2(\mathbb{C}^2) \cong \mathbb{C}^3$, the operator $F_0 = \1_3$ (trivial), while $\{F_X, F_Y, F_Z\}$ have eigenvalues $\{+1, 0, -1\}$: they are spin-1 observables satisfying $[F_X, F_Y]|_{\mathrm{Sym}} = iF_Z|_{\mathrm{Sym}}$ (and cyclic permutations), where the restriction to $\mathrm{Sym}^2(\mathbb{C}^2)$ is needed since $\mathrm{SWAP} = \1$ there. Since they are non-binary, Lemma~\ref{thm:linear_functional_compatibility} does not apply, and the f-compatibility robustness may be strictly less than the joint measurability robustness of the corresponding 3-outcome POVMs. The non-commutativity $[F_P, F_Q] \neq 0$ means no single projective measurement suffices (unlike the quadratic case above), but the spin-1 structure may allow more efficient approximate estimation than the spin-$\frac{1}{2}$ Pauli case. \textcolor{red}{[TODO: Compute the SDP value of $s^*$ for this functional set and compare with the spin-$\frac{1}{2}$ bound $\sqrt{3}$.]}
\color{black}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
To utilize the f-compatibility SDP for a $n$-qubit state $\rho^{\otimes n}$, we proposed that we can tensor product the optimal solution of f-compatibility SDP for a $k$-qubit state and forms a $k$-replica strategy to simultaneously estimate the $n$-qubit observables.
...
By definition (Sec.~\ref{sec:tensor}), a local strategy uses product POVMs $\Pi_{\vec{j}} = \bigotimes_l \Pi_{j_l}^{(l)}$ and factored estimators $h_\alpha(\vec{j}) = \prod_l h_{\alpha_l}^{(l)}(j_l)$.
The normalization constraint gives $\sum_{\vec{j}} \Pi_{\vec{j}} = \bigotimes_l (s_l\,\1)$, so $s = \prod_l s_l$.
By the distributive law, the estimation constraint becomes
$\bigotimes_l G_{\alpha_l}^{(l)} = \bigotimes_l F_{\alpha_l}^{(l)}$,
where $G_{\alpha_l}^{(l)} := \sum_{j_l} h_{\alpha_l}^{(l)}(j_l)\,\Pi_{j_l}^{(l)}$.
We claim $G_{\alpha_l}^{(l)} = c_l\,F_{\alpha_l}^{(l)}$ for constants $c_l \neq 0$ independent of $\alpha_l$, with $\prod_l c_l = 1$. This follows from uniqueness of simple tensor decomposition and the product index structure:
\emph{Step 1 (rank-1 tensor uniqueness).} If $A\ox B = C\ox D$ with $C,D \neq 0$, then $A = \lambda C$ and $B = \lambda^{-1}D$ for some scalar $\lambda$. (Proof: pick $\phi$ with $\tr(\phi^\dagger D) \neq 0$; applying $\tr_{2}[(\cdot)\cdot(\1\ox\phi^\dagger)]$ to both sides gives $A\tr(\phi^\dagger B) = C\tr(\phi^\dagger D)$, so $A = \lambda C$; substituting back gives $B = \lambda^{-1}D$.) By induction, grouping site $l$ against $\bigotimes_{m\neq l}$, we get $G_{\alpha_l}^{(l)} = c_l(\alpha)\,F_{\alpha_l}^{(l)}$ for some $c_l(\alpha)$ with $\prod_l c_l(\alpha) = 1$.
\emph{Step 2 (independence of $c_l$).} Since $G_{\alpha_l}^{(l)}$ depends only on $\alpha_l$ (not on $\alpha_{m\neq l}$), the ratio $c_l(\alpha) = G_{\alpha_l}^{(l)} / F_{\alpha_l}^{(l)}$ (as operators, in the sense of proportionality) is independent of the other sites' indices $\alpha_{m\neq l}$. Thus $c_l(\alpha) = c_l(\alpha_l)$. Now fix $\alpha_l$ and vary $\alpha_m$ for $m \neq l$: by the same argument at site $m$, $c_m$ is independent of $\alpha_l$, so $c_l(\alpha_l) = \prod_{m\neq l} c_m^{-1}$ is constant. Hence $c_l$ is independent of all indices.

Define per-site estimator bounds $B_l := \max_{j_l,\alpha_l} |h_{\alpha_l}^{(l)}(j_l)|$.
The global constraint $|h_\alpha(\vec{j})| = \prod_l |h_{\alpha_l}^{(l)}(j_l)| \leq 1$
implies $\prod_l B_l \leq 1$ (by choosing the maximizing index at each site independently).
Now construct a valid per-site f-compatibility solution:
$\hat{h}_{\alpha_l}(j_l) := \sgn(c_l)\,h_{\alpha_l}^{(l)}(j_l)/B_l$ and
$\hat{\Pi}_{j_l} := B_l\,|c_l|^{-1}\,\Pi_{j_l}^{(l)}$.
One verifies $\sum_{j_l} \hat{h}_{\alpha_l}(j_l)\,\hat{\Pi}_{j_l} = \frac{\sgn(c_l)\,c_l}{|c_l|}\,F_{\alpha_l}^{(l)} = F_{\alpha_l}^{(l)}$,
with $|\hat{h}_{\alpha_l}| \leq 1$, $\hat{\Pi}_{j_l} \geq 0$, and normalization $\hat{s}_l = B_l\,s_l/|c_l|$.
Per-site optimality requires $\hat{s}_l \geq s^*_l$, i.e., $s_l \geq |c_l|\,s^*_l/B_l$.
Multiplying over sites:
\begin{align}
s = \prod_l s_l \;\geq\; \frac{\prod_l |c_l| \cdot \prod_l s^*_l}{\prod_l B_l} = \frac{\prod_l s^*_l}{\prod_l B_l} \;\geq\; \prod_l s^*_l,
\end{align}
where we used $\prod_l |c_l| = |\prod_l c_l| = 1$ and $\prod_l B_l \leq 1$.

\color{red}
\emph{Remark (dual SDP verification).} An independent lower bound follows from the relaxed SDP dual~\eqref{eq:dual-SDP}. Constructing product dual variables $\Lambda_\alpha := \bigotimes_{l} \Lambda_{\alpha_l}^{*(l)}$, $W := \bigotimes_{l} W_l^*$ from per-site optimal duals gives $s^*_{\mathrm{global}} \geq \prod_l s^*_{l,\mathrm{SDP}}$, and $s^*_{\mathrm{loc}} \geq s^*_{\mathrm{global}}$ since restricting to local POVMs can only increase the primal optimum. This argument uses the relaxed SDP values $s^*_{l,\mathrm{SDP}} \leq s^*_l$, so it yields a weaker bound when the relaxation gap is nonzero. The direct argument above, which uses the product estimator constraint in the definition of local strategies, avoids this gap.
\color{black}
...
\emph{(d) f-Compatibility program as dual.}
The f-compatibility bilinear program~\eqref{eq:primal-bilinear} is the dual perspective: POVM elements $\{\Pi_j\}$ correspond to common eigenprojectors of the optimal $\{R_\alpha\}$, and estimators $h_\alpha(j)$ correspond to eigenvalues $r_{\alpha,\lambda}$.
\end{proposition}

\begin{theorem}[Tensor commutativity uniqueness, informal; {\cite[Theorem~4]{king2024exponential}}]\label{thm:king-nogo}
If two unitaries $U,V$ of finite order $d$ do not commute, yet there exist unitaries $\tilde{U},\tilde{V}$ on an auxiliary space such that $[U\ox\tilde{U},\,V\ox\tilde{V}]=0$, then $U$ and $V$ decompose as a direct sum of displacement operators of the discrete Heisenberg group $H(\mathbb{Z}_d)$. In particular, the squaring trick ($O \ox O$ commuting when the originals anticommute) is essentially unique to the Heisenberg-Weyl group; Pauli operators are the $d=2$ case.
\end{theorem}
...
\subsection{Bell measurement and its restriction}\label{app:Bell_shadow}
...
\end{align}
In the language of shadow tomography, this channel on the two-replica space $\mathcal{H}(d^2)$ is not invertible since $\mathcal{M}_{I}(\sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{q}}) = 0$ if $\mathbf{p} \neq \mathbf{q}$. 
...
\end{lemma}
    \begin{proof}
    The variance of an observable $O$ with respect to a quantum state $\rho$ is $\text{Var}_\rho(O) = \langle O^2\rangle_\rho - \langle O\rangle_\rho^2$.
    We first show that the function $f(\rho):= \text{Var}_{\rho}(O)$ is a convex function of $\rho$. Using the convexity of $g(x) = x^2$, i.e., $[(1 - \theta)x_1 + \theta x_2]^2 \leq (1- \theta)x_1^2 + \theta x_2^2$, we have
    \begin{align}
        f((1- \theta)\rho + \theta \sigma) - (1- \theta)f(\rho) - \theta f(\sigma) = (1- \theta)\left[\text{tr}(O\rho)\right]^2 + \theta \left[\text{tr}(O\sigma)\right]^2 -\left[(1- \theta)\text{tr}(O\rho) + \theta \text{tr}(O\sigma)\right]^2 \geq 0, \notag
    \end{align}
    which shows the convexity of $f(\rho)$.
    The maximum of a convex function over a compact convex set is attained at an extreme point. Therefore, the problem reduces to maximizing the variance over all pure states $|\psi\rangle,  \text{Var}_{\psi}(O) = \langle\psi|O^2|\psi\rangle - (\langle\psi|O|\psi\rangle)^2 $.
    ...
    \end{align}
    \end{proof}
This means that Bell shadow can efficiently estimate operators $O = \sum_{\mathbf{p}} \alpha_{\mathbf{p}} \sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}}$ with bounded $\infty$-norm $\|O\|_{\infty}$.
...
&\hat{o}_{\mathbf{q}}(\mathbf{r}) =\mathrm{tr}\left[\sigma_{\mathbf{q}} \otimes \sigma_{\mathbf{q}} \ket{\Phi_{\mathbf{r}}}\!\bra{{\Phi_{\mathbf{r}}}}\right] =(-1)^{[\mathbf{r}, \mathbf{q}] + \mathbf{q}_x\cdot \mathbf{q}_z},\label{eq:bell_shadow_eig}
\end{align}
respectively. Both of them are bounded within $[-1,1]$, thus induce two efficient estimation algorithms according to the Hoeffding inequality.
\begin{figure}[htbp!]
    \centering
...
\begin{align}
    O_{\mathbf{q}}:=(\sigma_{\mathbf{q}} \otimes \1_n) \cdot \mathrm{SWAP}^{\otimes n}=2^{-n}\sum_{\mathbf{p}}\sigma_{\mathbf{q}}\sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}},
\end{align}
...
We consider the case $\mathbf{q}_i \neq 0, \forall i = 1,\cdots,n$, otherwise we reorder the terms in $\sigma_{\mathbf{q}}$ such that $O = (\sigma_{\mathbf{q}'} \otimes \1_{n'}) \cdot \mathrm{SWAP}^{\otimes n'} \otimes \mathrm{SWAP}^{\otimes (n -n')} $, and run algorithms $\mathcal{A}$ on the first $n'$ qubits and Bell measurement strategy on the last $(n-n')$ qubits.
...
\label{eq:2rep_decomp_Prho2}
    O = 2^{-n}\sum_{\mathbf{p}}\sigma_{\mathbf{q}}\sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}}= 2^{-n}\sum_{\mathbf{p}} \bigotimes_{i \in A_{\mathbf{p}}} (-iL_{\mathbf{q}_i} \sigma_{\mathbf{p}_i}L_{\mathbf{q}_i}^{\dagger} \otimes \sigma_{\mathbf{p}_i}) \bigotimes_{i \in R_{\mathbf{p}}} (\1 \otimes \sigma_{\mathbf{q}_i}) \bigotimes_{i \in L_{\mathbf{p}}} (\sigma_{\mathbf{q}_i} \otimes \1)
\end{align}
...
\begin{align}
    (L \otimes \1_n)\ket{\Phi_{\mathbf{r}}}\!\bra{\Phi_{\mathbf{r}}}(L^\dagger \otimes \1_n) = 4^{-n}\sum_{\mathbf{p}} (-1)^{[\mathbf{r}, \mathbf{p}] + \mathbf{p}_x\cdot \mathbf{p}_z}(L\sigma_{\mathbf{p}}L^{\dagger}) \otimes \sigma_{\mathbf{p}},
\end{align}
...
\end{align}
For the rest part $R_{\mathbf{p}},L_{\mathbf{p}}$, we have to extract the information of insymmetric Pauli operator $\1 \otimes \sigma_{\mathbf{q}}, \sigma_{\mathbf{q}} \otimes \1$, which is prohibitive for local conjugated Bell measurement.
...
\begin{align}
    \Lambda_{i}(\cdot ) = (1/2)\1\cdot\mathrm{tr}(\cdot) + (1/2)\mathrm{tr}\left[\sigma_{\mathbf{q}_i} (\cdot)\right] \sigma_{\mathbf{q}_i}.
\end{align}
In the observable in Eq.~\eqref{eq:2rep_decomp_Prho2}, we average all possible separation.
Since the $\sigma_{\mathbf{q}_i}$ Pauli measurements are incompatible with the Bell measurement,  this indicates the naive strategy that uniformly selects a separation $A \cup ([n]/A)$ and then implements single-copy $\sigma_{\mathbf q}$ Pauli measurement on positions $[n]/A$ and $L_{\mathbf q}$ conjugated Bell measurement on positions $A$.
...
%     \vspace{5pt} \hrule % 底部的横线
% \end{figure}
  Notice that the measurement of operator $\sigma_{\mathbf{q}_i}$ with result ${r}_i \in \{0,1\}$ forms the $n$-qubit decoherence channel $\Lambda_{\mathbf{q}}(\rho) = \bigotimes_{i= 1}^{n} \Lambda_{i}(\rho)$, where $\Lambda_{i}(\cdot ) = (1/2)\1\cdot\mathrm{tr}(\cdot) + (1/2)\mathrm{tr}\left[\sigma_{\mathbf{q}_i} (\cdot)\right] \sigma_{\mathbf{q}_i}$. 
...
we noted that
  \begin{align}
      \left\{
      \begin{aligned}
      &\mathcal{M}_i(\sigma_{\mathbf{q}_i} \otimes \1) = (1/2)\cdot \sigma_{\mathbf{q}_i} \otimes \1,\quad \mathcal{M}_i( \1\otimes\sigma_{\mathbf{q}_i}) =(1/2)\cdot \1\otimes\sigma_{\mathbf{q}_i}, \\
  ...
  \end{align}
    ...
    Noted that
    \begin{align}
        &\mathrm{tr}\left[(L_{\mathbf{q}_i} \otimes \1)\Phi_{\mathbf{r}_i}(L^\dagger_{\mathbf{q}_i} \otimes \1) \cdot (\sigma_{\mathbf{q}_i} \otimes \1)\cdot \mathrm{SWAP}\right] \\
        &= 4^{-1}\sum_{\mathbf{u}_i} (-1)^{[\mathbf{r}_i, \mathbf{u}_i] +{u}_x\cdot {u}_z}\mathrm{tr}\left[(L_{\mathbf{q}_i} \otimes \1)\sigma_{\mathbf{u}_i} \otimes \sigma_{\mathbf{u}_i} (L^\dagger_{\mathbf{q}_i} \otimes \1) \cdot (\sigma_{\mathbf{q}_i} \otimes \1)\cdot \mathrm{SWAP}\right]\\
        ...
        &\mathrm{tr}\left[\left(\frac{ \1 + r_{i,x} \sigma_{\mathbf{q}_i}}{2}\right)\otimes  \left(\frac{ \1 + r_{i,z} \sigma_{\mathbf{q}_i}}{2}\right)\cdot (\sigma_{\mathbf{q}_i} \otimes \1)\cdot \mathrm{SWAP}\right]=|\mathbf{r}_i|/2.
    \end{align}
...
The simplest one is the SWAP-test that use single anccilla qubits and non-local operation between ancilla qubits and data qubits to estimate the observable $\mathrm{SWAP}^{\otimes n}\cdot( P\otimes\1_n )$, as illustrated in Figure~\ref{fig:nonlocal_strate} for $n=4$ and $P = XZYX$.
...
In this case, this observable $O_{\mathbf{r}}$ is a tensor product of local non-Hermitian operators $O_{\mathbf{r}} = [(Z \otimes \1) \cdot \mathrm{SWAP}]^{\otimes n}:= A^{\otimes n}$. The local operator $A$, although non-Hermitian, is a normal matrix that satisfies $[A,A^\dagger] = 0$, thus can still be diagonalized under unitary transformation. Actually, it satisfies the decomposition
\begin{align}
...
\end{align}
Then given the localized two-copy measurement basis $\mathcal{B}(\theta_i, U_i)$ at site $i$, the full $n$-site two-copy local measurement is the product basis $\bigotimes_{i=1}^n \mathcal{B}(\theta_i, U_i)$, and the resulting shadow channel $\mathcal{M}$ depends on the ensemble from which $(\theta_i, U_i)$ are drawn. Specific instances---including local Clifford 3-design ensembles---are analyzed in the next section.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This i.i.d. unitary 3-design setting includes the Clifford randomized measurement~\cite{zhu2016clifford} and the randomized Pauli basis measurement proposed in~\cite{huang2020predicting}.
The result of $\mathcal{M}$ is firstly summarized in Lemma~\ref{lemma:onsite_2design}.
%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma
\begin{lemma}[On site 3-design unitary ensemble]\label{lemma:onsite_2design}
    A $k$-site 3-design unitary ensemble $\mathbf{D}_{3}(d^k)^{\otimes n}$ separates $n\cdot k$ qudits into $n$ parts, and independently implements a $k$-qudit 3-design unitary, as defined in Definition~\ref{def:t_design}, within each part. In other words, $\mathbf{D}_{3}(d^k)^{\otimes n}:=\{\bigotimes_{i  =1}^n U_i| U_i \sim \mathbf{D}_{3}(d^k)\}$.
    This ensemble induces the channel
    \begin{align}
        \mathcal{M}(\rho) = (\mathcal{D}_{1/(2^k + 1)})^{\otimes n}(\rho), \text{with~} \mathcal{D}_{1/(2^k + 1)}(\cdot) = \frac{1}{2^k +1}(\cdot) + \frac{\text{tr}(\cdot)}{2^k + 1}{\1(d^k)}.
    \end{align}
    Specifically, the single-copy global 3-design unitary ensemble takes the case $n = 1$, $U \sim \mathbf{D}_{3}(d^k)$, and the channel $     \mathcal{M}(\rho) ={(d^k + 1)^{-1}} \rho + {d^k{(d^k + 1)^{-1}}  \text{tr}\rho \cdot }({\1(d^k)}/{d^k})$.
    The single-copy local 3-design unitary ensemble takes the case $k = 1$, $U \sim \mathbf{D}_{3}(d)^{\otimes n}$, and the channel $ \mathcal{M}(\rho) = {(d + 1)}^{-1} \rho + {d({d + 1})^{-1} \text{tr}\rho} \cdot ({\1(d)}/{d})$.
    (The channel formula uses only the 2-design property; the 3-design property is needed for the variance computation in Lemma~\ref{lemma:LC_shadow_kcopy}.)
\end{lemma}
\begin{proof}
...
\begin{align}
    ...
    &=\mathbb{E}_{\substack{U = \bigotimes_{i=1}^n\text{U}_i \\ \text{U}_i \sim \mathbf{D}_{3}(d^k)}} \sum_{\mathbf{r} \in \mathbb{F}_d^{kn}} \text{tr}_{\bar A} \left( \left(\otimes_{i = 1}^n \rho_i^{(\bar A)} \otimes \1^{(A)}(d^{kn})\right)\cdot  (\text{U}^{\dagger} \ket{{\mathbf r}}\! \bra{{\mathbf r}} \text{U})^{(\bar A)} \otimes (\text{U}^{\dagger} \ket{{\mathbf r}}\!\bra{{\mathbf r}} \text{U})^{(A)} \right)\notag\\
    &= \text{tr}_{\bar A}\left( \left(\otimes_{i = 1}^n \rho_i^{(\bar A)}\otimes \1^{(A)}(d^{kn})\right) \cdot \bigotimes_{i = 1}^n \sum_{\mathbf{r}_i \in \mathbb{F}_d^k} \mathbb{E}_{\substack{\text{U}_i \sim \mathbf{D}_{3}(d^k)}}  (\text{U}_i^{\dagger} \ket{\mathbf{r}_{i}}\bra{\mathbf{r}_{i}} \text{U}_i)^{(\bar A)} \otimes (\text{U}_i^{\dagger} \ket{\mathbf{r}_{i}}\!\bra{\mathbf{r}_{i}} \text{U}_i)^{(A)} \right)\notag\\
    &=  \text{tr}_{\bar A}\left( \left(\otimes_{i = 1}^n \rho_i^{(\bar A)}\otimes \1^{(A)}(d^{kn})\right) \cdot \bigotimes_{i = 1}^n \sum_{\mathbf{r}_{i}\in \mathbb{F}_d^{k}} \Phi_2(\ket{\mathbf{r}_{i}}\!\bra{\mathbf{r}_{i}}^{\otimes 2})^{(\bar A A)} \right)\notag \\
    &= \text{tr}_{\bar A}\left( \left(\otimes_{i = 1}^n \rho_i^{(\bar A)}\otimes \1^{(A)}(d^{kn})\right) \cdot \bigotimes_{i = 1}^n d^{k} \frac{\1^{(\bar A)}(d^k)\otimes \1^{(A)}(d^k)+ \mathbb{P}_{(12)}^{d,(\bar A A)}}{(d^k+1) \cdot d^k}\right).\label{eq:k_copy_shadow_channel}
\end{align}
Use the fact that  $\text{tr}_{\bar A}(\mathbb{P}_{(12)}^{d,(\bar A A)}\cdot \rho^{(\bar A)}\otimes \1^{(A)}) = \rho^{(A)}$, we get the channel output
\begin{align}
    \mathcal{M}(\otimes_{i = 1}^n \rho_i) 
    &= \bigotimes_{i = 1}^n \text{tr}(\rho_i) \frac{\1(d^k)}{d^k + 1} + \frac{1}{d^k +1}\rho_i = \mathcal{D}^{\otimes n}_{{1}/{(d^k +1)}}(\otimes_{i = 1}^n \rho_i).
\end{align}
By the linearity of $\mathcal{M}$, we  conclude that its inverse $\mathcal{D}_{{1}/{(d^k +1)}}^{-1}(\rho) = (d^k + 1)\rho - \text{tr}\rho \cdot \1(d^k) $ constitute an unbiased estimator of $\rho \in \mathcal{D}(\mathcal{H}(d^{nk}))$ through the transformation
\begin{align}
     \hat \rho_{U,\mathbf b} =\mathcal{M}^{-1}(\text{U}^\dagger \ket{\mathbf b}\!\bra{\mathbf b} \text{U})=\bigotimes_{i =1}^n\left((d^k +1)\text{U}_i^{\dagger} \ket{\mathbf{b}_i}\!\bra{\mathbf{b}_i} \text{U}_i - \1(d^k)\right).
\end{align}
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{lemma}[Local k-copy 3-design shadow]\label{lemma:LC_shadow_kcopy}
    ...
    \begin{align}\label{eq:R_expression}
        R(W,W') = \prod_{i = 1}^n r(W_i,W'_i), r(W_i,W'_i) := \begin{cases}
            1 &\text{if~}W_i = \1_k \text{~or~}W'_i = \1_k ,\\
            (2^k+1) \cdot \frac{2^{k-1}\delta_{W_i,W'_i} +\delta_{W_i, \mathcal{C}(W'_i)\backslash \1_k } }{2^{k-1}+1} &\text{if~}W_i \neq \1_k \text{~and~} W'_i \neq \1_k .
        \end{cases}
    \end{align}
...
The second moment is given by $  \langle (\hat{o}^{(k)})^2 \rangle_{\rho} =\mathbb{E}_{U \sim \mathbf{D}_3(2^k)^{\otimes n}}\sum_{\mathbf r}\bra{\mathbf r}U \rho^{\otimes k} U^\dagger \ket{\mathbf{r}} \bra{\mathbf r}U\mathcal{M}^{-1}(O^{(k)}) U^\dagger\ket{\mathbf r}^2$.
    ...
    \begin{align*}
        \langle (\hat{o}^{(k)})^2 \rangle_{\rho} &=
        \sum_{W,W'}\alpha_{W}\alpha_{W'}\sum_{s}\beta_s\prod_{i = 1}^n 2^k p \cdot \begin{cases}
         1\cdot 1\cdot [2^{2k}+3\cdot 2^k+2]\text{tr}{\rho}^{(s)}_i  &\text{if~} W_i=W_i' = \1_k,\\
         1 \cdot (2^k+1) \cdot[2^k + 2]\cdot \text{tr}({\rho}^{(s)}_i W'_i)&\text{if~} W_i = \1_k \neq W'_i,\\
         (2^k+1) \cdot 1 \cdot[2^k + 2]\cdot \text{tr}({\rho}^{(s)}_i W_i)&\text{if~} W'_i = \1_k \neq W_i,\\
    ...
    %     \text{tr}{\rho}^{(s)}_i  &\text{if~} W_i=W_i' = \1_k,\\
    %      \text{tr}({\rho}^{(s)}_i W'_i)&\text{if~} W_i = \1_k\neq W'_i,\\
    %      \text{tr}({\rho}^{(s)}_i W_i)&\text{if~} W'_i = \1_k\neq W_i,\\
    %     \frac{2^k+1}{2^k+2} \cdot\text{tr}\left[(2^k\delta_{W_i, W'_i}\1_k  +2\delta_{W_i, \mathcal{C}(W'_i)\backslash \1_k}  W_iW_i'){\rho}^{(s)}_i\right]&\text{if~} W_i, W'_i \neq \1_k.
    %  \end{cases}\\
     &= \sum_{W,W'}\alpha_{W}\alpha_{W'}\cdot \sum_{s}\beta_s\prod_{i = 1}^n \begin{cases}
        \text{tr}({\rho}^{(s)}_i W_iW'_i)  &\text{if~} W_i=W_i' = \1_k,\\
         \text{tr}({\rho}^{(s)}_i W_i W'_i)&\text{if~} W_i = \1_k\neq W'_i,\\
         \text{tr}({\rho}^{(s)}_i W_i W'_i)&\text{if~} W'_i = \1_k\neq W_i,\\
         \frac{2^k+1}{2^k+2} \cdot\text{tr}\left[{\rho}^{(s)}_i(2^k\delta_{W_i, W'_i}+2\delta_{W_i, \mathcal{C}(W'_i)\backslash \1_k})W_iW_i'\right]&\text{if~} W_i, W'_i \neq \1_k.
     \end{cases}\\
    ...
    \begin{align}
        \forall W_i, W'_i \neq \1_k, \text{tr}({\rho}^{(s)}_i\{W_i,W_i'\}) = \begin{cases}0 \!\!\!&\text{if~} W'_i \notin \mathcal{C}(W_i)\backslash \1_k\\2\text{tr}(\tilde{\rho}^{(s)}_i W_iW'_i)  \!\!\!&\text{if~} W_i \in \mathcal{C}(W'_i)\backslash \1_k \end{cases} = 2\delta_{W_i, \mathcal{C}(W'_i)\backslash \1_k} \text{tr}(\tilde{\rho}^{(s)}_i W_iW'_i),
    \end{align}
...
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The inherent difference between the single-copy local Clifford strategy and the multi-copy one reflected on the set of $k$-qubit Pauli operators that commute with a given $k$-qubit Pauli operator $W$, which we denoted as $ \mathcal{C}(W)$.
In the single-copy case, the center of $P_i \in \{X,Y,Z\}$ satisfied $\mathcal{C}(P_i) = \{P_i, \1\}$.
     However, for example, for the two-qubit Pauli operators $XX$, extra terms such as $ZZ$ involved.
     We denote the $n$-qubit Pauli operators as $\sigma_{\mathbf{r}} = \bigotimes_{i=1}^n \sigma_{{r}_{x,i},{r}_{z,i}} \in \mathcal{P}_n$ with $\mathbf{r} = (\mathbf{r}_x = ({r}_{x,1},\cdots,{r}_{x,n}),\mathbf{r}_z = ({r}_{z,1},\cdots,{r}_{z,n})) \in (\mathbb{F}_2^n)^2$ and $\{\sigma_{00},\sigma_{01},\sigma_{10},\sigma_{11}\} = \{\1, Z,X,Y = iXZ\}$. We denote $a \oplus b = a + b \mathrm{~mod~}2 = a + b - 2a\cdot b$ and $\mathbf{a} \odot \mathbf{b} = (a_1\cdot b_1,\cdots,a_n\cdot b_n)$ with $a,b \in \{0,1\}$.
...
\begin{definition}[Covariance operator]
    \label{def:cov_oper}
    Given two operators with decomposition $O_\alpha = \sum_{W} \alpha_{W} W, O_\beta = \sum_{W} \beta_{W} W$ with $W \in \{\1,X,Y,Z\}^{\otimes k}$, the we defined the covariance operator as 
    \begin{align}
...
In the case of $k  =2$, $\text{tr}((O \otimes \1_n) \text{SWAP}^{\otimes n}\cdot (\rho \otimes \rho))$ is an unbiased estimator for $\text{tr}(O \rho^2)$. If we decomposed the operator $O$ on the Pauli basis, it turns out $(O \otimes \1_n)\text{SWAP}^{\otimes n} = \sum_{\mathbf{r} \in \mathbb{F}_2^{2n}} (\sigma_{\mathbf{r}} \otimes  \1_n)\text{SWAP}^{\otimes n}:= \sum_{\mathbf{r} \in \mathbb{F}_2^{2n}} O_{\mathbf{r}} $.
Here below we use Lemma~\ref{lemma:LC_shadow_kcopy} to evaluate the covariance operator of $O_{\mathbf{r}}$ explicitly. 
%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma%%%%%%%%%%%Lemma
\begin{lemma}[Second moment of two-copy Pauli]\label{lemma:covar_Pauli}
    If we take $O_{\mathbf{r}}= (\sigma_{\mathbf{r}} \otimes \1_n) \cdot \text{SWAP}^{\otimes n}$ for Pauli operator $\sigma_{\mathbf{r}}$ and denote $\text{wt}(\sigma_{\mathbf{r}})$ as the weight of this Pauli operator, then we have $\mathbf{V}(O_{\mathbf{r}}, O_{\mathbf{r}'}) = \bigotimes_{j = 1}^n v(\mathbf{r}_j, \mathbf{r}_j') $ with
    \begin{align}\label{eq:v_full_operator}
        ...
        \left\{\begin{aligned}
            ...
            &\frac{11\1_4 + 4 \Pi_{\mathrm{asym}}}{3} &\text{~if~} \mathbf{r}_j=\mathbf{r}_j'= \mathbf{0}, \\
            &\left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}_j})+ \frac{5}{6}\sigma_{\mathbf{r}_j}\otimes\1 &\text{~if~} \mathbf{r}_j' = \mathbf{0},  \mathbf{r}_j\neq \mathbf{0},\\
            &\left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}'_j}) + \frac{5}{6}\sigma_{\mathbf{r}'_j}\otimes\1&\text{~if~} \mathbf{r}_j = \mathbf{0},  \mathbf{r}'_j\neq \mathbf{0}.
        \end{aligned}\right.
    \end{align}
    In particular, the diagonal element gives $v(\mathbf{r}_j,\mathbf{r}_j) = ({5}/{3})\sigma_{\mathbf{r}_j}^{\otimes 2}$ when $\mathbf{r}_j \neq \mathbf{0}$ and $v(\mathbf{0},\mathbf{0}) = ({11\1_4 + 4 \Pi_{\mathrm{asym}}})/{3}$ at identity sites. Note that the $\Pi_{\mathrm{asym}}$ terms do \emph{not} vanish on $\rho^{\otimes 2}$ for entangled $\rho$: while $\rho^{\otimes 2}$ is globally symmetric under copy exchange, it is not contained in $\bigotimes_j \mathrm{Sym}^2_j$ when $\rho$ has entanglement across qubit sites. For product states $\rho = \bigotimes_l \rho_l$, the per-site state $\rho_l^{\otimes 2} \in \mathrm{Sym}^2(\mathbb{C}^2)$ so $\Pi_{\mathrm{asym}}$ terms vanish and the second moment simplifies to $\mathrm{tr}[\mathbf{V}(O_{\mathbf{r}},O_{\mathbf{r}})\rho^{\otimes 2}] = ({11}/{3})^{n}({5}/{11})^{\mathrm{wt}(\sigma_{\mathbf{r}})}$. For entangled states the second moment can be larger: since $v(\mathbf{0},\mathbf{0})$ has eigenvalues $11/3$ on $\mathrm{Sym}^2$ and $5$ on the antisymmetric subspace, the operator norm is $\|\mathbf{V}(O_\mathbf{0},O_\mathbf{0})\| = 5^n$.
\end{lemma}
\begin{proof}
    We consider the observable $O_{\mathbf{r}}= (\sigma_{\mathbf{r}} \otimes \1_n) \cdot \text{SWAP}^{\otimes n}$ with $\mathbf{r} = (\mathbf{r}_1, \cdots, \mathbf{r}_n) \in (\mathbb{F}_2^2)^n$ and rewrite the observable according to the fact $\text{SWAP}^{\otimes n} = 2^{-n}\sum_{\mathbf{p} \in \mathbb{F}_2^{2n}} \sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}}$ as:
    \begin{align}
    ...
    \begin{align}
        \text{the 1st term} &= \sum_{\mathbf{p}} (-1)^{[\mathbf{p},\mathbf{r}']}\cdot \frac{5}{6} \delta_{\mathbf{r},\mathbf{r}'}\sigma_{\mathbf{r}}\sigma_{\mathbf{r}'} \otimes \1 =0, \\
    ...
    \begin{align}
        ...
        &= 4 \sigma_{00}^{\otimes 2} - \frac{1}{3}(\sigma_{01}^{\otimes 2}+\sigma_{10}^{\otimes 2}+\sigma_{11}^{\otimes 2}) = \frac{11\1_2 + 4 \Pi_{\mathrm{asym}}}{3}.
    \end{align}
    Here in the last line we use the definition that $\Pi_{\mathrm{asym}} = (\1_2 - \text{SWAP})/2 = {(\1_2 -\sigma_{01}^{\otimes 2}-\sigma_{10}^{\otimes 2}-\sigma_{11}^{\otimes 2})}/{4}$.

    In the case of $\mathbf{r}'= 0 \neq \mathbf{r}$, if we simply adapt the result in Eq.\eqref{eq:V_eq1}, we will get the result $v({\mathbf{r}_j}, {\mathbf{r}_j'}) =({5}/{6})\left(\sigma_{\mathbf{r}}\otimes \1 +\1\otimes \sigma_{\mathbf{r}} \right)$. However, in the case of $\mathbf{p}' = 0$ during the summation, we mistakenly counted the constant $r((\mathbf{r}+ \mathbf{p})\mathbf{p}, \mathbf{0}\mathbf{0}) = 1$ into $r((\mathbf{r}+ \mathbf{p})\mathbf{p}, \mathbf{0}\mathbf{0}) = (5/6)[4\delta_{\mathbf{r},\mathbf{0}}\delta_{\mathbf{p},\mathbf{0}} +2]$. To correct this term, we sum in the case $\mathbf{p}' = 0$ and get the result
    \begin{align}
        \Delta = -4^{-1}(5/6)\sum_{\mathbf{p}} [4\delta_{\mathbf{p},0}\delta_{\mathbf{r},0} + 2]\sigma_{\mathbf{r}}\sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}} + 4^{-1} \sum_{\mathbf{p}} \sigma_{\mathbf{r}}\sigma_{\mathbf{p}} \otimes \sigma_{\mathbf{p}} = - \frac{1}{3}(\sigma_{\mathbf{r}} \otimes \1)\text{SWAP}.
    \end{align} 
    This correction then gives the correct result
    \begin{align}
        v({\mathbf{r}}, {\mathbf{r}'}) &=\frac{5}{6}\left(\1\otimes \sigma_{\mathbf{r}} +  \sigma_{\mathbf{r}}\otimes\1 \right) -  \frac{1}{3}(\sigma_{\mathbf{r}} \otimes \1) \cdot \text{SWAP} = \left(\frac{5}{6} \1 - \frac{1}{3}\text{SWAP}\right)(\1 \otimes \sigma_{\mathbf{r}}) + \frac{5}{6}\sigma_{\mathbf{r}}\otimes\1 \notag\\
        &= \left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}}) + \frac{5}{6}\sigma_{\mathbf{r}}\otimes\1 \text{~if~} \mathbf{r}' = \mathbf{0},  \mathbf{r}\neq \mathbf{0}.
    \end{align}
    Similarly, in the case $\mathbf{r}= 0 \neq \mathbf{r}'$, we calculate the correction in the case $\mathbf{p} = 0$ as
    \begin{align}
        \Delta &= -4^{-1}(5/6)\sum_{\mathbf{p}'}[4\delta_{\mathbf{p}',0}\delta_{\mathbf{r}',0} + 2]\sigma_{\mathbf{r}'}\sigma_{\mathbf{p}'} \otimes \sigma_{\mathbf{p}'} + 4^{-1} \sum_{\mathbf{p}'} \sigma_{\mathbf{r}'}\sigma_{\mathbf{p}'} \otimes \sigma_{\mathbf{p}'} = - \frac{1}{3}(\sigma_{\mathbf{r}'} \otimes \1)\text{SWAP},\\
        v({\mathbf{r}}, {\mathbf{r}'}) &= \left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}'}) + \frac{5}{6}\sigma_{\mathbf{r}'}\otimes\1 \text{~if~} \mathbf{r} = \mathbf{0},  \mathbf{r}'\neq \mathbf{0}.
    \end{align} 
    ...
    \begin{align}
        ...
        \left\{\begin{aligned}
            &\frac{5}{6}\left(\sigma_{\mathbf{r}_j}\otimes \sigma_{\mathbf{r}'_j} +\sigma_{\mathbf{r}'_j}\otimes \sigma_{\mathbf{r}_j} \right)&\text{~if~} \mathbf{r}_j, \mathbf{r}_j'  \neq \mathbf{0},\\
            &\frac{11\1_2 + 4 \Pi_{\mathrm{asym}}}{3} &\text{~if~} \mathbf{r}_j=\mathbf{r}_j'= \mathbf{0}, \\
            &\left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}_j})+ \frac{5}{6}\sigma_{\mathbf{r}_j}\otimes\1 &\text{~if~} \mathbf{r}_j' = \mathbf{0},  \mathbf{r}_j\neq \mathbf{0},\\
            &\left(\frac{1}{2} \Pi_{\mathrm{sym}} + \frac{7}{6} \Pi_{\mathrm{asym}}\right)(\1 \otimes \sigma_{\mathbf{r}'_j}) + \frac{5}{6}\sigma_{\mathbf{r}'_j}\otimes\1&\text{~if~} \mathbf{r}_j = \mathbf{0},  \mathbf{r}'_j\neq \mathbf{0}.
        \end{aligned}\right.
    \end{align}
    This completes the derivation of the full operator identity~\eqref{eq:v_full_operator}. For the special case $\mathbf{r} = \mathbf{r}'$, the non-identity sites give $v(\mathbf{r}_j,\mathbf{r}_j) = (5/3)\sigma_{\mathbf{r}_j}^{\otimes 2}$ while the identity sites give $v(\mathbf{0},\mathbf{0}) = (11\1_4 + 4\Pi_{\mathrm{asym}})/3$.

    \emph{State dependence of the second moment.} While $\rho^{\otimes 2}$ is globally symmetric under copy exchange, it is \emph{not} contained in $\bigotimes_j \mathrm{Sym}^2_j$ for entangled $\rho$. At each identity site, $v(\mathbf{0},\mathbf{0}) = (13\1_4 - 2\,\mathrm{SWAP})/3$ has eigenvalue $11/3$ on $\mathrm{Sym}^2$ and $5$ on the antisymmetric subspace. For product states $\rho = \bigotimes_l \rho_l$, the per-site state $\rho_l^{\otimes 2} \in \mathrm{Sym}^2(\mathbb{C}^2)$ and the $\Pi_{\mathrm{asym}}$ terms vanish, giving $\mathrm{tr}[\mathbf{V}(O_\mathbf{r},O_\mathbf{r})\rho^{\otimes 2}] = (11/3)^n(5/11)^{\mathrm{wt}(\sigma_\mathbf{r})}$. For entangled states the second moment can be larger; the operator norm bound gives $\|\mathbf{V}(O_\mathbf{0},O_\mathbf{0})\| = 5^n$.
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We consider the estimation of the simplest operator $O^{(2)} = \text{SWAP}^{\otimes n} = 2^{-n}(\1\1+XX+YY+ZZ)^{\otimes n}$.
By Lemma~\ref{lemma:covar_Pauli}, the second moment operator for purity estimation is $\mathbf{V}(O_\mathbf{0},O_\mathbf{0}) = \left({(11\1_4 + 4\Pi_{\mathrm{asym}})}/{3}\right)^{\otimes n}$. For product states the second moment is $(11/3)^n$; for entangled states it can be larger (operator norm $5^n$). In all cases the cost is exponential.
For the single copy local Clifford shadow, the variance is calculated through:
\begin{align}
    &\hspace{-1.5cm}\mathbf{V}:= \sum_{W,W' \in \mathcal{P}_2^{\otimes n}} \alpha_{W}\alpha_{W'} R(W,W') W W'= 4^{-n}\bigotimes_{i =1}^n \left(\sum_{W_i,W_i' \in \{\1\1,XX,YY,ZZ\}} R(W_i,W'_i) W_i W_i'\right).\notag\\
   = 4^{-n} &\left( \1\1+XX+YY+ZZ+ XX+9\1\1 + YY + 9\1\1 +ZZ+ 9\1\1\right)^{\otimes n}\notag\\
  & \hspace{-1.15cm}= \left(7 \1\1 + \frac{1}{2}(XX+YY+ZZ)\right)^{\otimes n}= \left(\frac{13\1\1+2\text{SWAP}}{2}\right)^{\otimes n} = \left(\frac{11\1\1+ 4\Pi_{\mathrm{sym}}}{2}\right)^{\otimes n}. \label{eq:prity_1rep_shadow_variance}
\end{align}
For the single-copy shadow, this gives a worst-case second moment of $7.5^n$ (the operator norm, attained on product states since the symmetric eigenvalue $15/2$ exceeds the antisymmetric eigenvalue $11/2$). The 2-replica local shadow improves the product-state second moment to $(11/3)^n \approx 3.67^n$, but the antisymmetric eigenvalue $5 > 11/3$ means entangled states can have larger variance (operator norm $5^n$). In all cases, the cost remains exponential for $k \leq 2$. \color{red}We conjecture that using higher replica numbers $k > 2$ under local measurement constraints does not alter the complexity class, since the structural obstruction (non-product form of the lifted operators $A_P$) persists at every $k$; however, a rigorous proof for arbitrary $k$ remains open.\color{black}
...
\begin{theorem}[A $2k$-copy shadow protocol for $|\mathrm{tr}(P\rho^k)|$]
    Let $\mathcal{P}_n$ be the $n$-qubit Pauli group. Let $\mathbb{P}^{d}_{\pi}$ be the permutation operator on $2k$ replica of $n$-qubits ($d = 2^n$) and $\pi =(\mathrm{odds})(\mathrm{even})$ is a product of two $k$-cycles acting on odd labels $(1,3,5,\cdots,2k-1)$ and even labels $(2,4,\cdots,2k)$ respectively.
    ...
    The family of observables $\{A_P\}_{P \in \mathcal{P}_n}$ is a commuting family, i.e., $[A_P, A_Q] = 0$ for all $P, Q \in \mathcal{P}_n$.
    Furthermore, its expectation value on $\rho^{\otimes 2k}$ is
    $\mathrm{tr}(A_P \rho^{\otimes 2k}) = [\mathrm{tr}(P\rho^k)]^2 $.
    Therefore, a single projective measurement in the common eigenbasis of $\{A_P\}_P$ allows for the simultaneous estimation of $|\mathrm{tr}(P\rho^k)|$ for all $P \in \mathcal{P}_n$.
\end{theorem}
    \begin{proof}
    We first prove the commutativity of the family $\{A_P\}_{P \in \mathcal{P}_n}$.
    ...
    We now calculate 
    \begin{align}
        \mathrm{tr}(A_P \rho^{\otimes 2k}) &= \mathrm{tr}[\mathrm{sym}_{\pi}(P^{\otimes 2} \otimes \1^{\otimes 2k-2}) \pi \rho^{\otimes 2k}] = \mathrm{tr}[(P^{\otimes 2} \otimes \1^{\otimes 2k-2}) \pi \rho^{\otimes 2k}]\\
...
\end{proof}
\paragraph{Examples.}
 In the case $k = 2$, the commutants we choose writes $\{A_{\mathbf{r}} := {2}^{-1}{(\sigma_{\mathbf{r}}\sigma_{\mathbf{r}}\1_n\1_n + \1_n\1_n\sigma_{\mathbf{r}}\sigma_{\mathbf{r}})} \mathrm{S}_{13}\mathrm{S}_{24}\}_{\mathbf{r} \in \mathbb{F}_2^{2n}}$. Notice that $\sigma_{\mathbf{r}} \otimes \sigma_{\mathbf{r}} \ket{\Phi_{\mathbf{a}}} = (-1)^{[\mathbf{r},\mathbf{a}] + \mathbf{r}_x\cdot\mathbf{r}_z}\ket{\Phi_{\mathbf{a}}}$ (cf.\ Eq.~\eqref{eq:bell_shadow_eig}),
the shared eigenbasis of $\{A_\mathbf{r}\}_\mathbf{r}$ has the form $\ket{\Psi^{s}_{(\mathbf{a}, \mathbf{b})}}$ below with eigenvalues $\lambda_{(s,\mathbf{a},\mathbf{b})} :=(-1)^{\mathbf{r}_x\cdot\mathbf{r}_z}\cdot s \cdot 2^{-1}[(-1)^{[\mathbf{r}, \mathbf{a}]} + (-1)^{[\mathbf{r}, \mathbf{b}]}] $ and $s = \pm 1$.
\begin{align}
...
Note that the last $\text{CNOT}$ gates on computational basis can be simulated by classical post-processing. This then gives the $\mathbf{a}$ vector through $\mathbf{a}^{E} = {\mathbf{s}^{E}}$ and $s = +1, \mathbf{a}^{I} = {s}^{I}$ if $a_{i_1} = 0$, $s = -1, \mathbf{a}^{I} = \bar{s}^{I}$ if $a_{i_1} = 1$.
\begin{figure}[htbp!]
    \centering
...
\end{lemma}
\begin{proof}
    % [Note: Proof corrected in previous revision — fixed $S_4$ cycle counting and variance formula]
    \begin{align}
    ...
    &=\mathrm{tr}\left(\frac{\varepsilon^{-2} \1_{n}^{\otimes 2}+\varepsilon^{-1} \mathrm{SWAP}^{\otimes n}}{D(D+1)} \cdot \mathrm{SWAP}^{\otimes n}\right) = \frac{\varepsilon^{-2} 2^n + \varepsilon^{-1} 2^{2n}}{2^n\varepsilon^{-1}(2^n \varepsilon^{-1}+1)}= \frac{2^n\varepsilon + 1}{2^n + \varepsilon},\\
    ...
    \begin{align}
        \mathrm{Var}_{\rho \sim \mathcal{E}_{\varepsilon}} [\mathrm{tr}(\rho^2)] &= \frac{d_A^2 d_B^4+2 d_A^3 d_B^3+4 d_A d_B^3+10 d_A^2 d_B^2+4 d_A^3 d_B+2 d_A d_B+d_A^4 d_B^2}{D(D+1)(D+2)(D+3)} - \frac{(d_A + d_B)^2}{(D+1)^2}\notag\\
        &= \frac{2(d_A^2-1)(d_B^2-1)}{(D + 1)^2 (D+2)(D+3)} = \frac{2(2^{2n}-1)(1/\varepsilon^2-1)}{(2^n/\varepsilon + 1)^2 (2^n/\varepsilon+2)(2^n/\varepsilon+3)}
    \end{align} 
    \begin{table}[htbp!]
        \centering
        ...
        (13),(14),(23),(24)&  &  & (1234),(1243),(1342),(1432) & 1 & $4 d_A d_B^3$ \\ \hline
        $\substack{(123),(132),(124),(142),\\(134),(143),(234),(243)}$& $2C_4^3$ & 2 & $\substack{(134),(234),(143),(243),\\(123),(124),(132),(142)}$ & 2 & $8 d_A^2 d_B^2$ \\ \hline
        $\substack{(1234),(1243),\\(1342),(1432)}$ & 4 & 1 & $(13),(14),(23),(24)$ & 3 & $4 d_A^3 d_B$ \\ \hline
        $(1324),(1423)$ & 2 & 1 & $(1423),(1324)$ & 1 & $2 d_A d_B$ \\ \hline
        $\substack{(12)(34),}$ & 3 & 2 & $e$ & 4 & $d_A^4 d_B^2$ \\ \cline{4-6}
         $\substack{(13)(24),(14)(23)}$& & & $(14)(23),(13)(24)$ & 2 & $2 d_A^2 d_B^2$ \\ \hline
        \end{tabular}
...
\begin{problem}\label{prob:RHtest}
    ...
    \begin{align}
        &\mathcal{E}_{(1,0),C} := \left\{\sigma_{(1,0),C}:= C \left[\frac{1}{2} \ket{0}\!\bra{0} \otimes \psi + \frac{1}{2} \ket{1}\!\bra{1} \otimes (\1/2^{n-1}) \right] C^{\dagger}\right\}_{\psi \sim \mathrm{Haar}_{n-1}},\\
        &\mathcal{E}_{(1 - R\delta,L\delta),C} := \left\{ \sigma_{(1 - R\delta,L\delta),C} :=C \left[\frac{1}{2} \ket{0}\!\bra{0} \otimes \rho_{1 - R\delta} + \frac{1}{2} \ket{1}\!\bra{1} \otimes \rho_{L\delta} \right] C^{\dagger}\right\}_{\rho_{\varepsilon} \sim \mathcal{E}_{\varepsilon}}.
    \end{align}
\end{problem}
...
\begin{align}
    ...
    &\mathbb{E}_{\rho \sim \mathcal{E}_{(1 - R\delta,L\delta),C}} \mathrm{tr}\left(\rho^{2}\right) = \frac{1}{4}\frac{2^{n-1}(1 - R\delta) + 1}{2^{n-1} + 1 - R\delta} + \frac{1}{4}\frac{2^{n-1} L\delta + 1}{2^{n-1} + L\delta} = \frac{1}{4}(1 - \delta) + \frac{1}{4}(\frac{1}{2^{n-1}} + \delta) = \frac{1}{4}\left(1+\frac{1}{2^{n-1}}\right).
\end{align}
...
\begin{align}
    \mathbb{E}_{\psi}\sigma_{(1,0),I}^{\otimes 2T} &= \left[\frac{1}{2}\ket{0}\!\bra{0} \otimes \psi  + \frac{1}{2}\ket{1}\!\bra{1} \otimes 2^{-n+1}\1_{n-1}\right]^{\otimes 2T} = \sum_{\mathbf{a} \in \mathbb{F}_2^{2T}} {2^{-2T}\ket{\bar{\mathbf{a}}}\!\bra{\bar{\mathbf{a}}} }\otimes\frac{\sum_{\pi \in S_{2T}^{\mathbf{a}}} \mathbb{P}_{\pi}^{d}}{d^{\uparrow |\mathbf{a}|} d^{2T - |\mathbf{a}|}},\\
...
\begin{align}
  \Delta_{2} =  4^{-1}  &{\ket{11}\!\bra{11}} 
     \otimes \left[\frac{\1^{\otimes 2}}{ d^{2}} - \frac{\1^{\otimes 2} (L\delta)^{-2} + \mathbb{P}^{d}_{(12)}(L\delta)^{-1}}{[d/L\delta][d/L\delta + 1] }\right] \\
     &+ 4^{-1}  {\ket{00}\!\bra{00}} 
     \otimes \left[\frac{\1^{\otimes 2}  + \mathbb{P}^{d}_{(12)}}{d[d + 1] } - \frac{\1^{\otimes 2} (1 -R\delta)^{-2} + \mathbb{P}^{d}_{(12)}(1 -R\delta)^{-1}}{[d/(1 -R\delta)][d/(1 -R\delta) + 1] }\right].
\end{align}
...
\begin{align}
    \Phi_{\mathrm{Cl},2}[\Delta_2] &= \Phi_{\mathrm{Cl},2}\left[4^{-1} {\ket{11}\!\bra{11}} 
     \otimes \left[-\frac{\1^{\otimes 2} + \mathbb{P}^{d}_{(12)}L\delta}{d[d + L\delta] } +  \frac{\1^{\otimes 2}}{ d^{2}} + \frac{\1^{\otimes 2}  + \mathbb{P}^{d}_{(12)}}{d[d + 1] } -\frac{\1^{\otimes 2} + \mathbb{P}^{d}_{(12)}(1 -R\delta)}{d[d+ 1 -R\delta] } \right]\right]\\
     &=\Phi_{\mathrm{Cl},2}\left[4^{-1} {\ket{11}\!\bra{11}} 
     \otimes \left[\left(-\frac{1}{d[d + L\delta] } +  \frac{1}{ d^{2}} + \frac{1}{d[d + 1] } -\frac{1}{d[d+ 1 -R\delta] } \right)\1^{\otimes 2} +\right.\right.\notag\\
    ...
    &=\Phi_{\mathrm{Cl},2}\left[4^{-1} {\ket{11}\!\bra{11}} 
     \otimes \left[\left(\frac{L\delta}{d^2(d + L \delta)} -\frac{R\delta}{d(d+1)(d+1 - R\delta)} \right)\1^{\otimes 2} +\right.\right.\notag\\
    & \hspace{13em}\left.\left. \left(-\frac{L\delta}{d[d + L\delta] } + \frac{R\delta d}{d(d+1)(d+1-R\delta)}\right)\mathbb{P}^{d}_{(12)}\right]\right]\\
    &=\Phi_{\mathrm{Cl},2}\left[4^{-1} {\ket{11}\!\bra{11}}
     \otimes \left[\left(\frac{\delta}{d(d^2 -1)} -\frac{\delta}{d(d+1)(d - 1)} \right)\1^{\otimes 2} +\right.\right.\notag\\
...
\begin{align}
    &\rho_0 := \mathbb{E}_{\rho \sim \mathcal{E}_{(1,0),C\sim \mathcal{D}}} \rho^{\otimes 2}=\mathbb{E}_{\rho \sim \mathcal{E}_{(1 - R\delta,L\delta),C\sim \mathcal{D}}} \rho^{\otimes 2} = a\cdot \1_{2n}/2^{2n} + b\cdot \mathrm{SWAP}^{\otimes n}/2^{n}, \\
    & a + b = 1, \quad 2^{-n} a + 2^{n} b = \text{Avg. Purity} = 4^{-1}(1 + 2^{-n+1}).
\end{align}
Solve the equation for $a$ and $b$, we get the result:
\begin{align}
    \rho_0 = (1 - \mu)  \frac{\1_n^{\otimes 2}}{2^{2n}} + \mu \frac{\1_n^{\otimes 2} + \mathrm{SWAP}^{\otimes n}}{2^n(2^{n} + 1)}, \text{~where~} \mu = 4^{-1} \frac{2^n -2}{2^n - 1}.
\end{align}
This prevents the Lemma~\ref{lemma:hard_necess1} giving an efficient 2-replica strategy that discriminates $\mathcal{E}_{(1,0),C} $ and $\mathcal{E}_{(1 - R\delta,L\delta),C} $.
...
\begin{align}
    ...
    &\mathbb{E}_{\rho \sim \mathcal{E}_{(1 - R\delta,L\delta),C}} \mathrm{tr}\left(P\rho^{2}\right)  = \frac{1}{4}(1 - \delta) - \frac{1}{4}(\frac{1}{2^{n-1}} + \delta) = \frac{1}{4}\left(1- \frac{1}{2^{n-1}}\right) - \frac{\delta}{2}.
\end{align}
This means that if one can use strategy $\mathcal{A}$ to estimate $\mathrm{tr}(P \rho^2), \forall P \in \{C^\dagger Z_1 C\}_{C \in \mathcal{C}}$ within additive error $\delta/4$, he can use $\mathcal{A}$ to discriminate $\mathcal{E}_{(1,0),C} $ and $\mathcal{E}_{(1 - R\delta,L\delta),C} $, as rigorously proved below:
\begin{lemma}\label{lemma:hard_necess1}
    An algorithm that estimates $\mathrm{tr}(P \rho^2), \forall P \in \{C Z_1 C^\dagger\}_{C \in \mathcal{C}}$ can be used to distinguish $\mathcal{E}_{(1,0),C} $ from $\mathcal{E}_{(1 - R\delta,L\delta),C} $ for arbitrary distribution on $\mathcal{C}$.
\end{lemma}
\begin{proof}
    \textcolor{red}{Suppose algorithm $\mathcal{A}$ estimates $\mathrm{tr}(P\rho^2)$ within additive error $\delta/4$ with success probability $\geq 2/3$, for all $P \in \{CZ_1C^\dagger\}_{C \in \mathcal{C}}$. Given $C$ and copies of $\sigma$ drawn from either $\mathcal{E}_{(1,0),C}$ or $\mathcal{E}_{(1-R\delta,L\delta),C}$, run $\mathcal{A}$ with $P = CZ_1C^\dagger$ to obtain estimate $\hat{v}$ of $\mathrm{tr}(P\sigma^2)$. Note that $\mathrm{tr}(P\sigma^2)$ is constant over $\mathcal{E}_{(1,0),C}$ (since $\psi$ is pure and $Z_1$ is block-diagonal in the $\ket{0}/\ket{1}$ basis), equaling $\frac{1}{4}(1 - 2^{1-n})$. For $\sigma \in \mathcal{E}_{(1-R\delta,L\delta),C}$, by Lemma~\ref{lemma:purity_of_RH} the purity concentrates, so $\mathrm{tr}(P\sigma^2)$ concentrates around $\frac{1}{4}(1 - 2^{1-n}) - \delta/2$ with variance $O(D^{-2})$. Thresholding $\hat{v}$ at $\frac{1}{4}(1 - 2^{1-n}) - \delta/4$ thus discriminates the two ensembles with success probability $\geq 2/3 - o(1)$, uniformly over $C$.}
\end{proof}
...
% \begin{align}
%  &\leq \Phi_{\mathrm{Cl},2T}\left[\sum_{\mathbf{a} \in \mathbb{F}_2^{2T}} {2^{-2T}\ket{\bar{\mathbf{a}}}\!\bra{\bar{\mathbf{a}}}} \otimes \left( \frac{\sum_{\pi \in S_{2T}} \mathbb{P}_{\pi}^{d}}{d^{\uparrow 2T} } + \frac{\1(d)^{\otimes 2T}}{ d^{2T}} \right. \right.-\left.\left.\frac{\sum_{\pi \in S_{2T}} \mathbb{P}_{\pi}^{d} (1 - R \delta)^{-c(\pi)} }{[d/(1 - R\delta)]^{\uparrow 2T} }   - 
...
%     \otimes \frac{ \sum_{\pi \in S_{|\mathbf{a}|}}\mathbb{P}_{\pi}^{d}}{d^{\uparrow |\mathbf{a}|}} \otimes \left[\frac{ d^{\uparrow |\mathbf{a}|}(1 - R \delta)^{-c(\pi)}}{[d/L\delta]^{\uparrow |\bar{\mathbf{a}}|}[d/(1 - R\delta)]^{\uparrow |\mathbf{a}|} }{\sum_{ \sigma \in S_{|\bar{\mathbf{a}}|}}\mathbb{P}_{\sigma}^{d}   (L\delta)^{-c(\sigma)}} -  \frac{\1_{n-1}^{\otimes |\bar{\mathbf{a}}|}}{ d^{|\bar{\mathbf{a}}|}}\right].
% \end{align}
\begin{align}
...
\begin{align}
    &\frac{\left[\sum_{\pi \in S_2} \mathbb{P}_{\pi}^d (1 - R\delta)^{-c(\pi)}\right]^{\otimes T}}{[d/(1 - R\delta)]^{\uparrow 2T} }= \frac{d^T(d+1-R\delta)^T}{[d/(1 - R\delta)]^{\uparrow 2T} }\left[\frac{\1^{\otimes 2} + \mathbb{P}^{d}_{(12)}(1 -R\delta)}{d[d+ 1 -R\delta] }\right]^{\otimes T} \\
    &=d^T(d+1-R\delta)^T\left[-\frac{\1^{\otimes 2} + \mathbb{P}^{d}_{(12)}L\delta}{d[d + L\delta] } +  \frac{\1^{\otimes 2}}{ d^{2}} + \frac{\1^{\otimes 2}  + \mathbb{P}^{d}_{(12)}}{d[d + 1] } \right]^{\otimes T}
\end{align}
...
\end{align}
The trace term can be explicitly calculated as
\begin{align}
...
\end{align}
The difference of two average can then be reform into the discrimination between $\1_{n}^{\otimes 2T}/d^{2T}$ from the mixed state
\begin{align}
    \Phi_{\mathrm{U},2T}\left[ \mathbb{E}_{\psi}\sigma_{(1,0),I}^{\otimes 2T} \right] -\Phi_{\mathrm{U},2T}\left[ \mathbb{E}_{\psi}\sigma_{(1-R\delta,L\delta),I}^{\otimes 2T} \right] =  \1_{n}^{\otimes 2T}/D^{2T} + \left[\sum_{\rho \in S_{2T}} \lambda_{\rho} \mathbb{P}_{\rho}^{D} \right]
\end{align}
...
\end{align}
This further reduced the problem to the discrimination between $\1(D)^{\otimes 2}/D^2$ and $\sum_{\rho \in S_{2}} \lambda'_{\rho} \mathbb{P}_{\rho}^{D} $

\begin{lemma}[Martingale distinguishing lower bound]\label{theo:mart_distinguish_lb}
    \textcolor{red}{Let $\varepsilon > 0$ be a constant. To distinguish state $\rho_0$ from a set of states $\mathcal{S}$ with success probability $\geq 1/2 + \varepsilon$, using any $T$-round adaptive strategy with POVMs drawn from a restricted class $\mathcal{M}$, the number of rounds must satisfy $T \geq c(\varepsilon) \cdot \delta_{\mathcal{M},\rho_0,\mathcal{S}}^{-1}$, where $c(\varepsilon) > 0$ depends only on $\varepsilon$, and the single-round chi-squared divergence is
    \begin{align}
        \delta_{\mathcal{M},\rho_0,\mathcal{S}} := \max_{\{F_s\} \in \mathcal{M}} \mathbb{E}_{\sigma \sim \mathcal{S}} \sum_s \frac{[\mathrm{tr}(\sigma F_s) - \mathrm{tr}(\rho_0 F_s)]^2}{\mathrm{tr}(\rho_0 F_s)}.
    \end{align}
    This follows from a martingale analysis of the likelihood ratio process; see~\cite{chen2021exponential} for the detailed proof in the quantum setting.}
\end{lemma}

Based on Lemma~\ref{theo:mart_distinguish_lb}, we calculate the chi-squared divergence for the reduced two-copy problem.
\color{red}
The reduced problem discriminates $\rho_0 = \1_D^{\otimes 2}/D^2$ from $\sigma = \sum_{\rho \in S_2}\lambda'_\rho \mathbb{P}_\rho^D$, where $\lambda'_e, \lambda'_{(12)}$ are determined by the leaves likelihood reduction. Since $\sigma = \lambda'_e \1_D^{\otimes 2} + \lambda'_{(12)}\mathrm{SWAP}_D$ and $\mathrm{tr}(\rho_0 F_s) = D^{-2}\mathrm{tr}(F_s)$, the chi-squared divergence under measurement class $\mathcal{M}$ evaluates to
\begin{align}
    \delta_{\mathcal{M}} &= D^2 \max_{\{F_s\} \in \mathcal{M}} \sum_s \frac{[(\lambda'_e - D^{-2})\mathrm{tr}(F_s) + \lambda'_{(12)}\mathrm{tr}(\mathrm{SWAP}_D\, F_s)]^2}{\mathrm{tr}(F_s)}.
\end{align}
For the RH-ensemble (purity-identity test), $\sigma = (\1_D^{\otimes 2} + \mathrm{SWAP}_D)/[D(D+1)]$, so $\lambda'_e = \lambda'_{(12)} = [D(D+1)]^{-1}$, and the above simplifies to (cf.~\cite{chen2021exponential})
\begin{align}\label{eq:chi2_RH}
    \delta_{\mathcal{M}} = \frac{1}{(D+1)^2}\left[\max_{\{F_s\} \in \mathcal{M}} \sum_s \frac{[\mathrm{tr}(\mathrm{SWAP}_D\, F_s)]^2}{\mathrm{tr}(F_s)} - 1\right].
\end{align}
For local 2-replica measurements $F_s = \bigotimes_{i=1}^n F_{s_i}$, the swap factorizes as $\mathrm{SWAP}_D = \bigotimes_{i=1}^n \mathrm{SWAP}_2^{(i)}$ across qubit sites, giving
\begin{align}
    \max_{\{F_s\} \in \mathcal{M}_{\mathrm{loc}}} \sum_s \frac{[\mathrm{tr}(\mathrm{SWAP}_D\, F_s)]^2}{\mathrm{tr}(F_s)} = \prod_{i=1}^n \gamma_i, \quad \gamma_i := \max_{\{F_{s_i}\}} \sum_{s_i} \frac{[\mathrm{tr}(\mathrm{SWAP}_2\, F_{s_i})]^2}{\mathrm{tr}(F_{s_i})} = 4,
\end{align}
where $\gamma_i = 4$ is attained by Bell measurements (the eigenstates of $\mathrm{SWAP}_2$). Thus $\delta_{\mathrm{loc}} = (D^2 - 1)/(D+1)^2 = (D-1)/(D+1) = \Theta(1)$, yielding only the trivial bound $T = \Omega(1)$ from Lemma~\ref{theo:mart_distinguish_lb}.

The key insight is that $\mathrm{SWAP}_D$ factors over qubit sites, so local 2-replica measurements (with Bell at each site) are as powerful as entangled measurements for the purity-identity test. An exponential lower bound for local strategies therefore requires an ensemble where the signal is distributed across \emph{non-factoring} operators. This motivates the Clifford-dressed \textbf{MH}-ensemble below, where the Clifford gate spreads the distinguishing observable across high-weight Pauli operators that cannot be captured by any fixed local measurement basis.
\color{black}

\subsubsection{Subspace \textbf{MH}-ensemble test}
\begin{problem}
    Given the state ensemble $\mathcal{E}_{1,\mathrm{Cl}}:= \{\frac{1}{2} \mathrm{Cl}\left[\ket{0}\!\bra{0} \otimes \psi + \ket{1}\!\bra{1} \otimes \1/{2^{n-1}}\right]\mathrm{Cl}^\dagger\}_{\psi \sim \mathrm{Haar}_{n-1}}$ and the mixed state ensemble $\mathcal{E}_{2,\mathrm{Cl}}:=\left\{\mathrm{Cl}\left[{\1}/{2} \otimes \left(\frac{1}{2}\psi_1 + \frac{1}{2}\psi_2\right)\right]\mathrm{Cl}^\dagger\right\}_{\psi_1,\psi_2 \sim \mathrm{Haar}_{n-1}}$, try to design an algorithm using restrained measurements to distinguish these two ensembles with success rate $1/2 + \delta$.
\end{problem}
Given a fixed $n$-qubit Clifford gate $\mathrm{Cl} = \1$, these two ensembles have two-replica average as
\begin{align}
    &\mathbb{E}_{\rho \sim \mathcal{E}_1}\rho^{\otimes 2} = \frac{1}{4}\left[\ket{01}\!\bra{01} + \ket{10}\!\bra{10} + \ket{00}\!\bra{00}\right] \otimes \left(\frac{\1_{n-1}}{d}\right)^{\otimes 2} + \frac{1}{4}\ket{11}\!\bra{11} \otimes \left[\frac{\1_{n-1} \otimes \1_{n-1} + \mathrm{SWAP}^{\otimes (n-1)}}{d(d+1)}\right]\\
    &\mathbb{E}_{\rho \sim \mathcal{E}_2} \rho^{\otimes 2} = \left(\frac{\1}{2}\right)^{\otimes 2} \otimes \frac{1}{2} \left[\frac{\1_{n-1} \otimes \1_{n-1} + \mathrm{SWAP}^{\otimes (n-1)}}{d(d+1)} + \frac{\1_{n-1}}{d} \otimes \frac{\1_{n-1}}{d}\right]\\
    &\Delta = \frac{\ket{00}\!\bra{00} + \ket{10}\!\bra{10} +\ket{01}\!\bra{01} -\ket{11}\!\bra{11}}{4} \otimes \frac{1}{2}\left[\frac{\1_{n-1} \otimes \1_{n-1}}{d^2(d+1)} - \frac{\mathrm{SWAP}^{\otimes (n-1)}}{d(d+1)} \right]
\end{align}
In the case $\mathrm{Cl} = \1$, we have 2-replica localized strategy $\{\ket{00},\ket{01},\ket{10},\ket{11}\}$ on the first qubits and Bell measurement on the last $(n-1)$ qubits such that $\langle \Delta \rangle \sim \mathcal{O}(1)$.  
However, in the case $\mathrm{Cl}$ maps $Z_1$ to a weight $w$ Pauli operator, we show that $\mathrm{tr}(\mathrm{Cl}^{\otimes 2} \Delta \mathrm{Cl}^{\dagger,\otimes 2} \cdot \bigotimes_{j=1}^n\Phi_j) \leq d^2 2^{-\alpha \cdot w}$. This means that non-adaptive 2-replica local measurement strategy at least repeats $2^w$ times to discriminate $\mathcal{E}_{1,\mathrm{Cl}}$ from $\mathcal{E}_{2,\mathrm{Cl}}$
In the case of $T$-round of $2$-replica strategy, we analysis the $2T$-fold average as:
\begin{align}
    \mathbb{E}_{\rho \sim \mathcal{E}_1}\rho^{\otimes 2T} &= \frac{1}{2^{2T}} \sum_{\mathbf{a} \in \mathbb{F}_2^{2T}}\ket{\mathbf{a}}\!\bra{\mathbf{a}} \otimes  \frac{S^{\mathbf{a}}}{d^{\uparrow \mathbf{a}}} \otimes \frac{\1^{\bar{\mathbf{a}}}}{d^{|\bar {\mathbf{a}}|}},\\
    \mathbb{E}_{\rho \sim \mathcal{E}_2}\rho^{\otimes 2T} &= \frac{1}{2^{2T}} \sum_{\mathbf{a} \in \mathbb{F}_2^{2T}} \ket{\mathbf{a}}\!\bra{\mathbf{a}} \otimes \frac{S^{\mathbf{a}}}{d^{\uparrow \mathbf{a}}}
\end{align}
\color{red}
\emph{Argument strategy for the exponential lower bound.}
When $\mathrm{Cl}$ is drawn uniformly from $\mathcal{C}$, the Clifford 2-design property gives
$\rho_0 := \mathbb{E}_{C,\psi}\sigma_{C}^{\otimes 2} = (1-\mu)\frac{\1^{\otimes 2}}{D^2} + \mu \frac{\1^{\otimes 2}+\mathrm{SWAP}}{D(D+1)}$
for both ensembles (cf.\ Eq.~\eqref{eq:chi2_RH} and the discussion above), so 2-replica averages are indistinguishable.
For a $T$-round adaptive 2-replica strategy with measurement class $\mathcal{M}$, the chi-squared divergence (Lemma~\ref{theo:mart_distinguish_lb}) satisfies
\begin{align}
    \delta_{\mathcal{M}} &= \max_{\{F_s\} \in \mathcal{M}} \sum_s \frac{\mathbb{E}_{C,\psi}[\mathrm{tr}((\sigma_C^{\otimes 2} - \rho_0)F_s)]^2}{\mathrm{tr}(\rho_0 F_s)} \notag\\
    &\leq (1-\mu)^{-1} D^2 \max_{\{F_s\}} \sum_s \frac{\mathrm{tr}[\Phi_{\mathrm{Cl},4}(\mathbb{E}_\psi (\sigma_I^{\otimes 2} - \rho_0)^{\otimes 2}) \cdot F_s^{\otimes 2}]}{\mathrm{tr}(F_s)},
\end{align}
where the second line uses $C^{\otimes 2}\rho_0(C^\dagger)^{\otimes 2} = \rho_0$ and rewrites the Clifford average as a 4-fold channel $\Phi_{\mathrm{Cl},4}$.
Since the Clifford group is a 3-design, $\Phi_{\mathrm{Cl},4}(\mathbb{E}_\psi \sigma_I^{\otimes 4}) - \Phi_{\mathrm{U},4}(\mathbb{E}_\psi \sigma_I^{\otimes 4})$ reduces to the projection onto the 4th-order Clifford commutant modulo the Haar component. Specifically:
\begin{align}
    \Phi_{\mathrm{Cl},4}(\mathbb{E}_\psi(\sigma_I^{\otimes 2}-\rho_0)^{\otimes 2}) = \Phi_{\mathrm{Cl},4}(\mathbb{E}_\psi \sigma_I^{\otimes 4}) - \rho_0^{\otimes 2}.
\end{align}

[TODO: Complete the evaluation of $\Phi_{\mathrm{Cl},4}(\mathbb{E}_\psi \sigma_I^{\otimes 4})$. The 4-fold Clifford commutant is generated by $S_4$ permutations and the operator $\Omega_4 = \sum_{P \in \mathcal{P}_n} P^{\otimes 4}$ (cf.~\cite{gross2021schur,zhu2016clifford}). The key steps are:
(1)~Decompose $\mathbb{E}_\psi \sigma_I^{\otimes 4}$ in the Clifford commutant basis $\{\mathbb{P}_\pi, \Omega_4\}_{\pi \in S_4}$ on $D = 2d$ qubits.
(2)~For local 2-replica measurements $F_s = \bigotimes_{i=1}^n F_{s_i}$, the factored structure $F_s^{\otimes 2} = \bigotimes_i F_{s_i}^{\otimes 2}$ interacts with $\Omega_4 = \sum_P P^{\otimes 4} = \bigotimes_i (\sum_{P_i} P_i^{\otimes 4})$, which does NOT factor as a single permutation operator. Bound $\mathrm{tr}(\Omega_4 \cdot F_s^{\otimes 2}) \leq (5/3)^n \mathrm{tr}(F_s)$ using the single-qubit 4th moment structure.
(3)~Show $\delta_{\mathcal{M}} = O(2^{-n})$, yielding $T = \Omega(2^n)$ via Lemma~\ref{theo:mart_distinguish_lb}.]
\color{black}
